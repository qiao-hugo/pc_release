...
foreach($result as $row){
    ...
    if($adb->num_rows($adb->pquery($query,array($row['servicecontractid'])))){//如果存在订单，以订单为准不以工单为准
        continue;
    }
    //清除重复使用的回款金额
    $delSql="DELETE FROM vtiger_contractperformancecostnew WHERE servicecontractsid=? and receivedpaymentsids=?";
    $adb->pquery($delSql, array($row['servicecontractid'],$row['receivedpaymentsid'].','));
    $sql = "UPDATE vtiger_contractperformancecostnew SET repuntilprice=if(repuntilprice-?<=0,0,repuntilprice-?),receivedpaymentsids=REPLACE(receivedpaymentsids,'".$row['receivedpaymentsid'].",','') WHERE servicecontractsid=? and FIND_IN_SET(?,receivedpaymentsids)";
    $adb->pquery($sql, array($row['unit_price'],$row['unit_price'],$row['servicecontractid'],$row['receivedpaymentsid']));
    ...
    if(!empty($salesorderInfo['performanceoftime']) && $salesorderInfo['modulestatus']!='c_lackpayment'){//  工单审核 且 回款不为  回款不足
        ...
        if(!empty($row['productid'])){
            if($row['multitype']==1){
                $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_salesorderproductsrel.productnumber,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid LEFT JOIN vtiger_salesorderrayment as sd ON sd.salesorderid=vtiger_salesorderproductsrel.salesorderid   WHERE vtiger_salesorderproductsrel.servicecontractsid =? AND sd.receivedpaymentsid=? AND productcomboid IN(".$row['productid'].") AND multistatus=3 LIMIT 1 ";
                $product=$adb->pquery($sql,array($row['servicecontractid'],$row['receivedpaymentsid']));
            }else{
                $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_salesorderproductsrel.productnumber,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid  WHERE vtiger_salesorderproductsrel.servicecontractsid =? AND productcomboid IN(".$row['productid'].") AND multistatus=3 LIMIT 1 ";
        ...
        if(!empty($row['extraproductid'])){
            if($row['multitype']==1){
                $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_salesorderproductsrel.productnumber,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid LEFT JOIN vtiger_salesorderrayment as sd ON sd.salesorderid=vtiger_salesorderproductsrel.salesorderid WHERE vtiger_salesorderproductsrel.servicecontractsid =? AND sd.receivedpaymentsid=? AND productcomboid IN(".$row['extraproductid'].") AND multistatus=3  ";
                $product=$adb->pquery($sql,array($row['servicecontractid'],$row['receivedpaymentsid']));
            }else{
                $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_salesorderproductsrel.productnumber,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid  WHERE vtiger_salesorderproductsrel.servicecontractsid =? AND productcomboid IN(".$row['extraproductid'].") AND multistatus=3  ";
                $product=$adb->pquery($sql,array($row['servicecontractid']));
            }
            while ($rowsDat=$adb->fetch_array($product)){
                $productnumber=$rowsDat['productnumber']>1?$rowsDat['productnumber']:1;
                if((strpos($row['contract_no'],"XF")!=false || $row['servicecontractstype']!='新增')){
                    $marketingPriceExtra+=$rowsDat['renewalfee']*$rowsDat['agelife']*$productnumber;
                    $costpriceExtra+=$rowsDat['renewalcost']*$rowsDat['agelife']*$productnumber;
        ...
        if(strpos($row['contract_no'],"VRZ")){
            ...
            if($isNoMatch['extracost']>0){
                ...
                if($isMoreYears==1){
                    ...
                    $TempData=$recordModel->calcTSiteRenewANDNewAddMoreYear(array(
                        'contractid'=>$row['servicecontractid'],
                        'costdeduction'=>$costdeduction,
                        'onemarketrenewprice'=>$renewalfee,//续费市场价
                        'onecostrenewprice'=>$realprice,//续费成本价
                        'unit_price'=>$row['unit_price'],//本次回款金额
                        'total'=>$row['total'],//合同总额
                        'marketprice'=>$isNoMatch['marketprice'],//市场总价
                        'extracost'=>$costingdata['extracost'],
                    ...
                    $buysplitcost=$firstCostdeduction=$costdeduction-$realprice;
                    $buyArriveachievement=$TempData['buyArriveachievement'];
                    $buyContractamount=$TempData['buysplitcontractamount'];
                    $buyFirstMarketingPrice=$TempData['buysplitmarketprice'];

                    $renewContractamount=$TempData['renewsplitcontractamount'];
                    $renewMarketingPrice=$TempData['renewsplitmarketprice'];
                    $renewsplitcost=$realprice;
                    $renewArriveachievement=$TempData['renewArriveachievement'];
                    $lastExtracost=$TempData['lastExtracost'];
                ...
                if($isMoreYears==1){
                    ...
                    $TempData=$recordModel->calcTSiteRenewANDNewAddMoreYear(array(
                        ...
                        'unit_price'=>$row['unit_price'],//本次回款金额
                        'total'=>$row['total'],//合同总额
                        'marketprice'=>$row['marketprice'],//市场总价
                        'extracost'=>$costingdata['extracost'],
                        'extra_price'=>$otherdatas['extra_price'],
                        'lastExtracost'=>0,
                        'isTsiteDiscount'=>$isTsiteDiscount,
            ...
            if($isMoreYears==1){
                ...
                $tempArray[]=array_merge($dataArray,array('servicecontractid'=>$row['servicecontractid'],'unit_price'=>$row['unit_price']));
                ...
                updateAchievementallotSatstic($adb,$dataArray);
                $tempArray[]=array_merge($dataArray,array('servicecontractid'=>$row['servicecontractid'],'unit_price'=>$row['unit_price']));;
                $recordModel->setContractAchSetting($tempArray);
            }else{
...
function updateAchievementallotSatstic($adb,$dataArray){
    ...
    if($dataArray['achievementtype']='newadd'){
        $recordModel = Matchreceivements_Record_Model::getCleanInstance("matchreceivements");
        $recordModel->receivePaymentsRanking($dataArray['receivedpaymentownid'],$dataArray['receivedpaymentsid'],$dataArray['achievementmonth'],'add',true);
    }
