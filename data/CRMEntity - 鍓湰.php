	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				...
				} elseif ($uitype == 33 || $uitype ==54 || $uitype ==110) {
...
//Added by Richie
    ...
    function makeWorkflows($modulename,$workflowsid,$salesorderid,$isedit=''){
    	...
    	if(!in_array($modulename, $isallow)||$isedit||$modulename=='Products'){
    		return;
    	}
    	//工作流id防止为空
    	if($workflowsid>0){
    	...
    	//删除已经存在的阶段
    	if($isedit){
    		$sql="select workflowsid from vtiger_salesorderworkflowstages where salesorderid=? and modulename=?";
    		$result=$this->db->pquery($sql,array($salesorderid,$modulename));
    		if($this->db->num_rows($result)){
    			$oldworkflowsid=$this->db->query_result($result,0,'query_result');
    			if($oldworkflowsid!=$workflowsid&&$workflowsisedit){
    	...
    	if($isinsert){
    		...
    		if($countrows){
    			...
    			$sqlsub="INSERT INTO vtiger_salesorderworkflowstages (workflowstagesname,workflowstagesid,sequence,salesorderid,isaction,actiontime,addtime,workflowsid,modulename,smcreatorid,createdtime,productid,departmentid,ishigher,higherid) values (?,?,?,?,?,?, NOW(),?,?,?,NOW(),?,?,?,?)";
    				 ...
    				 if($row['handleaction']=='ProductCheck'){
    					...
    					if(is_array($checkproducts)){
    						foreach ($checkproducts as $productinfo){//$current_user->id为当前人，审核，产品部门的审核变化了。		
    							$this->db->pquery($sqlsub,array(str_replace('审核','',$row['workflowstagesname']).$productinfo['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,$productinfo['productid'],$user->get('current_user_parent_departments'),0,0));
    						}	
    					}		
    					...
    					$this->db->pquery($sqlsub,array('提单人上级审批',$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$reports_to_id));
    				}else{
    					$this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),0,0));
    				}
