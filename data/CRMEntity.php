...
include_once('config.php');//peizhi
...
class CRMEntity {
	...
	var $ownedby;
	var $relatedmodule_list;
	var $relatedmoduleNameSql;
	/**
	 * Detect if we are in bulk save mode, where some features can be turned-off
	 * to improve performance.
	 * 如果我们节省大量的检测模式，其中有些功能可以关闭，用来提高性能。
	 */
	...
	//静态方法，返回实际module的模型类即文件夹下的，module名称.php
	static function getInstance($module) {
		...
		if (!class_exists($modName)) {
			...
			require("modules/$module/$modName.php");
		}
		$focus = new $modName();
		$crm_tab_name=vglobal('all_crmtablename');
		$crm_tab_name[$modName]=$focus->tab_name;
		vglobal('all_crmtablename',$crm_tab_name);
		return $focus;
	}

	function saveentity($module, $fileid = '') {
		global $current_user, $adb,$isallow; //$adb added by raju for mass mailing
        ...
        //2015-12-16 添加请假流程,自动选择工作流id;
        if($module=='Vacate'){
            $sel_vacate_workids = $this->db->pquery("select workflowsid from vtiger_workflows where mountmodule =? limit 1",array($module));
            if($this->db->num_rows($sel_vacate_workids)>0){
                $_REQUEST['workflowsid'] = $this->db->query_result($sel_vacate_workids,0,'workflowsid');
            }
            //请假单在这里插入工作流；
        }
		//young.yang 2014-12-26 插入工作流
		//young.yang 2014-12-27 增加是否包含关联模块
		if($_REQUEST['workflowsid']&&in_array($module, $isallow)){
            if($module=='SalesOrder'){  //工单加入保存草稿，20150422 young + 同时加入只要流程状态为a_才可以修改此处，不然不能修改
                if($_REQUEST['issubmit']&&!empty($_REQUEST['issubmit'])){
                    $servicecontractsid=$_REQUEST['servicecontractsid'];
                    if($servicecontractsid>0){
                        //是否是指定的工作流
                        $workflowsid=$_REQUEST['workflowsid'];
                        if(ServiceContracts_Record_Model::selectWorkfows()==$workflowsid){
                            ...
                            if(ServiceContracts_Record_Model::createIsWorkflows('', $servicecontractsid)){
                                //是否V5系列的产品,满足V5系列的要求
                                if(!ServiceContracts_Record_Model::whetherPayment($servicecontractsid)){
                                    //回款小于成本价
                                    if(!ServiceContracts_Record_Model::receiveDayprice($servicecontractsid,$this->id)){
                ...
                if($_REQUEST['issubmit']&&!empty($_REQUEST['issubmit'])){
                    $this->makeWorkflows($module, $_REQUEST['workflowsid'], $this->id,$insertion_mode);
                }
            }else if($module=='OrderChargeback'){
                if($_REQUEST['issubmit']&&!empty($_REQUEST['issubmit'])){
                    $this->makeWorkflows($module, $_REQUEST['workflowsid'], $this->id,$insertion_mode);
                }
            }else{
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			$listtabid = $this->resolve_query_result_value($result, $i, "listtabid");
			...
			//echo $fieldname;
			//2015-01-24 wangbin 修改判断条件empty
			//echo $fieldname; echo $this->column_fields[$fieldname]; echo "<br>";
            //if($typeofdata_array[1]=='M' && empty($this->column_fields[$fieldname]) && $this->column_fields[$fieldname] !=='0'){
            //2015-3-2 young 加入只有post的数据才做验证的条件
			...
			if($typeofdata_array[1]=='M' && isset($_POST[$fieldname]) && empty($this->column_fields[$fieldname]) && $this->column_fields[$fieldname] !=='0' && $fieldname !="servicecontractsid"){
			    //var_dump($this->column_fields[$fieldname]);
				//新增数据出错delete=0
				if($insertion_mode != 'edit'){
...
//				echo $fieldname;  echo "</br>"; echo $this->column_fields[$fieldname];die;
			...
			if (isset($this->column_fields[$fieldname])) {
				...
				} elseif ($uitype == 33 || $uitype ==54 || $uitype ==110  ||$uitype ==103 || $uitype == 52 || $uitype == 160 || $uitype == 161) {
                ...
                if(2==$listtabid){
                    $tablename = $this->resolve_query_result_value($result, $i, "tablename");
                    $reltablename = $this->resolve_query_result_value($result, $i, "reltablename");
                    $reltablefield = $this->resolve_query_result_value($result, $i, "reltablefield");
                    $reltablecol = $this->resolve_query_result_value($result, $i, "reltablecol");
                    $this->relatedmoduleNameSql[]="UPDATE ".$tablename.",".$reltablename." SET ".$tablename.".".$columname."_name=".$reltablename.".".$reltablecol." WHERE ".$reltablename.".".$reltablefield."=".$tablename.".".$columname." AND ".$tablename.".".$columname.">0 AND ".$tablename.".".$this->tab_name_index[$table_name]."=?";
                }
			}
			else {
				$fldvalue = '';
		...
		if ($insertion_mode == 'edit') {
			...
			if (count($update) > 0) {
			        // cxh  如果是更新 vtiger_usermanger 则做更新记录 查询更新来源
			        if($table_name=='vtiger_usermanger'){
		                    $usersInfo=file_get_contents("php://input");
		                    $remark=$usersInfo;
		                    $remark.=json_encode($_REQUEST);
		                    $remark.=json_encode($_SERVER);
		                    $remark.=json_encode($update);
		                    $remark.=json_encode($update_params);
		                    $adb->pquery(" INSERT INTO `vtiger_achievementallot_nocalculation` (`contract_no`, `marks`, `date`) VALUES (?,? ,?) ",array("USERMANAGERGENGXIN",$remark,date("Y-m-d H:i:s")));
		            }
	 ...
	 * 调用当前的模块时，会对先前传入的id进行判断，如果找不到记录，就会抛出找不到记录的异常。
	 * wangbin 2015年5月26日 星期二
	 */
	...
	function save($module_name, $fileid = '') {
		...
		global $log;
		$log->debug("savemodule name is " . $module_name);
		...
		if(!empty($this->relatedmoduleNameSql)){
		    foreach($this->relatedmoduleNameSql as $relatedmoduleNameSqlVale){
                $adb->pquery($relatedmoduleNameSqlVale,array($this->id));
            }
        }
...
//Added by Richie
	...
	function trash($module, $id) {
		...
		if(!self::isBulkSaveMode()) {
            ...
            // Initialize Event trigger cache
            $em->initTriggerCache(false,false,$module);

            $entityData = VTEntityData::fromEntityId($adb, $id,$module);
	...
	function real_deleted($module,$id){
		//1.查找模块所属表
		$sql0="select * from vtiger_entityname where modulename=?";
		$result=$this->db->pquery($sql0,array($module));
		$tablename=$this->db->query_result($result,0,'tablename');
		$entityidfield=$this->db->query_result($result,0,'entityidfield');

		//$focus = CRMEntity::getInstance($module);
		$field=$this->table_index;
		//$this->db->startTransaction();
		$sql="delete from $tablename where $entityidfield=?";

		if(in_array($tablename.'cf',$this->tab_name)){
			$cfid=$this->tab_name_index[$tablename.'cf'];
			$sql1="delete from $tablename.'cf' where $cfid=?";
			$this->db->pquery($sql1,array($id));
		}
    ...
    /**
     * young.yang 2014-12-26 新增创建模块工作流，
     * 	1.合同，那么需要生成默认产品审批流
     * 	2.报价单或者拜访单，则生成流程即可
    ...
    function makeWorkflows($modulename,$workflowsid,$salesorderid,$isedit=''){
    	...
    	if($modulename=='SalesOrder'){
    		//$sql="select accountid,servicecontractsid from vtiger_salesorder where salesorderid=?";
            $sql="SELECT vtiger_crmentity.smcreatorid,vtiger_crmentity.smownerid,vtiger_salesorder.accountid,vtiger_salesorder.servicecontractsid FROM vtiger_salesorder 
                INNER JOIN vtiger_crmentity ON(vtiger_salesorder.salesorderid=vtiger_crmentity.crmid) where vtiger_salesorder.salesorderid=?";
    		$result=$this->db->pquery($sql,array($salesorderid));
    		if($this->db->num_rows($result)){
    			$servicecontractsid=$this->db->query_result($result,0,'servicecontractsid');
    			$accountid=$this->db->query_result($result,0,'accountid');
                //$smcreatorid=$this->db->query_result($result,0,'smcreatorid');
                $smcreatorid=$this->db->query_result($result,0,'smownerid');
    			if(!empty($servicecontractsid)){
			//多工单模式无法走
    				//$sql="update vtiger_salesorderproductsrel set salesorderid=?,accountid=? where servicecontractsid=?";
    				//$this->db->pquery($sql,array($salesorderid,$accountid,$servicecontractsid));
    			}
                //如果含有客服那么设置客服id
                $sql = "select serviceid,ifnull((select last_name from vtiger_users where id=vtiger_servicecomments.serviceid),'--') as last_name from vtiger_servicecomments where assigntype='accountby' and related_to=? order by servicecommentsid desc limit 1";
                $serviceresult=$this->db->pquery($sql,array($accountid));
                if($this->db->num_rows($serviceresult)){
                    $serviceid=$this->db->query_result($serviceresult,0,'serviceid');
                    $servicename=$this->db->query_result($serviceresult,0,'servicename');
                }
    		}
    	}
    	if($modulename=='ServiceContracts'){
    		$servicecontractsid=$salesorderid;
    	}
        $companyCode=$this->getContractsCompanyCode($modulename,$salesorderid);
    	//echo $modulename;echo $salesorderid.'|';echo $isinsert.'|';echo $workflowsid;die();
    	if($isinsert){
    		...
    		if($countrows){
                ...
                if($modulename!=='Vacate'){
                    // cxh  添加如果是 发票新模块 则不生成最后一个工作流
                    if($modulename=='Newinvoice'){
                        //查询电子发票是否是 增值税电子普通发票
                        $sql="SELECT * FROM vtiger_newinvoice WHERE  invoiceid=? ";
                        $result=$this->db->pquery($sql,array($salesorderid));
                        $result=$this->db->query_result($result,0,'taxtype');
                        if($result=='electronicinvoice'){
                            $isNewInvoice=1;
                        }
                    }
                    while($row=$this->db->fetch_array($wresult)){
                        if($isNewInvoice==1 && $row['workflowstagesflag']=='receive_invoice'){
                            continue;
                        }
                        //第一个节点激活
                        $row['sequence']+=$current_sequence;
                        if($row['handleaction']=='ProductCheck'){
                            ...
                            if(is_array($checkproducts)){
                                foreach ($checkproducts as $productinfo){//$current_user->id为当前人，审核，产品部门的审核变化了。
                                    $this->db->pquery($sqlsub,array(str_replace('审核','',$row['workflowstagesname']).$productinfo['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,$productinfo['productid'],$user->get('current_user_parent_departments'),0,0,$row['workflowstagesflag'],$row['handleaction']));
                                }
                            }
                        }elseif($row['handleaction']=='MyCheck'){//自己审核
                            //$this->db->pquery($sqlsub,array('提单人确认',$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$current_user->id));
                            if($modulename=='SalesOrder'){
                                $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$smcreatorid,$row['workflowstagesflag'],$row['handleaction']));
                            }else{
                                $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$current_user->id,$row['workflowstagesflag'],$row['handleaction']));
                            }
                            ...
                            $this->db->pquery($sqlsub,array('提单人上级审批',$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$reports_to_id,$row['workflowstagesflag'],$row['handleaction']));
                            ...
                            if($if_zhongxiao_rows>0){
                                ...
                                if($serviceid>0){ //如果以及分配客服
                                    ...
                                    $this->db->pquery($sqlsub,array('客服'.$servicename.'审核',0,$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$serviceid,$row['workflowstagesflag'],$row['handleaction']));
                                }else { //如果未分配客服
                                    $this->db->pquery($sqlsub, array('客服经理分配客服', $row['workflowstagesid'], $row['sequence'], $salesorderid, $isaction, $actiontime, $row['workflowsid'], $modulename, $current_user->id, 0, $user->get('current_user_parent_departments'), 0, 0,$row['workflowstagesflag'],$row['handleaction']));
                                }
                            }
                        }elseif($row['handleaction']=='NextCheck'){ //下个节点指定审核人
                            $this->db->pquery($sqlsub,array('指定下个节点审核人',$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),0,0,$row['workflowstagesflag'],$row['handleaction']));
                            ...
                            $sqlsub_platform="INSERT INTO vtiger_salesorderworkflowstages (modulestatus,workflowstagesname,workflowstagesid,sequence,salesorderid,isaction,actiontime,addtime,workflowsid,modulename,smcreatorid,createdtime,productid,departmentid,ishigher,higherid,platformids,workflowstagesflag,handleaction) values ('p_process',?,?,?,?,?,?, NOW(),?,?,?,NOW(),?,?,?,?,?,?,?)";
                            if($topplatform_rows>0){
                                for($i=0;$i<$topplatform_rows;$i++){
                                    ...
                                    $this->db->pquery($sqlsub_platform,array(str_replace('审核','',$row['workflowstagesname']).'-'.$topplatform,$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),0,0,$platformmanIds,$row['workflowstagesflag'],$row['handleaction']));
                                }
                            }
                            ...
                            if(!empty($_SESSION['userdepartmentid']) || !empty($current_user->departmentid)){
                                ...
                                foreach($parentdepartment as $department){
                                    ...
                                    ++$current_sequence;
                                    $this->db->pquery($sqlsub,array('【'.$departAudituid[$department]['departmentname'].'】负责人审核',$row['workflowstagesid'],($row['sequence']+$current_sequence),$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$departAudituid[$department]['peopleid'],$row['workflowstagesflag'],$row['handleaction']));
                                    $isaction=0;
                                }
                            }else{
                                $this->db->pquery($sqlsub,array('【】负责人审核',$row['workflowstagesid'],($row['sequence']+$current_sequence),$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,2110,$row['workflowstagesflag'],$row['handleaction']));
                            }
                            ...
                            if($this->db->num_rows($maincompanyResult) && $maincompanyResult->fields['userid']>0){
                                ...
                                $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$maincompanyResultuserid,$row['workflowstagesflag'],$row['handleaction']));
                            }else{
                                $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),0,0,$row['workflowstagesflag'],$row['handleaction']));
                            }
                        }else{ //默认情况
                            if($row['workflowstagesflag']=='CWSH'||(in_array($modulename,array('ServiceContracts','SupplierContracts','ContractsAgreement','SuppContractsAgreement','RefillApplication','SupplierStatement','CustomerStatement'))&&in_array($row['workflowstagesflag'],array('CREATE_CODE','TREASURER_CODE','TREASURER_TWO','FINANCE_MANAGER')))){//运营经理审核
                                $userId=$this->getDepartmentById($modulename,$salesorderid,$row['workflowstagesname']);
                                if($userId){
                                    $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),1,$userId,$row['workflowstagesflag'],$row['handleaction']));
                                }else{
                                    $this->db->pquery($sqlsub,array($row['workflowstagesname'],$row['workflowstagesid'],$row['sequence'],$salesorderid,$isaction,$actiontime,$row['workflowsid'],$modulename,$current_user->id,0,$user->get('current_user_parent_departments'),0,0,$row['workflowstagesflag'],$row['handleaction']));
                                }
                            }else{
     ...
     * @param $entityI
     */
    public function deleteAfter($module,$entityId)
    {
        global $adb;
        $adb->pquery('UPDATE vtiger_salesorderworkflowstages SET isaction=0 WHERE isaction=1 AND salesorderid=? AND modulename=?',array($entityId,$module));
    }
     ...
     *
     */
    public function getAudituid1($moduleName,$departmentid){
        global $current_user;
        ...
        //$departmentid=empty($departmentid)?$_SESSION['userdepartmentid']:$departmentid;
        ...
        $returnData=array('oneaudituid'=>2110,'towaudituid'=>2110,'audituid3'=>2110,'audituid4'=>2110,'audituid5'=>2110);
        if($this->db->num_rows($result)>0){
            ...
            foreach($departmentids AS $value){
                $result=$this->db->pquery("SELECT oneaudituid,towaudituid,audituid3,audituid4,audituid5,department,auditsettingtype FROM `vtiger_auditsettings` WHERE auditsettingtype =? AND department=?",array($moduleName,$value));
                if($this->db->num_rows($result)){
                    $rData=$this->db->query_result_rowdata($result,0);
                    $returnData['oneaudituid']=$rData['oneaudituid'];
                    $returnData['towaudituid']=$rData['towaudituid'];
                    $returnData['audituid3']=$rData['audituid3'];
                    $returnData['audituid4']=$rData['audituid4'];
                    $returnData['audituid5']=$rData['audituid5'];
                    break;
                }
            }
        }
     ...
     *
     */
    public function getAudituid($moduleName,$departmentid){
        $returnData=$this->getAudituid1($moduleName,$departmentid);
        if($moduleName!='ContractsAuditset' && $returnData['oneaudituid']==2110){
            $returnData=$this->getAudituid1('ContractsAuditset',$departmentid);
        }
        return $returnData;
    }
     ...
     *
     */
    public function setAudituid($moduleName,$departmentid,$salesorderid,$workflowsid){
        $data=$this->getAudituid($moduleName,$departmentid);
        $auditunode=array('oneaudituid'=>'AUDIT_VERIFICATION','towaudituid'=>'TWO_VERIFICATION','audituid3'=>'THREE_VERIFICATION','audituid4'=>'FOUR_VERIFICATION','audituid5'=>'FIVE_VERIFICATION');
        $updateSql="UPDATE vtiger_salesorderworkflowstages,
                ...
                AND vtiger_workflowstages.workflowstagesflag =?
        ...
        foreach($data as $key=>$value){
            if($value>0){
                $this->db->pquery($updateSql,array($value,$salesorderid,$auditunode[$key],$workflowsid));
            }
        }
        //财务总监审核
        /*$updateSql="UPDATE vtiger_salesorderworkflowstages,
                ...
                WHERE
                    vtiger_workflowstages.workflowstagesid = vtiger_salesorderworkflowstages.workflowstagesid
                AND vtiger_salesorderworkflowstages.salesorderid =?
                AND vtiger_workflowstages.workflowstagesflag IN('FINANCE_MANAGER','CREATE_CODE')
                AND vtiger_salesorderworkflowstages.workflowsid = vtiger_workflowstages.workflowsid
                AND vtiger_salesorderworkflowstages.workflowsid =?";
        $this->db->pquery($updateSql,array(16,$salesorderid,$workflowsid));*/

    }
    public function getDepartAudituid($departmentid){
        global $adb;
        $query="SELECT dd.* FROM vtiger_departments dd WHERE FIND_IN_SET(dd.departmentid,REPLACE((SELECT parentdepartment FROM vtiger_departments WHERE departmentid=?),'::',',')) AND dd.depth>0";
     ...
     * 工作流部门审核节点判断
     * @param $record 主记录
     * @param $workflowsid 工作流ID
     * @param $assigned_user_id 负责人
     * @param $AuditName 审核模块的名称
     * @param $modulename 模块名称
     * @param int $Grade 第几级(1第一级,2第二级)
     * @author: steel.liu
     * @Date:xxx
     *
     */
    public function AuditAuditNodeJump($record,$workflowsid,$assigned_user_id,$AuditName,$modulename,$Grade=1){
        global $current_user;
        $user = new Users();
        $datetime=date("Y-m-d H:i:s");
        $current_user_tmp = $user->retrieveCurrentUserInfoFromFile($assigned_user_id);
        $departmentid=empty($current_user_tmp->departmentid)?'H1':$current_user_tmp->departmentid;
        $audituData=$this->getAudituid($AuditName,$departmentid);
        $oneaudituid=$audituData['oneaudituid'];
        $towaudituid=$audituData['towaudituid'];
        $audituid3=$audituData['audituid3'];
        $updateSql="UPDATE vtiger_salesorderworkflowstages SET isaction=?,auditorid=?,auditortime=? WHERE salesorderid =? AND workflowsid =?";
        $activeSql="UPDATE vtiger_salesorderworkflowstages SET isaction=1,actiontime=? WHERE modulename=? AND salesorderworkflowstagesid=?";
        $query='SELECT salesorderworkflowstagesid FROM vtiger_salesorderworkflowstages WHERE isaction=0 AND vtiger_salesorderworkflowstages.salesorderid=? AND sequence=(SELECT s.sequence FROM vtiger_salesorderworkflowstages as s WHERE s.isaction=0 AND s.salesorderid=? ORDER BY s.sequence ASC LIMIT 1)';
        $checkactive='SELECT 1 FROM vtiger_salesorderworkflowstages WHERE isaction=1 AND vtiger_salesorderworkflowstages.salesorderid=? AND workflowsid=?';
        if($Grade==1 && $oneaudituid==$towaudituid && $towaudituid==$audituid3)
        {//当前第一审核人与第二审核人是同一人，则第二个节点关闭直接跳到第三个工作流
            $updateSql.=" AND workflowstagesflag in('TWO_VERIFICATION','THREE_VERIFICATION')";

            $this->db->pquery($updateSql,array(2,$current_user->id,$datetime,$record,$workflowsid));
            $selectData=$this->db->pquery($checkactive,array($record,$workflowsid));
            if($this->db->num_rows($selectData)==0){
                $qResult=$this->db->pquery($query,array($record,$record));
                if($this->db->num_rows($qResult)>0){
                    while($row=$this->db->fetch_array($qResult)){
                        $this->db->pquery($activeSql,array($datetime,$modulename,$row['salesorderworkflowstagesid']));
                    }
                }
            }

        }elseif($Grade==1 && $oneaudituid==$towaudituid){
            $updateSql.=" AND workflowstagesflag='TWO_VERIFICATION'";
            $this->db->pquery($updateSql,array(2,$current_user->id,$datetime,$record,$workflowsid));
            $selectData=$this->db->pquery($checkactive,array($record,$workflowsid));
            if($this->db->num_rows($selectData)==0) {
                $qResult=$this->db->pquery($query,array($record,$record));
        ...
        }elseif($Grade==2 && $towaudituid==$audituid3){
            $updateSql.=" AND workflowstagesflag='THREE_VERIFICATION'";
            $this->db->pquery($updateSql,array(2,$current_user->id,$datetime,$record,$workflowsid));
            $selectData=$this->db->pquery($checkactive,array($record,$workflowsid));
            if($this->db->num_rows($selectData)==0) {
                $qResult=$this->db->pquery($query,array($record,$record));
    ...
    /**
     * cxh 2019 -08 -02
     * 审核结束之后是判断审核流每个节点都已审核 如果都已经审核 更改 vtiger_salesorderworkflowstages 该 salesorderid 的modulestatus改成已完成c_complete
     */
    public function  hasAllAuditorsChecked($request){
        ...
        $list = $this->db->pquery($querySql,array($request['salesorderid'],$request['workflowsid']));
        $updateSql = " UPDATE  vtiger_salesorderworkflowstages SET modulestatus=? WHERE salesorderid = ?  AND workflowsid =? ";
        if($this->db->num_rows($list)>0){
            $this->db->pquery($updateSql,array('p_process',$request['salesorderid'],$request['workflowsid']));
        }else {
            $this->db->pquery($updateSql,array('c_complete',$request['salesorderid'],$request['workflowsid']));
        }
    }
    ...
    public function getContractsCompanyCode($modulename,$salesorderid){
        $modulenameArray=array('SalesOrder',
            'Invoice',
            'ServiceContracts',
            'OrderChargeback',
            'RefillApplication',
        ...
        try{
            global $adb;
            $recordModel=Vtiger_Record_Model::getInstanceById($salesorderid,$modulename,true);
            if(in_array($modulename,array('ServiceContracts','SupplierContracts','Newinvoice','InputInvoice'))){
                return $recordModel->get('companycode');
            }
            $contractsidField=array(
                'SalesOrder'=>'servicecontractsid',
                'Newinvoice'=>'contractid',
                'RefillApplication'=>'servicecontractsid',
                'OrderChargeback'=>'servicecontractsid',
                'ContractsAgreement'=>'servicecontractsid',
                'SuppContractsAgreement'=>'suppliercontractsid',
                'Staypayment'=>'contractid',
                'CustomerStatement'=>'contractid',
                'SupplierStatement'=>'suppliercontractsid'
            );
            $contractsid=$recordModel->get($contractsidField[$modulename]);
            if($contractsid){
...
WHERE
     ...
     * @param unknown $salesorderid
     */
    function makeWorkfilterflows($modulename,$workflowsid,$salesorderid,$sourceid,$ceocheck=0,$isedit=''){
        global $adb,$current_user,$isallow;
        $workflowsisedit=false;//流程是否可以修改
        $isNewInvoice=0;// 是否是发票新生成工作流
        $wfResoult=array();
        ...
        $companyCode=$this->getContractsCompanyCode($modulename,$salesorderid);
        //获取过滤的工作流
		$filterResult =$this->db->pquery("select * from vtiger_filterworkflowstage where modulename=? and workflowsid=? and sourceid=? and deleted=0",array($modulename,$workflowsid,$sourceid));
        $filterworkflowstageids='';
        if($this->db->num_rows($filterResult)){
        	$filterArray=array();
        	$tempdepartments=array();
        	while($row=$this->db->fetch_array($filterResult)){
				$tempdepartmentid=explode(',',$row['departmentid']);
        		foreach($tempdepartmentid as $tempvalue){
					$filterArray[$tempvalue]=$row;
					$tempdepartments[]=$tempvalue;
				}

			}

			foreach($parentdepartment as $value){
				if(in_array($value,$tempdepartments)){
					$filterworkflowstageids=$filterArray[$value];
					break;
				}
			}
        }
        ...
        if($isinsert){
            //@TODO 前置条件，为扩展功能，如何触发
			if($filterworkflowstageids){
                //插入数据第一个节点
                $sql="SELECT * FROM vtiger_workflowstages WHERE workflowsid =? and workflowstagesid in (".$filterworkflowstageids['workflowstageids'].") ORDER BY field(workflowstagesid,".$filterworkflowstageids['workflowstageids'].") ";
			}else{
                $sql="SELECT * FROM vtiger_workflowstages WHERE workflowsid =? ORDER BY sequence ASC";
            }
                ...
                if($modulename!=='Vacate'){
                    ...
                    while($row=$this->db->fetch_array($wresult)){
                        ...
                        if($row['handleaction']=='ProductCheck'){
							...
							if(!empty($filterworkflowstageids)) {
								//$departmentid = $filterworkflowstageids['departmentid'];
								$departmentid = $userInfo['departmentid'];
								$departAudituid = $this->getDepartAudituid($departmentid);
								$currentdepartment = $departAudituid[$departmentid];
								$parentdepartment = $currentdepartment['parentdepartment'];
								$parentdepartment = str_replace('H1::', '', $parentdepartment);
								$parentdepartment = explode('::', $parentdepartment);
								$auditunode = array('DAUDIT_VERIFICATION' => 0, 'DTWO_VERIFICATION' => 1, 'DTHREE_VERIFICATION' => 2, 'DFOUR_VERIFICATION' => 3, 'DFIVE_VERIFICATION' => 4);
								$workflowstagesnum = (!empty($row['workflowstagesflag']) && is_numeric($auditunode[$row['workflowstagesflag']])) ? $auditunode[$row['workflowstagesflag']] : 100;
								if (!empty($parentdepartment[$workflowstagesnum])) {
									$currentdepartempid = $parentdepartment[$workflowstagesnum];
									if ($workflowstagesnum>0) {
										$predepartempid = $parentdepartment[$workflowstagesnum - 1];
										if ($departAudituid[$currentdepartempid]['peopleid'] != $departAudituid[$predepartempid]['peopleid'] && !empty($departAudituid[$currentdepartempid]['peopleid'])) {
											$this->db->pquery($sqlsub, array('【' . $departAudituid[$parentdepartment[$workflowstagesnum]]['departmentname'] . '】负责人审核', $row['workflowstagesid'], ($row['sequence']), $salesorderid, $isaction, $actiontime, $row['workflowsid'], $modulename, $current_user->id, 0, $user->get('current_user_parent_departments'), 1, $departAudituid[$currentdepartempid]['peopleid'], $row['workflowstagesflag'], $row['handleaction']));
										}else{
											continue;
										}
									} else {
