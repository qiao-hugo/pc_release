...
<div class='container-fluid editViewContainer'>
	...
	<input type="hidden" name="t_schoold_id" value="{$SCHOOLID}">
	<input type="hidden" name="t_schoold_name" value="{$SCHOOLNAME}">
								...
								{if $FIELD_MODEL->get('uitype') eq "15"}
									...
									<select class="chzn-select" name="{$FIELDNAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
										...
										{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
									        <option value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if $FIELDNAME eq 'shool_resume_source' && isset($SCHOOLID) &&  $PICKLIST_NAME eq 'school_recruit' }selected{/if}  {if trim(decode_html($FIELD_MODEL->get('fieldvalue'))) eq trim($PICKLIST_NAME)} selected {/if}     {if $FIELDNAME eq 'accountrank' && isset($RANKLIMIT) }{if !$RANKLIMIT[$PICKLIST_NAME]} disabled="true"{/if}{/if}>{$PICKLIST_VALUE}</option>
									    {/foreach}
									</select>
								{else}
