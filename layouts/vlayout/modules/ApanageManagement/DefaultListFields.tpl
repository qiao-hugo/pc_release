{strip}
         ...
         <span class="span4">
			{if $LISTVIEW_LINKS['LISTVIEWBASIC']|@count gt 0}
				{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
					...
					<button id="UpdateUserINFO" class="btn addButton" type="button" <i class="icon-plus icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong></button>
					</span>
                {/foreach}
			{/if}
					{if $ISDEFAULT}
    ...
    <span>
    &nbsp;<input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn" type="button" value="列表自定义"></span>{/if}{/if}
             &nbsp;{*&nbsp;<span><button type="button" class="btn btn-primary applicationUpdateMonth">申请修改业绩月份</button></span>*}
