...
{assign var=ISCANDO value=0}
{assign var=WITHHOLD value=0}
...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['recordid']}' data-recordUrl='index.php?module=ApanageManagement&view=Detail&record={$LISTVIEW_ENTRY['recordid']}&mode=showDetailViewByMode' id="ApanageManagement_listView_row_{$smarty.foreach.listview.index+1}">
            ...
            <td class="listViewEntryValue">
                {if $LISTVIEW_HEADER@last}
                    <div  style="width:90px">
                        <a  href="index.php?module=ApanageManagement&view=Detail&record={$LISTVIEW_ENTRY['recordid']}"><i title="详细信息" class="icon-th-list alignMiddle"></i></a>&nbsp;
						{if $IS_MODULE_EDITABLE}
							<i title="{vtranslate('LBL_DELETE', $MODULE)}" data-id="{$LISTVIEW_ENTRY['recordid']}" class="icon-trash alignMiddle deleteRecordButtont"></i>
						{/if}
					</div>
                {/if}
            </td>
