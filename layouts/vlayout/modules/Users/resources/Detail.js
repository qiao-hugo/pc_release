...
Vtiger_Detail_Js("Users_Detail_Js",{
	...
	savePassword : function(form){
		...
		if(new_password.val() == confirm_password.val()){
			...
			AppConnector.request(params).then(
				function(data) {
					if(data.success){
						...
						if(data.error.code=='JS_PASSWORD_WEAK'){
                            new_password.validationEngine('showPrompt', app.vtranslate(data.error.message) , 'error','bottomLeft',true);
						}else{
                            old_password.validationEngine('showPrompt', app.vtranslate(data.error.message) , 'error','bottomLeft',true);
						}
						return false;
					}
				}
			...
			new_password.validationEngine('showPrompt', app.vtranslate('JS_REENTER_PASSWORDS') , 'error','bottomLeft',true);
			return false;
		}
	},
