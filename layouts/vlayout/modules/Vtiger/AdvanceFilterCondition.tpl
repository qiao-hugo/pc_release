{strip}
<div class="conditionRow span6" style="margin-left:0;padding-left:0">
	<span class="span3">
		<select class="{if empty($NOCHOSEN)}chzn-select{/if} row-fluid"  name="columnname">
			...
			{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
					...
					{/if}
					{if is_array($ALLFIELDS_CURRENTMODEL) && in_array($FIELD_MODEL->get('table'),$ALLFIELDS_CURRENTMODEL)}
					...
					{if $FIELD_MODEL->getFieldDataType() eq 'reference'}
						...
						{if is_array($referenceList) && in_array('Users', $referenceList)}
								...
								{$FIELD_INFO['picklistvalues'] = $ACCESSIBLE_USERS}
		...
		<span class="span6 fieldUiHolder">
		<input  class="row-fluid" name="{if $SELECTED_FIELD_MODEL}{$SELECTED_FIELD_MODEL->get('name')}{/if}" data-value="value" type="text" value="{$CONDITION_INFO['value']|escape}" />
		</span>
		<span class="hide">
		{if empty($CONDITION)}
