...
jQuery.Class("Vtiger_Detail_Js",{
	...
	saveComment : function(e) {
        ...
        // 跟进阶段
        var modcommentstage = closestCommentBlock.find('.modcommentstage');
        var modcommentstageValue = modcommentstage.val();
        ...
        if(modcommenttypeValue == ""){
            errorMsg = app.vtranslate('此处不可空白');
            modcommenttype.validationEngine('showPrompt', errorMsg , 'error','bottomLeft',true);
            aDeferred.reject();
            return aDeferred.promise();
        }
        if(modcommentcontactsValue == ""){
            errorMsg = app.vtranslate('此处不可空白');
            modcommentcontacts.validationEngine('showPrompt', errorMsg , 'error','bottomLeft',true);
            aDeferred.reject();
            return aDeferred.promise();
        }
        if(modcommentmodeValue == ""){
            errorMsg = app.vtranslate('此处不可空白');
            modcommentmode.validationEngine('showPrompt', errorMsg , 'error','bottomLeft',true);
            aDeferred.reject();
            return aDeferred.promise();
        }
		...
		var postData =
			...
			'modcommentpurpose' : modcommentpurposeValue,
                        'modcommentstage' : modcommentstageValue,
...
//		});
    ...
    deleteuploadFile:function(){
        $('form').on('mouseover','.deletefile',function(){
            ...
            var module=$('#module').val();
            var url='index.php?module='+module+'&action=DeleteFile&id='+delclassid+'&record='+$('#recordId').val();
            AppConnector.request(url).then(
                function(data){
                    if(data['success']) {
...
//			app.registerEventForTextAreaFields(jQuery('.commentcontent',commentInfoBlock));
    ...
    changeAuditor:function(){
        $('.details').on("click",'.auditorid',function() {
            var salesorderworkflowstagesid = $(this).data("salesorderworkflowstagesid");
            var workflowstagesname = $(this).data("workflowstagesname");
            var params_r = [];
            params_r['action'] = 'ChangeAjax';
            params_r['module'] = 'Accounts';
                ...
                function(data) {
                    if(data.result.success){
                        str = '<div id="myModal" class="modal" style="">\n' +
                            ...
                            '\t\t\t\t<h4 class="modal-title" style="height:55px;">请选择<'+workflowstagesname+'>节点审核人</h4>\n' +
                            '\t\t\t\t<div style="margin-top: 20px;height: 136px;margin-left: 175px;" id="supervisor">\n';
                        str +='<input type="hidden" name="salesorderworkflowstagesid" value="'+salesorderworkflowstagesid+'">';
                        var datas = data.result.data;
                        str += '<span style="font-size: 14px;"></span>\n' +
                            '                        <select id="userid" class="chzn-select referenceModulesList streched" name="userid">\n';
                        ...
                        str += '\t\t\t\t</div>\n' +
                            ...
                            '\t\t\t\t<button class="btn btn-success" id="updateAuditorId" type="submit">确定</button>\n' +
                            '\t\t\t</div>\n' +
                            '\t\t</div>\n' +
                            '\t</div>\n' +
                            '</div>';
                        ...
                        $("#updateAuditorId").on("click",function () {
                            var salesorderworkflowstagesid = $("input[name='salesorderworkflowstagesid']").val();
                            var userid = $("select[name='userid']").val();
                            var params_r = [];
                            params_r['action'] = 'ChangeAjax';
                            params_r['module'] = 'SalesorderWorkflowStages';
                            params_r['mode'] = 'updateAuditorId';
                            params_r['salesorderworkflowstagesid'] = salesorderworkflowstagesid;
                            params_r['auditorid'] = userid;
                            AppConnector.request(params_r).then(
                                function(data) {
                                    location.reload();
                                }
