...
jQuery.Class("Vtiger_Popup_Js",{
	...
	show : function(urlOrParams, cb, windowName, eventName, onLoadCb){
		...
		var popupWinRef =  window.open(url, windowName ,'width=960,height=650,resizable=0,scrollbars=1,location=no,status=no,menubar=no');
		if(popupWinRef.location.href!='about:blank'){
			//popupWinRef.close();
		}
		popupWinRef.focus();
	...
	updatePagination : function(){
        ...
        var pageLimit = jQuery('#pageLimit').text();
        var pageNumber = jQuery('#pageNumber').val();
		if(pages == 1){
		...
		if((nextPageExist != "") && (listViewEntriesCount==pageNumber)){
