...
if (typeof(ImportJs) == 'undefined') {
    ...
    ImportJs = {
		...
		validateFilePath: function() {
			...
			if(!ImportJs.uploadFilter("import_file", "xls|xlsx|csv|vcf")) {
				return false;
			}
		...
		validateMergeCriteria: function() {
			...
			if($mergeChecked) {
				...
				var errorMessage = app.vtranslate('JS_IMPORT_CHECKBOX_CHECKED');
				var params = {
					text: errorMessage,
					'type': 'error'
				};
		...
		sanitizeAndSubmit: function() {
			...
			if(!ImportJs.validateCustomMap()) return false; //字段映射另存为;
			return true;
		},

		sanitizeFieldMapping: function() {
			var fieldsList = jQuery('.fieldIdentifier');//每一行匹配的数据;
			var mappedFields = {};
			var errorMessage;
			var params = {};
			...
			for(var i=0; i<fieldsList.length; ++i) {
				...
				var selectedFieldName = selectedFieldElement.val();//crm字段
				var selectedFieldDefaultValueElement = jQuery('#'+selectedFieldName+'_defaultvalue', fieldElement);//默认值
				var defaultValue = '';
