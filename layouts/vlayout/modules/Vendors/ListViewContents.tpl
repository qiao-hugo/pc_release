...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue {if $LISTVIEW_HEADER['columnname'] eq 'vendorstate'}c_vendorstate{/if} {if $LISTVIEW_HEADER['columnname'] eq 'allowtransaction'}c_allowtransaction{/if}"  nowrap>
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
                        ...
                        {if $IS_MODULE_LISTBTNEDIT AND $LISTVIEW_ENTRY['modulestatus'] eq 'c_complete'}
               			<a  href="javascript:void(0)" data-status="{$LISTVIEW_ENTRY['vendorstate']}" class="setVendorstate" ><i title="供应商状态变更" class="icon-move alignMiddle"></i></a>&nbsp;
               			{/if}
               			{*<a  href="javascript:void(0)" data-status="{$LISTVIEW_ENTRY['allowtransaction']}" class="setAllowtransaction" ><i title="是否允许交易" class="icon-share alignMiddle"></i></a>*}&nbsp;
	...
	var vendorsTateData = {
		{foreach item=VENDORSTATE from=$VENDORSTATE_DATA name=listview} {$VENDORSTATE} : '{vtranslate($VENDORSTATE,$MODULE)}',{/foreach}
	};
	</script>
