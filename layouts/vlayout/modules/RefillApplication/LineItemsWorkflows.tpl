                ...
                <div style="padding:5px;">
                    ...
                    <ul class="nav nav-pills">
                        {foreach key=index item=vals from=$STAGES}
                            {assign var=tempvalsid value=$vals|current}
                            {if $vals@first}
                                {assign var=tempworkflowsid value=$tempvalsid['workflowsid']}
                            {elseif $tempworkflowsid eq $tempvalsid['workflowsid']}
                                <li style="float: none;vertical-align: middle;display: inline-block;">
                                    <i class="icon-arrow-right" style=""></i></li>
                            {/if}
                            {if $tempworkflowsid neq $tempvalsid['workflowsid']}
                            {assign var=tempworkflowsid value=$tempvalsid['workflowsid']}
                            </ul>
                                <hr>
                            <ul class="nav nav-pills">
                            ...
                            <li style="float: none;vertical-align: middle;display: inline-block;">
                                {foreach from=$vals item=val}
                                    <span class="label {if $val['isaction'] eq 2} label-inverse {elseif $val['check'] eq 1} label-success {elseif $val["isaction"] eq 1} label-info{/if} "
                                          title="{$val['actiontime']}">{$val["workflowstagesname"]}323232</span>
    ...
    {if $ISROLE and $STAGERECORDID}
        ...
        <table class="table table-bordered mergeTables detailview-table ">
            ...
            <tr>
                ...
                <th class="detailViewBlockHeader" >平台负责人</th>
            ...
            {foreach key=index item=value from=$WORKFLOWSSTAGELIST name=listview}
                {if $value@first}{assign var=tempworkflowsid value=$value['workflowsid']}{/if}
                <tr{if $tempworkflowsid neq $value['workflowsid']}{assign var=tempworkflowsid value=$value['workflowsid']} style="background-color:#e9e9e9"{/if}>
					...
					<span class="n_hauto" data-trigger="hover" data-content="{$value["platformids"]}">{$value["platformids"]}{if empty($value["platformids"])}--{/if}</span></a></td>*}
					<td ><a><i class="icon-hand-right pull-left hide" style="margin-top:4px">&nbsp;</i>
...
<br>
        ...
        <table class="table table-bordered equalSplit detailview-table ">
            <thead>
            {if $ISROLE and $STAGERECORDID}
           	<tr class="hide realremarkbutton">
			...
			<tr>
                ...
                <th class="detailViewBlockHeader" nowrap>备注原因</th>
                <th class="detailViewBlockHeader" nowrap>创建日期</th>
                <th class="detailViewBlockHeader" nowrap>修改时间</th>
                <th class="detailViewBlockHeader" nowrap>
                {if $ISROLE and $STAGERECORDID}<div class="pull-right"><button type="button" onclick="$('.realremarkbutton').show();$('#remarkvalue').focus();" class="btn btn-info">添加备注</button>&nbsp;</div>{/if}
                </th>
            </tr>
            </thead>
            {foreach key=index item=value from=$REMARKLIST}
                <tr>
                    ...
                    {*<td nowrap>{$value["rejectname"]}</td>*}
                    <td>{$value["reject"]}</td>
                    <td nowrap>{$value["rejecttime"]}</td>
                    <td nowrap>{$value["modifytime"]}</td>
                    <td id="editremark" nowrap>
                    	{*{if $USER eq $value["rejectid"]}
