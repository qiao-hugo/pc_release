...
{/if}
	...
	{assign var="COINRETURNFIELDSNO" value=array('accountid')}
	{assign var="INCREASE" value=array('mservicecontractsid')}
<input name="m{$FIELD_MODEL->getFieldName()}[{$DATANUM}]" type="hidden" othername="{$FIELD_MODEL->getFieldName()}" data-num="{$DATANUM}" value="{$FIELD_MODEL->get('fieldvalue')}" data-multiple="{$FIELD_MODEL->get('ismultiple')}" class="sourceField" data-displayvalue='{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}' data-fieldinfo='{$FIELD_INFO}' />
{assign var="displayId" value=$FIELD_MODEL->get('fieldvalue')}
<div class="row-fluid input-prepend input-append">
	...
	{if (in_array($FIELD_NAME,$ACCOUNTSFIELDSNO) && $RECHARGESOURCE eq 'Accounts')
...
<span class="add-on clearReferenceSelection cursorPointer">
	...
	{/if}
<input id="{$FIELD_NAME}_display" name="m{$FIELD_MODEL->getFieldName()}[display{$DATANUM}]_display" type="text" class="{if $smarty.request.view eq 'Edit'} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
