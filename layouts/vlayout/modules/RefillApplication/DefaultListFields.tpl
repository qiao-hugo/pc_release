{strip}
    ...
    &nbsp;<input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn"
             ...
             <span> <button type="button" class="btn btn-primary printall">打印</button></span>
             {if  $USERID neq 10710}
                 <span> <button type="button" class="btn btn-primary exportdata">导出</button></span>
             {/if}

             <span> <button type="button" class="btn btn-primary matchreceivements">匹配回款</button></span>
             <span> <button type="button" class="btn btn-primary contractChangeApplication" title="充值/外采单变更服务合同或者采够合同申请">合同变更申请</button></span>
             <span id="totalAmount" style="margin-left:30px">合计应收款总额：0</span>
        </span>
        {if $ISPAGE}
