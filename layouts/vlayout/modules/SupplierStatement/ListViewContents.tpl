                        ...
                        <tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}&realoperate={setoperate($LISTVIEW_ENTRY['id'],MODULE)}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
                            ...
                            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                                <td class="listViewEntryValue"  nowrap>
                                    ...
                                    {elseif $LISTVIEW_HEADER['columnname'] eq 'suppliercontractsid'}
                                        <a class="btn-link" href='index.php?module=SupplierContracts&view=Detail&record={$LISTVIEW_ENTRY['suppliercontractsid_reference']}' target="_block">{$LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']]}</a>
                                    {elseif $LISTVIEW_HEADER['columnname'] eq 'vendorid'}
                                        ...
                                        {else}
                                            <a class="btn-link" href='index.php?module=Vendors&view=Detail&record={$LISTVIEW_ENTRY['vendorid_reference']}' target="_block">{$LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']]}</a>
                                        {/if}
                                    {else}
