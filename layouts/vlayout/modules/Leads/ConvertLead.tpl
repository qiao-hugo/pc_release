...
-->*}
{strip}
{if $CURRENT_CHECK_ACCOUNTNAME['flag'] eq 1}
...
<div class="bootbox-body">{$CURRENT_CHECK_ACCOUNTNAME['msg']}请到客户中查看</div>
</div><div class="modal-footer">  <a href="/index.php?module=Accounts&view=Detail&record={$CURRENT_CHECK_ACCOUNTNAME['accountid']}" class="btn">确定</a></div></div></div></div>
{else}
						...
						<div id="{$MODULE_NAME}_FieldInfo" class="{$MODULE_NAME}_FieldInfo accordion-body collapse fieldInfo {if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts"} in {elseif !$CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Contacts"} in {/if}">
							<table class="table table-bordered moduleBlock">
								...
								<tr>
									...
									<td class="fieldValue">
                                        {if $FIELD_MODEL->get('name') eq 'accountcategory'}
                                                <span class="redColor">正常</span>
                                        {elseif $FIELD_MODEL->get('name') eq 'accountrank'}
                                                <span class="redColor">机会客户</span>
                                        {else}
...
{/if}
	<script>
        var aaaaa="<option value='' >请选择</option>{foreach key=DEPARTMENTID item=BELONGDEPARTMENT from=$BELONGDEPARTMENTS}<option {if $DEPARTMENTID==$LEADBELONGSYSTEM}selected {/if}value='{$DEPARTMENTID}'>{$BELONGDEPARTMENT}</option>{/foreach}";
	</script>
{/strip}