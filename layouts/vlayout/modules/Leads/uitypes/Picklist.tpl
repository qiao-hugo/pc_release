...
{assign var="FIELDNAME" value={$FIELD_MODEL->getFieldName()}}
    {if $FIELDNAME eq 'leadsource'}
        <select class="chzn-select" name="{$FIELDNAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
            ...
            {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
                {if !in_array($PICKLIST_NAME ,array('公司询盘','SCRM'))}{continue}{/if}
    ...
    {else}
        <select class="chzn-select" {if  $RECORD&&in_array($FIELD_MODEL->getFieldName(),$INPUTREADONLY)}readonly="readonly" {/if} name="{$FIELDNAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
        {if $FIELDNAME neq 'assignerstatus'}
            {if $FIELD_MODEL->isEmptyPicklistOptionAllowed()}<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>{/if}{/if}
            {foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
                {if $PICKLIST_NAME eq 'c_complete' or $PICKLIST_NAME eq 'c_transformation' or $PICKLIST_NAME eq 'c_cancelled' or $PICKLIST_NAME eq 'c_Related' or $PICKLIST_NAME eq 'c_Forced_Related'}{continue}{/if}
                <option value="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if trim(decode_html($FIELD_MODEL->get('fieldvalue'))) eq trim($PICKLIST_NAME)} selected {/if}     {if $FIELDNAME eq 'accountrank' && isset($RANKLIMIT) }{if !$RANKLIMIT[$PICKLIST_NAME]} disabled="true"{/if}{/if}>{$PICKLIST_VALUE}</option>
            {/foreach}
        </select>
    {/if}
