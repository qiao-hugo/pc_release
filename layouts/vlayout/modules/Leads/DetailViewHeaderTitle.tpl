            ...
            <h4 class="recordLabel pushDown" title="{$RECORD->getDisplayValue('salutationtype')}&nbsp;{$RECORD->getName()}"> &nbsp;
                ...
                {foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}
                    ...
                    {if $FIELD_MODEL->getPermissions()}
                        <span class="{$NAME_FIELD}">{$RECORD->get($NAME_FIELD)}
                    {if $COUNTER eq 0 && ($RECORD->get($NAME_FIELD))}&nbsp;</span> {if $CLUEFOLLOWSTATUS}<span style="padding: 3px;font-size: 8px;color:green;border: 1px solid green;border-radius: 3px;" id="cluefollowstatus">{$CLUEFOLLOWSTATUS}</span>{/if}{assign var=COUNTER value=$COUNTER+1}{/if}
                {/if}
            {/foreach}
            </h4>
        </span>
