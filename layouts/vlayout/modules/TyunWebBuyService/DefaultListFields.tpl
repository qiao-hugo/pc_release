{strip}
    ...
    &nbsp;<input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn"
             ...
             {if $FLAGEXPORTDATA}<span> <button type="button" class="btn btn-primary exportdata">导出(会计)</button></span>{/if}
             <!-- <span> <button type="button" class="btn btn-primary exportdatayun">导出(运)</button></span>-->
             {if $IS_FILED eq true}
              &nbsp;&nbsp;<span><button type="button" class="btn btn-primary reconciliationResult">对账</button></span>
	      {/if}
             {if $IS_RECONCLILITION eq true }
             &nbsp;&nbsp;<span><button type="button" class="btn btn-primary startToFiled">保存归档</button></span>
	     {/if}
        </span>
        {if $ISPAGE}
