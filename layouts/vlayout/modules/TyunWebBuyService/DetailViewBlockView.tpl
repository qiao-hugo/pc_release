 ...
 * All Rights Reserved.
 * $IS_FILED 指的是 是否是归档数据详情。
*<!--去除双击编辑-->
 ********************************************************************************/
-->*}
{strip}
	{if $IS_FILED eq 'filed'}
		{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
			...
			<table class="table table-bordered equalSplit detailview-table">
				...
				<tr>
					...
					{assign var=COUNTER value=$COUNTER+1}
					{/if}
					{if $FIELD_MODEL->get('label') eq 'isitfiled' || $FIELD_MODEL->get('label') eq 'reconciliationresult'}
					{else}
						<td class="fieldLabel {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldLabel_{$FIELD_MODEL->getName()}">
							<label class="muted pull-right marginRight10px">
								{*字段名称*}{vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}
						...
						<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						 <span class="value" data-field-type="{$FIELD_MODEL->getFieldDataType()}">
							 {* 如果不相等则显示归档数据内容 因为这里是以 归档前的数据字段显示来显示数据的 所以只要核对归档数据和归档前的表数据核对不一样显示归档数据就ok 了 （这样就不用了考虑归档的数据字段显示否是和归档前字段显示是否一致了） *}
							  {if $FILEDDATA[$FIELD_MODEL->get('label')] neq $FIELD_MODEL->get('fieldvalue')}
								  {vtranslate($FILEDDATA[$FIELD_MODEL->get('label')],$MODULE_NAME)}
							  {else}
							  {*值*}{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getDetailViewTemplateName(),$MODULE_NAME) FIELD_MODEL=$FIELD_MODEL USER_MODEL=$USER_MODEL MODULE=$MODULE_NAME RECORD=$RECORD}{if $FIELD_MODEL->name eq 'probability'}%{/if}
							  {/if}
						 </span>
						</td>
					{/if}
    ...
    {else}
		...
		{/if}
		{include file="LineItemsDetail.tpl"|@vtemplate_path:'ContractActivaCode'}
		{if $SERVICECONTRACTID>0}
			<div class="widgetContainer_servicecontract" data-url="module=ServiceContracts&view=Detail&mode=getProducts&amp;record={$SERVICECONTRACTID}" data-name="Workflows"><div class="widget_contents"></div></div>
		{/if}
	{/if}
