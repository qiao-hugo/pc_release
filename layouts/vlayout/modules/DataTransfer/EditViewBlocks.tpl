...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{/if}
			<span class="pull-right">
				<button class="btn btn-success" type="submit"><strong>{vtranslate('LBL_TRANSFER', $MODULE)}</strong></button>
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
								{if $FIELD_MODEL->get('name') eq 'transferid'}
									{$USER_MODEL->get('last_name')}
								{else}
            ...
            <tr>
                <td class="fieldLabel medium"><label class="muted pull-right marginRight10px">客户转移</label></td><td  colspan="3">可选择被转移人下的正常客户，临时区客户进行转移，转移后所选客户对应的合同、销售机会、报价单同步转移</td>
            </tr>
			</table>
			<br>
		{/foreach}
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewHeaders success">
			<th colspan="3">客户列表</th>
		</tr>
		</thead>
		<tr class="listViewHeaders success" id="tdheader">
			<td nowrap="" style="width: 15%">
				<div class="noclick" style="width: 100%;height:100%;">
					<button type="button" class="btn btn-success over_checkedall">全选</button>
					<button type="button" class="btn btn-inverse over_checkedinverse">反选</button>
				</div>
			</td>
			<td nowrap=""  class="listViewEntries">
				客户名称
			</td>
			<td nowrap=""  class="listViewEntries">
				客户等级
