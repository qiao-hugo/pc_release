...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table">
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			{if !$FIELD_MODEL->isViewableInDetailView() || $FIELD_MODEL->get('name') eq 'idaccount' || $FIELD_MODEL->get('name') eq 'accountplatform'}
				 {continue}
			{/if}
	...
	<table class="table table-bordered equalSplit detailview-table">
		...
		<tr>
			...
			<th class="blockHeader">
				...
				<input type="button" id="batchSave" value="批量保存" class="btn btn-success" title="批量保存" style="margin-left:30px; cursor: pointer;color: white !important">
			</th>
		</tr>
		</thead>
		...
		{foreach  key=key item=FIELD from=$DETAIL_INFO_LIST }
			<tr  data-id="{$FIELD['accountplatform_detail_id']}" >
				<td class="fieldLabel {$WIDTHTYPE}" >
				...
				<td class="fieldValue {$WIDTHTYPE}"  >
					...
					<input id="idaccount" style="display: none;" type="text" onkeyup="this.value=this.value.replace(/\s+/g,'')" class="input-large" data-validation-engine="validate[ required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="idaccount[]"  value="{$FIELD['idaccount']}" >
				</td>
				<td class="fieldLabel {$WIDTHTYPE}" >
					<span class="value" data-field-type="string" id="textaccountplatform">{$FIELD['accountplatform']}</span>
					<input id="accountplatform" style="display: none;" type="text" class="input-large" data-validation-engine="validate[ funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="accountplatform[]" value="{$FIELD['accountplatform']}" >
				</td>
				<td class="fieldValue {$WIDTHTYPE}" >
	...
	<literal>
		<script>
            var appendAccountDetail ='<tr>' +
                ...
                '<input id="accountplatform" type="text" class="input-large" data-validation-engine="validate[ funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="accountplatform[]"  placeholder="请输入账户名称" value="" >'+
                '</td>' +
                '<td class="fieldValue medium">' +
