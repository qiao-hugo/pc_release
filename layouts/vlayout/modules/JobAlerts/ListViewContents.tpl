...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}&List=List' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                {if $LISTVIEW_HEADER['columnname'] eq 'ownerid'}
                    <td class="listViewEntryValue"  nowrap>
                    <span class="{if $LISTVIEW_ENTRY['state'] eq 0 } label label-a_normal {else}label label-c_complete{/if}"> {uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE)}</span>
                    </td>
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
                        <a  href="index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}&List=List"><i title="详细信息" class="icon-th-list alignMiddle"></i></a>&nbsp;
