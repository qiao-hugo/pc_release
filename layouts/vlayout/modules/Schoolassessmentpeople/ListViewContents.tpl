	...
	<table class="table listViewEntriesTable">
			...
			<tr class="listViewHeaders">
                <th nowrap>
                    <div  class="noclick" style="width: 100%;height:100%;">
                        ...
                        <button type="button" class="btn btn-primary stampall">考核通过</button>
                    </div>
                </th>
				{if $LISTVIEW_FIELDS}
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            <td style="text-align: left;">
                {if $LISTVIEW_ENTRY['assessmentresult'] neq 'assessmentresult_yes'}
                <div class="deletedflag"><label style="height:100%;display: inline-block;">
                    ...
                    <button type="button" data-id="{$LISTVIEW_ENTRY['id']}" style="display:inline" class="btn btn stamp">考核通过</button>
                </div>
                {/if}
            </td>
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue {if $LISTVIEW_HEADER['columnname'] eq 'assessmentresult'}assessmentresult_value{/if}
                {if $LISTVIEW_HEADER['columnname'] eq 'assessmentdate'}assessmentdate_value{/if}
                 " nowrap>
                    ...
                    {elseif $LISTVIEW_HEADER['columnname'] eq 'p_assessownerid'}
                        <a class="btn-link" href='index.php?module=Schoolassessment&view=Detail&record={$LISTVIEW_ENTRY['assessmentid']}' target="_blank">{vtranslate( $LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']],$MODULE)}</a>
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
                        ...
                        {if $IS_CHECK}
                        <a class="set_assessmentresult" data-status="{$LISTVIEW_ENTRY['assessmentresult']}"><i title="考核通过" class="icon-share alignMiddle"></i></a>&nbsp;
                        {/if}
