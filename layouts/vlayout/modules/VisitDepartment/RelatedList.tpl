    ...
    <div class="bottomscroll-div">
        ...
        <table class="table table-bordered listViewEntriesTable">
            ...
            {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
                <tr class="listViewEntries1  opendialog" data-id='{$RELATED_RECORD->getId()}' data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'>
                    {foreach item=HEADER_FIELD key=FIELDNAME from=$RELATED_HEADERS}
                        {assign var=RELATED_HEADERNAME value=$FIELDNAME}
							<td class="{$WIDTHTYPE}"  nowrap>
                                {if $RELATED_HEADERNAME eq 'accountid'}
                                    {$RELATED_RECORD->get('accountname')}
                                {else}
					...
					<td>
                        <div class="pull-right actions">
                            <span class="actionImages">
                                ...
                                <i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" data-id="{$RELATED_RECORD->getId()}" class="icon-th-list alignMiddle"></i>
							</span>
                        </div>
                    </td>
                </tr>
