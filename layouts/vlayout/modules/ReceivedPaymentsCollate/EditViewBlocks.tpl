...
<div class='container-fluid editViewContainer'>
    <form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php"
        ...
        <input type="hidden" name="paymentnum" value=""/>
        <input type="hidden" name="isshow" value=""/>
        <input type="hidden" name="receivedpaymentsid" value="{$RECEIVEDPAYMENTSID}"/>
        <div class="contentHeader row-fluid">
			...
			<button class="btn btn-success" id="saveData" type="submit"><strong>{vtranslate('LBL_SAVE', $MODULE)}</strong></button>
			<a class="cancelLink" type="reset"
               onclick="javascript:window.history.back();">{vtranslate('LBL_CANCEL', $MODULE)}</a>
		</span>
        </div>
        {assign var="COLLATEFIELDS" value=['paymentchannel', 'paymentcode', 'owncompany', 'reality_date']}
        {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
            {if $BLOCK_FIELDS|@count lte 0}{continue}{/if}
            {if $BLOCK_LABEL eq 'LBL_ACCOUNT_RECEIVABLE'}{continue}{/if}
            <table class="table table-bordered blockContainer showInlineTable">
                <tr>
                    <th class="blockHeader" colspan="4">出纳</th>
                </tr>
                <tr>
                        ...
                        {if $isReferenceField neq "reference"}<label class="muted pull-right marginRight10px">{/if}
                            ...
                            {if $isReferenceField eq "reference"}
								...
								<select id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}_dropDown"
                                        class="chzn-select referenceModulesList streched" style="width:140px;">
									<optgroup>
                    ...
                    {if $FIELD_MODEL->get('uitype') neq "83"}
                        <td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                            {if $FIELD_MODEL->get('editread') || ($FIELD_MODEL->get('readonly') eq '0' and !empty($RECORD_ID))}
                                ...
                                <input disabled="disabled" name="{$FIELD_MODEL->get('column')}" type="hidden"
