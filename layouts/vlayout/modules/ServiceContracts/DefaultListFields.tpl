{strip}
    <div class="row" style="padding: 10px 20px 0 20px;">
        ...
        &nbsp;<button id="exportContract" class="btn btn-info"><strong>导出</strong></button>&nbsp;

        {if $MODULE_MODEL->exportGrouprt('ServiceContracts','COLLATE')}
        <button id="collateContract" class="btn btn-info"><strong>核对</strong></button>&nbsp;
        {/if}
        {if $MODULE_MODEL->exportGrouprt('ServiceContracts','closedContracts')}
        <button id="closeContract" class="btn btn-info"><strong>关停合同</strong></button>&nbsp;
        {/if}
    <span>
    &nbsp;<input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn diy"
