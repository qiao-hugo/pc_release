...
{assign var="FIEDLONLY" value=['stageshow']}
{assign var="FIELD_NAME" value=$FIELD_MODEL->get('name')}
<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}{$INITNUM}" type="text"
	   class="input-large {if $FIELD_MODEL->isNameField()}nameField{/if}" 
	   data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" 
	   name="m{$FIELD_MODEL->getFieldName()}[]"
	   value="{$FIELD_MODEL->get('fieldvalue')}"
		{if $FIELD_MODEL->get('name') eq 'collectiondescription'}style="width:95%;"{/if}
		{if $FIELD_MODEL->get('prompt') neq ''}
	   placeholder="{$FIELD_MODEL->get('prompt')}"{/if}
