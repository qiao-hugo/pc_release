...
{assign var="NOCHZN" value=['eleccontracttplid']}
 {if $FIELDNAME eq 'contract_type'}
    ...
    {/if}
 {else}
     {if $FIELDNAME neq 'eleccontracttplid' && $FIELDNAME neq 'elereceiver'}
     ...
     {elseif $FIELDNAME eq 'eleccontracttplid'}
         <select class="chzn-select" name="{$FIELDNAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
              {if $RECORD_ID>0}
                 <option value="{$FIELD_MODEL->get('fieldvalue')}" selected>{$RECORD_STRUCTURE_MODEL->getRecord()->get('eleccontracttpl')}</option>
             {/if}
         </select>
     {elseif $FIELDNAME eq 'elereceiver'}
         <select class="chzn-select" name="{$FIELDNAME}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
             {if $RECORD_ID>0}
                 <option value="{$FIELD_MODEL->get('fieldvalue')}" selected>{$FIELD_MODEL->get('fieldvalue')}</option>
             {/if}
         </select>
     {/if}
