...
<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if $smarty.request.view eq 'Edit'} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
 ...
 data-fieldinfo='{$FIELD_INFO}' style="{if $FIELD_MODEL->getFieldName() eq 'sc_related_to'}margin-bottom:3px{/if}"  placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"
...
{/if}
-->
	{if $FIELD_MODEL->getFieldName() eq 'sc_related_to' }
		<input id="needZizhi" name="needZizhi" type="hidden" value="no">
		<div class="fileUploadContainerZizhi" style="display: none" xmlns="http://www.w3.org/1999/html">
			{assign var=FIELD_VALUE value=explode('*|*',$FIELD_MODEL->get('fieldvalue'))}
			<div class="upload {$FIELD_VALUE[0]}">
				<div  id="fileUploadRemand" style="display:inline-block;height:30px;overflow: hidden;vertical-align: middle;"  title="文件名请勿包含空格"><div style="margin-top:-2px;">文件名请勿</div><div style="margin-top:-5px;">包含空格</div></div>
				<input type="button" id="uploadzizhiButton" value="上传资质"  title="文件名请勿包含空格" />
				<div style="display:inline-block" id="fileallzizhi">
					{if !empty($FIELD_VALUE[0])}
						{foreach item=NEWFILD from=$FIELD_VALUE}
							...
							<input class="ke-input-text file{$NFIELD_VALUE[1]}" type="hidden" name="zizhifile[{$NFIELD_VALUE[1]}]" data-id="{$NFIELD_VALUE[1]}" id="zizhifile" value="{$NFIELD_VALUE[0]}" readonly="readonly" />
							<input class="file{$NFIELD_VALUE[1]}" type="hidden" name="attachmentsid[{$NFIELD_VALUE[1]}]" value="{$NFIELD_VALUE[1]}">
						{/foreach}
					{else}
						<input class="ke-input-text zizhifiledelete" type="hidden" name="zizhifile" id="zizhifile" value="" readonly="readonly" />
						<input class="zizhifiledelete" type="hidden" name="attachmentsid" value="">
					{/if}
				</div>
			</div>
		</div>
