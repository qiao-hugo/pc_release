{strip}
    <table class="table table-bordered equalSplit detailview-table"><thead>
        <th colspan="2">{if $CSELECTED eq 1}合同领取  &nbsp;&nbsp;&nbsp;&nbsp;<span style="color: red;font-weight: bold">(合同作废：根据合同页数收费，每页合同收取1元；合同遗失：收取300元/份；请谨慎使用并保管合同！)</span>{elseif  $CSELECTED eq 2}合同归还{elseif  $CSELECTED eq 3}合同(未签)归还{/if}</th></thead><tbody>
        ...
        <tr><td style="text-align: right;">请输入
            ...
            <tr><td style="text-align: right">&nbsp;
                ...
                <span class="pull-left" style="font-size: 20px;color:red;">&nbsp;&nbsp;领取&nbsp;&nbsp;&nbsp;</span>
                {elseif $CSELECTED eq 2}
                    <label class="pull-left"  style="display:none;">
                    <input type="radio" name="cselected" value="2" checked>
                </label>
                <span class="pull-left" style="font-size: 20px;color:red;">&nbsp;&nbsp;归还&nbsp;&nbsp;&nbsp;</span>
                    {elseif $CSELECTED eq 3}
                <label class="pull-left" style="display:none;">
                    <input type="radio" name="cselected" value="3" checked>
                </label>
                        <span class="pull-left" style="font-size: 20px;color:red;">&nbsp;&nbsp;(未签)归还&nbsp;&nbsp;&nbsp;</span>
                    {elseif $CSELECTED eq 4}
                        <label class="pull-left" style="display:none;">
                            <input type="radio" name="cselected" value="4" checked>
                        </label>
                        <span class="pull-left" style="font-size: 20px;color:red;">&nbsp;&nbsp;(未签)作废&nbsp;&nbsp;&nbsp;</span>
                    {/if}
            </td></tr>
        </tbody></table>
    ...
    <script>
        ...
        $(function(){
            ...
            //$('input[name="cselected"]').click(function(){
                var clabel=$('input[name="cselected"]').val()==1?'领取人':($('input[name="cselected"]').val()==2?'归还人':($('input[name="cselected"]').val()==3?'未签归还人':'(未签)作废'));
                /*if($(this).val()!=1){
                    supermanflag=0;
                        ...
                        if(data.result!=null) {
                            if(data.result.rstatus=='userset'){
                               ...
                               alert(data.result.msg);
                                ...
                                var contract_classification = data.result.contract_classification;
                                if(contract_classification=='tripcontract'){
                                    var agentlists = data.result.agentlists;
                                    if(!agentlists.success){
                                        alert("获取代理商列表失败,请重试");
                                        return;
                                    }
                                    var optionstr = '';
                                    $.each(agentlists.data,function (k, v) {
                                        optionstr += "<option value='"+v.agentIdentity+"'>"+v.companyName+"</option>";
                                    });
                                    console.log(data);
                                      var message='<h3>请选择代理商</h3>';
                                    var msg={
                                        'message':message,
                                        "width":800
                                    };
                                    ...
                                    //     '<td class="fieldValue medium" colspan="4">' +
                                        var str ='<div class="row-fluid"><span class="span12">' +
                                        '<select  class="span12" name="agentid" >'+
                                        optionstr +
                                        '</select></span>' +
                                        '</div>' ;
                                        // '</td>' +
                                        // '</tr>' +
                                        // '<tr><td class="fieldValue medium" style="color: red;" colspan="4">三方合同在领取时候，需要l录入一个代理商</td></tr>'+
                                        // '</tbody></table>';
                                    Vtiger_Helper_Js.showConfirmationBox(msg).then(function(e){
                                        var agentid  = $("select[name='agentid']").val();
                                        var agentname = $("select[name='agentid'] option:selected").text();
                                        var params2={};
                                        params2['inputcode']=inputcode;
                                        params2['userid']=userid;
                                        params2['action']='BasicAjax';
                                        params2['module']=module;
                                        params2['mode']='tripContractReceived';
                                        params2['servicecontractsid']=data.result.servicecontractsid;
                                        params2['signpath']=data.result.signpath;
                                        params2['tempfield']=data.result.tempfield;
                                        params2['codenumbertemp']=data.result.codenumbertemp;
                                        params2['agentid']=agentid;
                                        params2['agentname']=agentname;
                                        AppConnector.request(params2).then(function(data2){
                                            if(data2.result!=null) {
                                                console.log(data2);
                                                if (data2.result.rstatus == 'contractok') {
                                                    /***合同处理***/
                                                    sumusernum++;
                                                    ...
                                                    var contractnoStr=mode=='Returned'?'<a href="/index.php?module=ServiceContracts&view=Detail&record='+data2.result.contractid+'" target="_blank">'+data2.result.contractno+'</a>':data2.result.contractno;
                                    ...
                                    },function(error, err) {}).fail(function(){setTimeout(bodytrigger,100);});
                                ...
                                var usrmsgcolor=mode=='Received'?'success':'warning';
                                var supermanflagl=supermanflag==0?'':'<span class="label label-warning"  style="margin-left:20px;">超领审核人: '+supermanname+'</span>';
                                var contractnoStr=mode=='Returned'?'<a href="/index.php?module=ServiceContracts&view=Detail&record='+data.result.contractid+'" target="_blank">'+data.result.contractno+'</a>':data.result.contractno;
                                $('<tr><td>' +contractnoStr+'</td><td><span class="label label-'+usercolor+'"  style="margin-left:20px;">'+usernamelabel+'</span><span class="label label-'+usrmsgcolor+'"  style="margin-left:20px;">'+usrmsg+'</span><span class="label label-'+usercolor+'"  style="font-size: 18px;margin-left:20px;">'+sumusernum+'</span>'+supermanflagl+'</td></tr>').insertAfter($('.contractsno thead'));
                                sumcontracts++;
                                colorflag=0;
                                $('.sumcontracts').text(sumcontracts);
                            ...
                            }else if(data.result.rstatus=='superman_status'){
                                superconllar=1;
                                if(supermanflag==1){
                                    supermanflag=0;
                                    superconllar=0;
                                    ...
                                    supermanflag=1;
                                    supermanid=data.result.userid;
                                    supermanname=data.result.username;
                                    supermancode=data.result.ucode;
                                    $('#inputcode').attr('placeholder','请再次输入合同编号');
                                    $('.usernamelabel').text(usernamelabel+'---超领审核人:'+data.result.username);
                                }
                            }else if(data.result.rstatus=='do_ok'){
                                if(data.result.mode=='NoSignReturned'){
                                    var bodyStr='<tr><td>'+data.result.contract_no+'</td><td>'+$(".usernamelabel").text()+'</td></tr>';
                                    $("#bartable1 tbody").append(bodyStr);
                                }
                    ...
                    AppConnector.request(params).then(
                        function(data) {
                            ...
                            $('#insertcode').html('<td><label  class="pull-right">' +
                                '<img id="qrcodeid" src="/index.php?module=ServiceContracts&view=ListAjax&mode=getQrcode&status={if $CSELECTED eq 1}Received{elseif  $CSELECTED eq 2}Returned{elseif  $CSELECTED eq 3}NoSignReturned{/if}" data-url="/index.php?module=ServiceContracts&view=ListAjax&mode=getQrcode&status={if $CSELECTED eq 1}Received{elseif  $CSELECTED eq 2}Returned{elseif  $CSELECTED eq 3}NoSignReturned{/if}"></label>' +
