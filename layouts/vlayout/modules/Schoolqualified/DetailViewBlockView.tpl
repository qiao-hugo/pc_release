...
{strip}
	...
	<input type="hidden" name="is_make_assessment" value="{$IS_MAKE_ASSESSMENT}">
	...
	<table class="table table-bordered blockContainer showInlineTable  detailview-table" id = "fallintotable">
        ...
        <tr >
            ...
            <td>{$VO['is_report']}</td>
            <td>{$VO['reportdate']}</td>
            <td>{$VO['trainstartdate']}</td>
            <td>{$VO['is_trainok']}</td>
            <td>{$VO['is_assessment']}</td>
            <td>{$VO['assessmentuser']}</td>
            <td>
            	<a  href="index.php?module=Schoolqualifiedpeople&view=Detail&record={$VO['schoolqualifiedpeopleid']}"><i title="详细信息" class="icon-th-list alignMiddle"></i></a>&nbsp;

				<a  href="index.php?module=Schoolqualifiedpeople&view=Edit&record={$VO['schoolqualifiedpeopleid']}"  target="_block"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="icon-pencil alignMiddle"></i></a>&nbsp;
            </td>
        </tr>
        {/foreach}
        </tbody>
