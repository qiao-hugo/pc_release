                                ...
                                {if $RELATED_HEADERNAME eq 'name'}
                                    <td class="{$WIDTHTYPE}  {$RELATED_HEADERNAME}"  nowrap>
                                        {if $IS_ROLEID OR (!$IS_ROLEID AND !in_array($MODULESTATUS,array('c_complete','c_cancel')))}
                                            <a class="btn-link" href="index.php?module=ContractExecution&action=DownloadFile&filename={$RELATED_RECORD->getId()|base64_encode}">{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}
                                            </a>
                                        {else}
