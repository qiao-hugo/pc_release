            ...
            <div class="btn-toolbar row-fluid">
                <div class="span8">
                    ...
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                        ...
                        data-url="{$RELATED_LINK->getUrl()}{if !empty($smarty.get.realoperate)}&realoperate={$smarty.get.realoperate}{/if}"
                    {/if}
...
<div class="contents-topscroll">
    <div class="topscroll-div">
        ...
        {elseif $RELATED_MODULE->get('name') neq 'Files'}
            <div class="relatedContents contents-bottomscroll">
                <div class="bottomscroll-div">
                    {assign var=WIDTHTYPE value=$USER_MODEL->get('rowheight')}
                    <table class="table table-bordered listViewEntriesTable">
                        ...
                        {assign var=RELATED_HEADERNAME value=$FIELDNAME}
                                ...
                                {if $RELATED_HEADERNAME eq 'name'}
                                    <td class="{$WIDTHTYPE}  {$RELATED_HEADERNAME}"  nowrap>
                                        ...
                                        {if $RELATED_RECORD->getDisplayValue('style') eq '合同D' && in_array($EXTENSION,array('pdf'))}
                                        ...
                                        {else}
                                            <a title="点击下载" class="btn-link" data-id="{$RELATED_RECORD->getDisplayValue('style')}" href="index.php?module=Vendors&action=DownloadFile&filename={$RELATED_RECORD->getId()|base64_encode}">{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}</a>
                                        {/if}
                                    </td>
                                {else}
                                    <td class="{$WIDTHTYPE}  {$RELATED_HEADERNAME}"  nowrap>
                        ...
                        <div class="pull-right actions">
                            <span class="actionImages">
                                {if $FILESTATE eq '草稿'}
                                    ...
                                    {/if}
                                    {if $CAN_DELETE eq '1' || $UPLOADER eq $LAST_NAME}
                                        <a  data-id="{$RELATED_RECORD->getId()}" class="files_delete" href="javascript:void(0);"><i title="删除" class="icon-trash alignMiddle"></i></a>&nbsp;
                                    {/if}
                                {else}
                                    {if $IS_CONTRACTSFILESDELETE eq '1' || $CAN_DELETE eq '1' || $UPLOADER eq $LAST_NAME}
                                        <a  data-id="{$RELATED_RECORD->getId()}" class="files_delete" href="javascript:void(0);"><i title="删除" class="icon-trash alignMiddle"></i></a>
                                    {/if}
                                {/if}

                                {if $CAN_DOWNLOAD eq 1 && $RELATED_RECORD->getDisplayValue('style') eq '合同D'}
                                    <a   class="btn-link" href="index.php?module=Vendors&action=DownloadFile&filename={$RELATED_RECORD->getId()|base64_encode}"><i title="合同下载" class="icon-download alignMiddle"></i></a>
                                {/if}
                                {*
