...
{assign var="READYONLY_FIELD" value=['bankaccount','bankname','bankcode','banknumber']}
...
<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" 
	   ...
	   value="{$FIELD_MODEL->get('fieldvalue')}"
		{if ($FIELD_MODEL->get('uitype') eq '106' && $MODE neq '') || $FIELD_MODEL->get('uitype') eq '3'
				|| $FIELD_MODEL->get('uitype') eq '4'|| $FIELD_MODEL->isReadOnly() || in_array($FIELD_NAME,$READYONLY_FIELD)}
				readonly="readonly"
		{/if}
