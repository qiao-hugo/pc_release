	...
	<div class="related span2 marginLeftZero">
		<div class="">
			<ul class="nav nav-stacked nav-pills">
				...
				<li class="{if $RELATED_LINK->getLabel()==$SELECTED_TAB_LABEL}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}{if !empty($smarty.get.realoperate)}&realoperate={$smarty.get.realoperate}{/if}" data-label-key="{$RELATED_LINK->getLabel()}" data-link-key="{$RELATED_LINK->get('linkKey')}" >
				...
				<li class="{if $RELATED_LINK['linklabel']==$SELECTED_TAB_LABEL}active{/if}" data-url="{$RELATED_LINK['linkurl']}&tab_label={$RELATED_LINK['linklabel']}{if !empty($smarty.get.realoperate)}&realoperate={$smarty.get.realoperate}{/if}" data-label-key="{$RELATED_LINK['linklabel']}" >
