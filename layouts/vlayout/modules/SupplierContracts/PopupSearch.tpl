            ...
            <span class="paddingLeft10px help-inline pushDownHalfper">
                ...
                <select style="width: 150px;" class="chzn-select help-inline" id="searchableColumnsList">
                    {if $MODULE eq 'Users'}
                        ...
                        {assign var="SEARCHARR" value=['contract_no'=>'contract_no','vendorid'=>'vendorid']}
                        {foreach key=block item=fields from=$RECORD_STRUCTURE}
                            {foreach key=fieldName item=fieldObject from=$fields}
                                {if in_array(strtolower($fieldName),$LISTVIEW_HEADERS) && in_array($fieldName,array_flip($SEARCHARR))}
                                    <option value="{$SEARCHARR[$fieldName]}">{if $fieldName eq 'vendorid'}客户/供应商{else}{vtranslate($fieldObject->get('label'),$MODULE)}{/if}</option>
