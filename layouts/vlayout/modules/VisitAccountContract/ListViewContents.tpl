...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries1 opendialog"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue"  nowrap>
                    ...
                    {elseif $LISTVIEW_HEADER['columnname'] eq 'commentstaus'}
						<span class="commentstaus{$LISTVIEW_ENTRY['id']}">{uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE)}</span>
                    {else}
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
						...
						<i title="点评详情" style="cursor:pointer;" class="icon-list alignMiddle opendialogc" data-id="{$LISTVIEW_ENTRY['id']}"></i>&nbsp;
                        {if $LISTVIEW_ENTRY['commentstaus'] eq ''}
						<i title="添加点评" style="cursor:pointer;" class="icon-pencil alignMiddle addcomments" data-id="{$LISTVIEW_ENTRY['id']}"></i>
						{/if}
