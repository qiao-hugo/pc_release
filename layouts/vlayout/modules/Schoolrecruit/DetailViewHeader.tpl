				...
				<div class="row-fluid">
					...
					<div class="span7">
						<div class="pull-right detailViewButtoncontainer">
							...
							<span class="btn-group"><button id="Schoolrecruit_detailView_basicAction_LBL_RELATED_LEAD_MAKESCHOOLQUALIFIED" class="btn make_schoolqualified btn-primary" data-act="LBL_RELATED_LEAD" ;=""><strong>
							{vtranslate('MAKESCHOOLQUALIFIED', 'Schoolrecruit')}
							</strong></button></span>
						
							
							{foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                            {if $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_ADD_RECORD'}{continue}{/if}
							<span class="btn-group">
								<button  id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
									{if $DETAIL_VIEW_BASIC_LINK->isPageLoadLink() && $DETAIL_VIEW_BASIC_LINK->getLabel() neq 'LBL_cancelled_LEAD'}
                                    class="btn" onclick="window.location.href='{$DETAIL_VIEW_BASIC_LINK->getUrl()}'"
									{elseif $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_cancelled_LEAD' or $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_RELATED_LEAD' or $DETAIL_VIEW_BASIC_LINK->getLabel() eq 'LBL_ACTIVATION_LEAD'}
                                        class="btn stagesubmit btn-primary" data-act="{$DETAIL_VIEW_BASIC_LINK->getLabel()}";
                                    {else}
                                    class="btn" onclick={$DETAIL_VIEW_BASIC_LINK->getUrl()}
									{/if}>
