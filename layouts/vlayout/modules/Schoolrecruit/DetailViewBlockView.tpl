...
{strip}
	...
	<script type="text/javascript">
		...
		var accessible_users = "<select style=\"width:200px;\" id=\"ddddd\" class=\"chzn-select chzn-done\" name=\"reportsower\"> 	{foreach key=OWNER_ID item=OWNER_NAME from=$ACCESSIBLE_USERS} <option value=\"{$OWNER_ID}\" data-picklistvalue= '{$OWNER_ID}' {if $FIELD_VALUE eq $OWNER_ID} selected {/if} data-userId=\"{$CURRENT_USER_ID}\">{$OWNER_NAME}</option> {/foreach} 	</select>";
	</script>
	<style type="text/css">
		 .select2-drop{
			z-index: 1000043;
	...
	<input type="hidden" name="is_make_qualified" value="{$IS_MAKE_QUALIFIED}">
    ...
    {*负责人签到*}
	{if $SIGNDATA1|@count neq 0}
	{foreach item=vo from=$SIGNDATA1}
	<table class="table table-bordered equalSplit detailview-table">
		<thead>
		<tr>
	...
	{*陪同人签到*}
	{if $SIGNDATA2|@count neq 0}
		{foreach item=vo key=FIELD_NAME from=$SIGNDATA2}
	<table class="table table-bordered equalSplit detailview-table">
		<thead>
		<tr>
				<th class="blockHeader" colspan="4">
