...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['receivedpaymentsid2']}' data-recordUrl='index.php?module=ReceivedPayments&view=Detail&record={$LISTVIEW_ENTRY['receivedpaymentsid2']}' id="ReceivedPayments_listView_row_{$smarty.foreach.listview.index+1}">
            ...
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue"  nowrap>
	                ...
	                {else if $LISTVIEW_HEADER['columnname'] eq sc_related_to }
	                ...
	                {else if $LISTVIEW_HEADER['columnname'] eq contract_no }
						<a class="btn-link" href="index.php?module=ServiceContracts&view=Detail&record={$LISTVIEW_ENTRY['servicecontractid2']}&realoperate={setoperate($LISTVIEW_ENTRY['servicecontractid2'],'ServiceContracts')}" target=_blank>{uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE)}</a>	                
	                {else}
	                	{uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE)}
	                {/if}
                </td>
            ...
            <td class="listViewEntryValue">
                {if $LISTVIEW_HEADER@last}
                    <div  style="width:90px">
                        <a  href="index.php?module=ReceivedPayments&view=Detail&record={$LISTVIEW_ENTRY['receivedpaymentsid2']}"><i title="修改" class="icon-th-list alignMiddle"></i></a>&nbsp;
                        {if $IS_MODULE_EDITABLE}
                        	<a  href='index.php?module=ReceivedPayments&view=Edit&record={$LISTVIEW_ENTRY['receivedpaymentsid2']}'  target="_block"><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="icon-pencil alignMiddle"></i></a>&nbsp;
                        {/if}
                    </div>
                {/if}
            </td>
