{strip}
    ...
    {*<span>*}
        ...
        {if Users_Privileges_Model::isReportPermissions("ContractReceivable", 'ExportOverdue')}
        <button id="exportReceivableOverdue" class="btn btn-info"><strong>导出</strong></button>&nbsp;
        {/if}
        <button id="collateReceivableOverdue" class="btn btn-info"><strong>核对</strong></button>&nbsp;
        <input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn" type="button" value="列表自定义">
        </span>
        </span>
        {*</span>*}
        {if $ISPAGE}
