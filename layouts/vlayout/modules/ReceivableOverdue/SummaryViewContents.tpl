		...
		{if $FIELD_MODEL->get('name') neq 'modifiedtime' && $FIELD_MODEL->get('name') neq 'createdtime'}
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue" style="width:70%">
					<div class="row-fluid">
						{if ($FIELD_MODEL->get('name')=='collection' || $FIELD_MODEL->get('name') =='bussinesstype') }
						<span class="value span10" style="word-wrap: break-word;">
							{vtranslate($FIELD_MODEL->get('fieldvalue'),'ReceivableOverdue')}
						</span>
						{elseif $FIELD_MODEL->get('name') !='accountid'}
						<span class="value span10" style="word-wrap: break-word;">
                        ...
                        {assign var=EDIT_AJAX_NAMES value=['LinkName','Title','Gender','Makedecision','Email','Website','Phone']}
                        {if in_array($FIELD_MODEL->get('label'),$EDIT_AJAX_NAMES)}
                            ...
                            {/if}
                        {elseif ($FIELD_MODEL->get('name')=='accountname' && $NAME_FLAG) || ($FIELD_MODEL->get('name')=='accountrank' && $RANK_FLAG)}
                                <span class="summaryViewEdit cursorPointer span2">
                                        <i class="icon-pencil" title="{vtranslate('LBL_EDIT',$MODULE_NAME)}"></i>
                                    </span>
                                <span class="hide edit span10">
