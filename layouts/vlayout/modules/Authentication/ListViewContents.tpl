                ...
                <table class="table listViewEntriesTable">
                    ...
                    {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
                        <tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}'  id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
                            ...
                            {/foreach}
                            <td class="listViewEntryValue">
                                {if $LISTVIEW_ENTRY['successorfail'] eq 'success'}
                                    <a href="#" class="btn btn-success">通过</a>
                                {else}
                                    <button class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="{$LISTVIEW_ENTRY['failreason']}">失败</button>
                                {/if}
                            </td>
                            <td class="listViewEntryValue">
                                {$LISTVIEW_ENTRY['createdtime']}
                            </td>

                            <td class="listViewEntryValue" >
                                {if $LISTVIEW_HEADER@last}
                                    <div  style="width:90px">
                                            <a class="deleteAuth" data-id="{$LISTVIEW_ENTRY['authenticationid']}"><i title="删除" class="icon-trash alignMiddle"></i></a>
                                    </div>
                                {/if}
                            </td>
                        </tr>
