...
{strip}
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                     ...
                     {assign var=FIELDAJAX value=array('overdue')}
                     {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->isAjaxEditable() eq 'true' && in_array($FIELD_MODEL->getName(),$FIELDAJAX)&& $OVERDUEISWRITE eq 1}
	...
	<table class="table table-bordered equalSplit detailview-table">
		...
		<tr>
				<th class="blockHeader" colspan="4">
					{vtranslate('陪同人签到情况',{$MODULE_NAME})}
				</th>
		</tr>
		</thead>
		 <tbody>
		 <tr>
			<td class="fieldLabel">
				<label class='muted pull-right marginRight10px'>签到人</label>
			</td>
			<td class="fieldValue">
			 	<span class="value">
					{$FIELD_MODEL.last_name} 
				</span>
			</td>

			<td class="fieldLabel">
				<label class='muted pull-right marginRight10px'>是否签到</label>
			</td>
			<td class="fieldValue">
			 	<span class="value">
					{$FIELD_MODEL.issign}
