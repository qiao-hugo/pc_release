...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{if $RECORD_ID neq ''}
			<h3 class="span8 textOverflowEllipsis" title="{$RECORD_ACCOUNTNAME}">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$RECORD_ACCOUNTNAME}</h3>
		{else}
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<table id="blockContainer_{$BLOCK_LABEL}" class="table table-bordered blockContainer showInlineTable {if $BLOCK_LABEL eq 'LBL_IDCRECORDS_INFORMATION' && $IDCRECORDS_TYPE eq 'LBL_ROREGIN_AIDCTYPE'} hide tableadv{/if}">
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				<td class="fieldLabel {$WIDTHTYPE}">
                   {* 自动读取工单编号，设置隐藏值
                    {if ($FIELD_MODEL->getName() eq 'salesorder_no')}
                        <input type="hidden" name="hidden_salesorder_no" value="{$FIELD_MODEL->get('fieldvalue')}"/>
                    {/if}*}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                        {if $FIELD_MODEL->get('label') eq 'DomainName'}
                            <textarea class="span6" height="80" name="domainname" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]">
                                 {$IDCRECORDS_NAME}
                            </textarea>
                        {else}
