 ...
 * All Rights Reserved.
*<!--去除双击编辑-->
{var_dump($LISTTASKDATA)}
 ********************************************************************************/
-->*}
{strip}
<input type="hidden" value="{$AUTOFOLOID}" id="source_record">
	<table class="table table-bordered ">
		...
		<tbody>
		{foreach item=DATA from=$LISTTASKDATA}
		<tr>
		<td>{vtranslate({$DATA['modulename']},{$DATA['modulename']})}</td>
		<td>{$DATA['autoworkflowtaskname']}</td>
            <td>
                <span  class='label {if $DATA['isaction'] eq "0"}label-warning{elseif $DATA['isaction'] eq "1"}label-success{else if $DATA['isaction'] eq "2"}label-important{/if}'>
                    {if $DATA['isaction'] eq "0"}未开始{elseif $DATA['isaction'] eq "1"}已激活{else if $DATA['isaction'] eq "2"}已完成{/if}
                </span>
            </td>
            <td>{$DATA['auditedtime']}</td>
            <td>
                {if $DATA['mail_report'] eq '1'}
                    <a target="_blank" href="index.php?module=AutoTask&view=Detail&record={$DATA['autoworkflowentityid']}&source_record={$DATA['autoworkflowid']}&mailid= {$DATA['mail_id']}&mode=maildetail">成功</a>
                {elseif $DATA['mail_report'] eq '2'}
                    失败
                {else}
                    未发送
                {/if}
		    ...
		    <td width="400">{$DATA['taskremark']}</td>
