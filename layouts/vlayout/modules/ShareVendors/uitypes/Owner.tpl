...
{if $FIELD_MODEL->get('uitype') eq '54' && $FIELD_MODEL->get('name') eq 'userid'}
    ...
    {assign var="FIELD_VALUE_LIST" value=$FIELD_MODEL->get('fieldvalue')}
    {if $FIELD_VALUE eq ''}
        {assign var=FIELD_VALUE value=$CURRENT_USER_ID}
    {/if}
        ...
        {foreach key=DEPARTMENTNAME item=DEPARTMENTNAME_LIST from=$ALL_ACTIVEUSER_LIST}
			<optgroup label="{$DEPARTMENTNAME}">
                {foreach key=OWNER_ID item=OWNER_NAME from=$DEPARTMENTNAME_LIST}
					{if $RECORD_ID>0}
                    {if $OWNER_ID eq  $FIELD_VALUE_LIST}
                    {$OWNER_ID}
					<option value="{$OWNER_ID}" data-picklistvalue= '{$OWNER_NAME}' {if $OWNER_ID eq  $FIELD_VALUE_LIST} selected {/if}
...
{elseif $FIELD_MODEL->get('uitype') eq '54'}
	{assign var=ALL_ACTIVEUSER_LIST value=$USER_MODEL->getAccessibleUsers($FIELD_MODEL->get('uitype'),'','',$DEPARTMENTID)}
