...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{if $RECORD_ID neq ''}
			<h3 class="span8 textOverflowEllipsis" title="{$RECORD_CONTRACT_TYPE}">{vtranslate('LBL_EDITING', $MODULE)} {vtranslate($SINGLE_MODULE_NAME, $MODULE)} - {$RECORD_CONTRACT_TYPE}</h3>
		{else}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						<div class="row-fluid">
							<span class="span10">
								{assign var=EDIT_FIELD_NAMES value=['disposeid','ownerid','productmaintainer','productman','addtime','finishtime','timeconsuming','disposeresult']}
								{if in_array($FIELD_MODEL->get('name'),$EDIT_FIELD_NAMES)}
									{if ($FIELD_MODEL->get('name') eq 'productmaintainer')}
									...
									{/if}
								{else}
								    {if ($IS_RELATION_OPERATION eq 'true')}
								    	{if ($FIELD_MODEL->get('name') eq 'productid' or $FIELD_MODEL->get('name') eq 'related_to' or $FIELD_MODEL->get('name') eq 'serviceid')}
								        	{$FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue'), $record, $RECORD)}
								        {else}
											{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
										{/if}
								    {else}
								    	{if ($RELATION_OPERATION eq '1')}
									        {if ($FIELD_MODEL->get('name') eq 'productid' or $FIELD_MODEL->get('name') eq 'related_to' or $FIELD_MODEL->get('name') eq 'serviceid')}
									        	{$FIELD_MODEL->getDisplayValue($FIELD_MODEL->get('fieldvalue'), $record, $RECORD)}
									        {else}
												{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
    ...
    {include file='EditViewBlocksProducts.tpl'|@vtemplate_path:$MODULE}
{/strip}