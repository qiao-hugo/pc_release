...
<script type="text/javascript">
    {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
    $('#ContractsAgreement_editView_fieldName_{$FIELD_MODEL->getFieldName()}').validationEngine();
    {/foreach}
    $('select[name="eleccontracttplid"]').removeClass('chzn-select');
    $('select[name="eleccontracttplid"]').after('<button type="button" class="btn btn-info preeleccontracttpl" data-name="eleccontracttpl">预览</button>');
    //$('input[name="relatedattachment"]').after('<button type="button" class="btn btn-info prerelatedattachment" data-name="relatedattachment">预览</button>');
