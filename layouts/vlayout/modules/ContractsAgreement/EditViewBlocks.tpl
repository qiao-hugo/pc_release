...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{assign var="SIGNATURETYPE" value=$RECORD_STRUCTURE_MODEL->getRecord()->get('signaturetype')}
		{assign var="COUNTINUFIELDS" value=['eleccontracttpl','eleccontractid']}
		{assign var="ELECCONTRACT" value=['originator','originatormobile','elereceiver','elereceivermobile','eleccontractstatus','eleccontracttpl','relatedattachment','contractattribute','clientproperty','eleccontracttplid','eleccontractid','relatedattachmentid','clientproperty']}
		{assign var="ELECCONTRACTNODISPLAY" value=['receiptorid','eleccontracttplid']}
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			{if $BLOCK_FIELDS|@count lte 0}{continue}{/if}
			{if $BLOCK_LABEL eq 'ELECCONTRACT_INFO' && $SIGNATURETYPEHREF neq 'eleccontract'}{continue}{/if}
			<table class="table table-bordered blockContainer showInlineTable {$BLOCK_LABEL}">
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if in_array($FIELD_MODEL->getFieldName(),$ELECCONTRACTNODISPLAY) && $SIGNATURETYPEHREF eq 'eleccontract'}{continue}{/if}
