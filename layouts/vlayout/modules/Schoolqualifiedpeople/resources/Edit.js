...
Vtiger_Edit_Js("Schoolqualifiedpeople_Edit_Js",{},{
	...
	registerRecordPreSaveEvent : function(form) {
		...
		form.on(Vtiger_Edit_Js.recordPreSave, function(e, data) {
			...
			var is_trainok = $('#Schoolqualifiedpeople_editView_fieldName_is_assessment').attr('checked');
			var assessmentuser = $('select[name=assessmentuser]').val();
			if (!assessmentuser && is_trainok=='checked') {
				var  params = {text : app.vtranslate(' 参与考核必须选择考核人员'),
				title : app.vtranslate('')}
				Vtiger_Helper_Js.showPnotify(params);
				e.preventDefault();
			}
	...
	init: function() {
		...
		$('input[name=qualifiedpeoplename]').attr('readonly', 'readonly');
		$('input[name=telephone]').attr('readonly', 'readonly');
		//$('select[name=gendertype]').next().find('.chzn-results').remove(); // 填写人员不可修改
	},
