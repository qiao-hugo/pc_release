	...
	<table class="table listViewEntriesTable">
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue {if $LISTVIEW_HEADER['columnname'] eq 'assessmentresult'}assessmentresult_value{/if} "   nowrap>
                    {if $LISTVIEW_HEADER['columnname'] eq 'p_assessownerid'}
                         <a class="btn-link" href='index.php?module=Schoolemploy&view=Detail&record={$LISTVIEW_ENTRY['schoolemployid']}' target="_blank">{vtranslate( $LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']],$MODULE)}</a>
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
                        ...
                        <a target="_blank"  href="index.php?module=Users&parent=Settings&view=Edit&sid={($LISTVIEW_ENTRY['schoolresumeid_reference']+ 99999)|base64_encode}" ><i title="正式录用" class="icon-shopping-cart alignMiddle"></i></a>&nbsp;
