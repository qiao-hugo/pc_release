...
{strip}
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                     ...
                     {assign var=AJAX_EDIT value=['linkname','makedecisiontype','gendertype','email1','mobile','title']}
    ...
    <table class="table table-bordered table-striped blockContainer showInlineTable" id = "">
        <thead>
        <tr><th class="blockHeader" colspan="4"><span >客户回访计划详细</span></th></tr>
        <tr>
            <td><b>回访次数</b></td>
            <td><b>间隔时间上限</b></td>
            <td><b>间隔时间下限</b></td>
            ...
            {foreach item="returnplan_detail" from="$RETURNPLAN_DETAIL"}
                <tr>
                    <td style="width: 70px;">
                        ...
                        <b>{$returnplan_detail['sequence']}</b>
                    </td>
                    <td style="width: 100px;" >
                           <b>{$returnplan_detail['upperlimit']}</b>
                    </td>
                    <td style="width: 100px;">
                            <b>{$returnplan_detail['lowerlimit']}</b>
                    </td>
                    <td>
                        {$returnplan_detail['returnplantext']}
                    </td>
                </tr>
            {/foreach}
        {/if}
