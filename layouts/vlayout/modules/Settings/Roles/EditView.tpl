	...
	<div class="container-fluid">
		...
		<form name="EditRole" action="index.php" method="post" id="EditView" class="form-horizontal">
			...
			<div class="row-fluid">
				...
				<div class="row-fluid">
					<label class="fieldLabel span3"><strong>职位类别<span style="color:red;">*</span>: </strong></label>
					<div class="span8 fieldValue">
						{assign var="JOBCATEGORY" value=$RECORD_MODEL->getjobcategory()}
						<select class="select2" id="jobcategory" name="jobcategory" data-placeholder="{vtranslate('LBL_CHOOSE_PROFILES',$QUALIFIED_MODULE)}" data-validation-engine='validate[required]' style="width: 800px">
							{foreach from=$JOBCATEGORY item=CATEGORY}
								<option value="{$CATEGORY['jobcategory']}" {if $RECORD_MODEL->getcategory() eq $CATEGORY['jobcategory']}selected="true"{/if}>{vtranslate($CATEGORY['jobcategory'],$QUALIFIED_MODULE)}</option>
							{/foreach}
						</select>
					</div>
				</div><br>
