...
var Settings_Departments_Js = {
	...
	initEditView: function() {
		...
		function modalActionHandler(event) {
			...
			app.showModalWindow(null, target.data('url'),function(data){
				Settings_Departments_Js.initDeleteView();
			});
		}
	...
	registerShowNewProfilePrivilegesEvent : function() {
		jQuery('[name="profile_directly_related_to_role"]').on('change',function(e){
			...
			if(hanlder == 'new'){
				Settings_Departments_Js.getProfilePriviliges();return false;
			}
	...
	getProfilePriviliges : function() {
		...
		if(Settings_Departments_Js.newPriviliges == true) {
		...
		AppConnector.request(params).then(function(data) {
			...
			Settings_Departments_Js.registerExistingProfilesChangeEvent();
			progressIndicatorElement.progressIndicator({
				'mode' : 'hide'
			});
			Settings_Departments_Js.newPriviliges = true;
			jQuery('[data-content="existing"]').fadeOut('slow',function(){
		...
		form.on('submit',function(e) {
			if(form.data('submit') == 'true' && form.data('performCheck') == 'true') {
				...
				if(form.data('jqv').InvalidFields.length <= 0) {
					...
					if(formData.isjuridicalperson==1 && formData.erpaccount==''){
                        var params = {};
                        params['text'] = '必须要指定法人的ERP账套';
                        params['type'] = 'error';
                        Settings_Vtiger_Index_Js.showMessage(params);
                        return false;
					}

					thisInstance.checkDuplicateName({   //检查重复
     ...
     */
	showOrHide:function(){
		$('.icon-eye-open-right').on('click',function(){
        ...
        $('.icon-eye-close-lower').on('click',function(){
            var classid=$(this).data('id');
            $('.'+classid).hide();
            $('.open'+classid).show();
	...
	/*
	 * Function to check Duplication of Department Names
	 * returns boolean true or false
	 */

	checkDuplicateName : function(details) {
		var aDeferred = jQuery.Deferred();  //链式调用的工具
	...
	registerEvents : function() {
		Settings_Departments_Js.initEditView();
		Settings_Departments_Js.registerShowNewProfilePrivilegesEvent();
		Settings_Departments_Js.onLoadProfilePrivilegesAjax();
		Settings_Departments_Js.registerSubmitEvent();
        Settings_Departments_Js.showOrHide();
	}
}
jQuery(document).ready(function(){
	Settings_Departments_Js.registerEvents();
})