				...
				<table class="table table-striped table-bordered">
					<thead>
						...
						<tr>
							...
							<th width="11%" style="border-left: 1px solid #DDD !important;">
								<span class="horizontalAlignCenter" >{'LBL_POPUP_PRVILIGE'|vtranslate:$QUALIFIED_MODULE}</span>
							</th>
					...
					<tbody>
						...
						{foreach from=$RECORD_MODEL->getModulePermissions() key=TABID item=PROFILE_MODULE}
							...
							<tr>
								...
								{assign var="ACTION_PERMISSION" value=$PROFILE_MODULE->permissions}
								...
								{foreach from=$BASIC_ACTION_ORDER item=ACTION_ID}
									<td style="border-left: 1px solid #DDD !important;">
										...
										{if !$IS_RESTRICTED_MODULE && $ACTION_MODEL->isModuleEnabled($PROFILE_MODULE)}
											{assign var="PERMISSION_VALUE" value=$PERMISSION_LIST[$ACTION_PERMISSION['actions'][$ACTION_ID]]}
											 
											{$PERMISSION_VALUE|vtranslate:$QUALIFIED_MODULE}
										{/if}
									</td>
								{/foreach}
								<td style="border-left: 1px solid #DDD !important;">
