					...
					<thead>
						<tr class="blockHeader">
							...
							<th width="14%" style="border-left: 1px solid #DDD !important;">
								<!--<input {if empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} class="alignTop"  checked="true" {/if} type="checkbox"  />-->
								{'LBL_VIEW_PRVILIGE'|vtranslate:$QUALIFIED_MODULE}&nbsp;
								<select  style="width:140px;" id="mainAction4CheckBox">
										<optgroup>
											{foreach key=index item=value from=$ACTION_AUTH}
												<option value="{$index}" {if $value eq $REFERENCED_MODULE_NAME} selected {/if}>{vtranslate($value, $QUALIFIED_MODULE)} </option>
											{/foreach}
										</optgroup>
									</select>
							</th>
							<th width="14%" style="border-left: 1px solid #DDD !important;">
								...
								<select  style="width:100px;" id="mainAction1CheckBox">
										<optgroup>
											{foreach key=index item=value from=$ACTION_AUTH}
												<option value="{$index}" {if $value eq $REFERENCED_MODULE_NAME} selected {/if}>{vtranslate($value, $QUALIFIED_MODULE)} </option>
											{/foreach}
							...
							<th width="14%" style="border-left: 1px solid #DDD !important;">
								<!--<input checked="true" class="alignTop" type="checkbox"  />-->&nbsp;
								{'LBL_DELETE_PRVILIGE'|vtranslate:$QUALIFIED_MODULE}
								<select  style="width:100px;" id="mainAction2CheckBox">
										<optgroup>
											{foreach key=index item=value from=$ACTION_AUTH}
												<option value="{$index}" {if $value eq $REFERENCED_MODULE_NAME} selected {/if}>{vtranslate($value, $QUALIFIED_MODULE)} </option>
											{/foreach}
							...
							<th width="14%" style="border-left: 1px solid #DDD !important;">
								<!--<input checked="true" class="alignTop" type="checkbox"  />-->&nbsp;
								{'LBL_POPUP_PRVILIGE'|vtranslate:$QUALIFIED_MODULE}
								<select  style="width:100px;" id="mainAction11CheckBox">
										<optgroup>
											{foreach key=index item=value from=$ACTION_AUTH}
												<option value="{$index}" {if $value eq $REFERENCED_MODULE_NAME} selected {/if}>{vtranslate($value, $QUALIFIED_MODULE)} </option>
											{/foreach}
					...
					<tbody>
						...
						{foreach from=$PROFILE_MODULES key=TABID item=PROFILE_MODULE}
							...
							<tr>
								...
								{assign var="BASIC_ACTION_ORDER" value=array(4,1,2,11)}
								{foreach from=$BASIC_ACTION_ORDER item=ORDERID}
									<td style="border-left: 1px solid #DDD !important;">
										...
										{if !$IS_RESTRICTED_MODULE && $ACTION_MODEL->isModuleEnabled($PROFILE_MODULE)}
											<!--<input style="margin-left: 45% !important" type="checkbox" data-action-state="{$ACTION_MODEL->getName()}" {if $RECORD_MODEL->hasModuleActionPermission($PROFILE_MODULE, $ACTION_MODEL)}checked="true"{elseif empty($RECORD_ID) && empty($IS_DUPLICATE_RECORD)} checked="true" {else} data-action{$ACTION_ID}-unchecked="true"{/if}>-->
											
											{assign var=AUTH_VALUE value=$PROFILE_MODULE->get('permissions')}
											
											 
											<select  style="width:140px;"  class="action{$ACTION_ID}CheckBox" name="permissions[{$TABID}][actions][{$ACTION_ID}]" data-action-state="{$ACTION_MODEL->getName()}" >
										<optgroup>
											{foreach key=index item=value from=$ACTION_AUTH}
												<option value="{$index}" {if $index eq $AUTH_VALUE['actions'][$ACTION_ID]} selected {/if}>{vtranslate($value, $QUALIFIED_MODULE)} </option>
											{/foreach}
										</optgroup>
									</select>
						...
						<tr class="hide">
							...
							<td colspan="6" class="row-fluid" style="padding-left: 5%;padding-right: 5%;background-image: none !important;">
								{if !empty($ALL_UTILITY_ACTIONS_ARRAY)}
								<div class="row-fluid hide" data-togglecontent="{$TABID}-fields">
