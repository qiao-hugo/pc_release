...
jQuery.Class('Settings_Sharing_Access_Js', {}, {
	...
	saveMultiCustomRule : function(form, e) {
		var thisInstance = this;
		var data = form.serializeFormData();
	...
	newCustomRule:function(url) {
		var form = jQuery('#newCustomRule');

		form.on('submit', function(e) {
			//To stop the submit of form
			e.preventDefault();
			var formElement = jQuery(e.currentTarget);
			thisInstance.saveMultiCustomRule(formElement, e);
		})
	},
	...
	registerRecordPreSave : function(){
		var thisInstance = this;
		$('body').on('submit','#EditView', function(e, data) {
			var source_id=$('select[name="source_id"]').val();
			if(source_id==''){
				var params = {};
				params['text'] = '团队群组不能为空';
				params['type'] = 'error';
				Settings_Vtiger_Index_Js.showMessage(params);
				return false;
			}
			/*var compay_id=$('select[name="compay_id[]"]').val();
			if(compay_id==null ){
				var params = {};
				params['text'] = '所属公司不能为空';
				params['type'] = 'error';
				Settings_Vtiger_Index_Js.showMessage(params);
				return false;
			}*/
			/*var target_id=$('select[name="target_id[]"]').val();
			if(target_id==null){
				var params = {};
				params['text'] = '可访问部门不能为空';
				params['type'] = 'error';
				Settings_Vtiger_Index_Js.showMessage(params);
				return false;
			}*/
			var for_module=$('select[name="for_module[]"]').val();
			if(for_module==null){
				var params = {};
				params['text'] = '可访问模块不能为空';
				params['type'] = 'error';
				Settings_Vtiger_Index_Js.showMessage(params);
				return false;
			}
 ...
 * function to register click event for radio buttons
 */
	registerSharingAccessNew : function() {
		$('#searchButton').click(function () {
			var search_select_value = $("#search_select").val();
			console.log(search_select_value);
			app.showModalWindow(null,'index.php?module=SharingAccess&parent=Settings&view=IndexAjax&mode=searchRules&share_roleid='+search_select_value);
		});
	},
	...
	registerEvents : function() {
		...
		thisInstance.registerSharingAccessNew();
		thisInstance.registerRecordPreSave();
