				...
				<div class="row-fluid">
					<span class="span6">
						...
						<select id="memberList" class="row-fluid members" multiple="true" name="members[]" data-placeholder="{vtranslate('LBL_ADD_USERS_ROLES', $QUALIFIED_MODULE)}" data-validation-engine="validate[required]">
							{*{foreach from=$MEMBER_GROUPS key=GROUP_LABEL item=ALL_ACTIVEUSER_LIST}
...
<option value="{$OWNER_ID}" data-member-type="Users" data-picklistvalue= '{$OWNER_NAME}' {if isset($GROUP_MEMBERS['Users'][$KEYID])} selected {/if}>
				                    	{$OWNER_NAME}
				                    </option>
								{/foreach}
								</optgroup>
					...
					<span class="span3">
						<span class="pull-right groupMembersColors">
							<ul class="liStyleNone" style="width:100%;">
