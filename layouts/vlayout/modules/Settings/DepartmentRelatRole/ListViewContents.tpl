	...
	<table class="table table-bordered table-condensed listViewEntriesTable">
		...
		{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES}
			<tr class="listViewEntries" data-id="{$LISTVIEW_ENTRY->getId()}"
				data-departmentid="{$LISTVIEW_ENTRY->get('departmentid')}"
				data-roleid="{$LISTVIEW_ENTRY->get('roleid')}"
				data-remark="{$LISTVIEW_ENTRY->getDisplayValue('remark')}"
			...
			<td width="1%" nowrap class="{$WIDTHTYPE}">
				...
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					<td class="listViewEntryValue {$WIDTHTYPE} {$LISTVIEW_HEADERNAME}"  width="{$WIDTH}%" nowrap>
									...
									<span class="actionImages">
										{foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
											{assign var="RECORD_LINK_URL" value=$RECORD_LINK->getUrl()}
												<i class="{$RECORD_LINK->getIcon()} alignMiddle {$RECORD_LINK->getLabel()}"  title="{vtranslate($RECORD_LINK->getLabel(), $QUALIFIED_MODULE)}"></i>
											{if !$RECORD_LINK@last}
												&nbsp;&nbsp
											{/if}
										{/foreach}
