 ...
 * All Rights Reserved.
 *************************************************************************************/
Settings_Vtiger_Edit_Js("Settings_AutoWorkflows_Edit_Js",{
	instance : {},
	editInstances : "wangbin"
},{
	...
	getInstance : function(step) {
		if(step in Settings_AutoWorkflows_Edit_Js.instance ){
			return Settings_AutoWorkflows_Edit_Js.instance[step];
		} else {
			var moduleClassName = 'Settings_AutoWorkflows_Edit'+step+'_Js' ;
			//console.log (window[moduleClassName]); //window 为什么是这种调用方式呢；
			Settings_AutoWorkflows_Edit_Js.instance[step] =  new window[moduleClassName]();
			return Settings_AutoWorkflows_Edit_Js.instance[step]
		}
	},
	...
	activateHeader : function(step) {
		...
		if(source_record){
			...
			jQuery('#step3',headersContainer).addClass('active');
		}else{
			headersContainer.find('.active').removeClass('active');
	...
	//wangbin 选择模块时加载 字段条件
	loadingcondition : function(){
		$("select[name='module_name']").change(function(){
			var widgetContainer = $(".tempcondition");
			widgetContainer.progressIndicator({});
			
			var selectmodulename = $(this).val();
			var AutoWorkflowsrecord = $("input[name='record']").val();
			
			
			var urlParams = "module=AutoWorkflows&parent=Settings&view=Edit&mode=getcondition&record="+AutoWorkflowsrecord+"&selectmodulename="+selectmodulename;
			var params = {
					'type' : 'GET',
					'dataType': 'html',
					'data' : urlParams
	...
	//wangbin 任务属性设置人员信息;
		registerEventForSelect2Element : function(){
			...
			var selectElement = jQuery('#memberList');
			var params = {};
