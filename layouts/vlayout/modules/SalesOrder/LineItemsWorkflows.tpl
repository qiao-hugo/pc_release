...
<br>
        ...
        {assign var=USERDEPARTMENTH223 value=getDepartmentUser('H223')}
        <table class="table table-bordered equalSplit detailview-table ">
            <thead>
            {if ($ISROLE and $STAGERECORDID) || in_array($USER,$USERDEPARTMENT) || in_array($USER,$USERDEPARTMENTH223)}
			...
			<tr>
                ...
                <th class="detailViewBlockHeader" nowrap>
                {if ($ISROLE and $STAGERECORDID) or in_array($USER,$USERDEPARTMENT) OR in_array($USER,$USERDEPARTMENTH223)}<div class="pull-right"><button type="button" onclick="$('.realremarkbutton').show();$('#remarkvalue').focus();" class="btn btn-info">添加备注</button>&nbsp;</div>{/if}
                </th>
            </tr>
            </thead>
