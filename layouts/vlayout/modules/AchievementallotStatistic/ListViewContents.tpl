...
{assign var=WITHHOLD value=$RECORD->personalAuthority('AchievementallotStatistic','withhold')}
	...
	<table class="table listViewEntriesTable">
		<thead>
			<tr class="listViewHeaders">
				...
				<td><input type="checkbox"  name="checkAll" class="entryCheckBox"></td>
		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['achievementallotid']}' data-recordUrl='index.php?module=AchievementallotStatistic&view=Detail&record={$LISTVIEW_ENTRY['achievementallotid']}&mode=showDetailViewByMode' id="AchievementallotStatistic_listView_row_{$smarty.foreach.listview.index+1}">
			{* 修改业绩月份  <td style="text-align: left;">
					<div class="deletedflag">
						<label style="height:100%;display: inline-block;">
						<input type="checkbox" value="{$LISTVIEW_ENTRY['achievementallotid']}" class="entryCheckBox" name="Detailrecord[]" ></label>
					</div>
			</td> *}
			<td><label><input type="checkbox" value="{$LISTVIEW_ENTRY['achievementallotid']}"  data-amount="{$LISTVIEW_ENTRY['arriveachievement']}"  class="entryCheckBox" name="Detailrecord[]"></label></td>
			{foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
				{if $LISTVIEW_HEADER['ishidden']}
					{continue}
				{/if}
                <td class="listViewEntryValue {if $LISTVIEW_HEADER['columnname'] eq adjustachievement } adjustachievementTd {/if} {if $LISTVIEW_HEADER['columnname'] eq adjustremarks } adjustremarksTd {/if} {if $LISTVIEW_HEADER['columnname'] eq arriveachievement } arriveachievementTd {/if}"     nowrap>
            ...
            <td class="listViewEntryValue">
                {if $LISTVIEW_HEADER@last}
                    <div  style="width:90px">
						{if $ISCANDO }
							<a  href="###" data-isover="{$LISTVIEW_ENTRY['confirmstatus']}" class="applicationUpdateAchievement">调整业绩</a>&nbsp;
						{/if}
						{if $WITHHOLD}
							{if $LISTVIEW_ENTRY['is_deduction'] eq '是'}
								<a style="color:green"><i title="确认发放" class="alignMiddle applicationWithholdAchievement" data-waithold="1" data-updatetitle="暂缓发放" data-updatewaithold="0" data-color="red" data-currentcolor="green">确认发放</i></a>&nbsp;
							{else}
								<a style="color:red"><i title="暂缓发放" class="alignMiddle applicationWithholdAchievement"  data-waithold="0"  data-updatetitle="确认发放" data-updatewaithold="1" data-color="green" data-currentcolor="red">暂缓发放</i></a>&nbsp;
							{/if}
						{/if}
                        <a  href="index.php?module=AchievementallotStatistic&view=Detail&record={$LISTVIEW_ENTRY['achievementallotid']}"><i title="详细信息" class="icon-th-list alignMiddle"></i></a>&nbsp;
                    </div>
                {/if}
            </td>
		</tr>
