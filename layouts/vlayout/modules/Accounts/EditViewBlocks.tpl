...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<table class="table table-bordered blockContainer showInlineTable aaa">
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						...
						{else}
						<div class="row-fluid" {if $FIELD_MODEL->get('name') eq 'industry'} style="width: 420px;" {/if}>
							<span class="span10">
								{include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getTemplateName(),$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
								{if $FIELD_MODEL->get('name') eq 'newleadsource'}
									<br>
									<span style="color: orange">温馨提示：切勿瞎填，数据的准确性可帮助数据统计与分析</span>
								{elseif $FIELD_MODEL->get('name') eq 'accountlabeling'}
									<br>
									<span style="color: red">提示：仅渠道部人员填写</span>
								{/if}
							</span>
						</div>
						{/if}
