...
{strip}
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                     ...
                     {* 添加实时在线修改 steel 2015-05-11 *}
                     {assign var=AJAX_EDIT value=['linkname','makedecisiontype','gendertype','email1','title','phone']}
					  {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->isAjaxEditable() eq 'true' && in_array($FIELD_MODEL->get('name'),$AJAX_EDIT)}
	...
	<div class="widgetContainer_servicecontract" data-url="module=ServiceContracts&view=Detail&mode=getProducts&amp;record={$RECORD->getId()}" data-name="Workflows">
