...
{strip}
	...
	<input id="taxType" type="hidden" value="{$RECORD->get('taxtype')}" />
    <input id="today" type="hidden" value="{date('Y-m-d')}" />
					...
					<div class="span7">
						<div class="pull-right detailViewButtoncontainer">
							...
							{foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
                                {if $DETAIL_VIEW_BASIC_LINK->getLabel() neq 'LBL_ADD_RECORD'}
								{if Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel()) eq 'LBL_CHANGE_CONTRACTS'}
									{if Users_Privileges_Model::isReportPermissions("SalesorderWorkflowStages", 'ChangeContracts')}
