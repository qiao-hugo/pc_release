{strip}
    ...
    <form action="/index.php?module=Newinvoice&view=List&filter=search_newinvoiceextende_data" method="post" onsubmit="return _submit()">
        <th colspan="2"><h4>发票查询</h4></th></thead><tbody>
        
        <tr><td style="text-align: right">发票代码
            </td><td>
                <label class="pull-left">
                    <input type="text" name="invoicecodeextend" >
                </label>
            </td>
        </tr>
        <tr><td style="text-align: right">发票号码
            </td><td>
                <label class="pull-left">
                    <input type="text" name="invoice_noextend">
                </label>
            </td>
        </tr>
        <tr><td style="text-align: right">实际开票抬头
            </td><td>
                <label class="pull-left">
                    <input type="text" name="businessnamesextend">
                </label>
            </td>
        </tr>
        <tr><td style="text-align: right">开票人
            </td><td>
                <label class="pull-left">
                <select  name="drawerextend" class="chzn-select referenceModulesList streched"">
        ...
        <tr><td style="text-align: right">开票日期
            </td><td>
                <label class="pull-left">
                    {assign var="signdate_end"   value=date('Y-m-d')} 
                    {assign var="signdate_start" value=date('Y-m-d', strtotime('-3 month'))} 
                    <input type="text" value="{$signdate_start}" id="signdate_start" name="billingtimerextend_start"> 到 <input type="text" id="signdate_end" name="billingtimerextend_end" value="{$signdate_end}">
                </label>
            </td>
        </tr>
        ...
        <div class="row-fluid" id="c" style="width:100%;">
            <div style="border:1px solid #ccc;margin:0 auto 20px;padding-right:20px;padding-top:10px;">
                ...
                <table id="tbl_Detail" class="table listViewEntriesTable" width="100%"><thead><tr>
                        <th nowrap><b>发票代码</b></th>
                        <th nowrap><b>发票号码</b></th>
                        <th nowrap><b>发票状态</b></th>
                        <th nowrap><b>开票日期</b></th>
                        <th nowrap><b>开票人</b></th>

                        <th nowrap><b>商品名称</b></th>
                        <th nowrap><b>税价合计</b></th>
                        <th nowrap><b>剩余税价合计</b></th>
                ...
                {foreach item=value from=$RES_DATA}
                    <tr>
                        <td nowrap><b>{$value['invoicecodeextend']}</b></td>
                        <td nowrap><b>{$value['invoice_noextend']}
                            </b></td>
                        <td nowrap><b>{$value['t_invoicestatus']}</b></td>
                        <td nowrap><b>{$value['billingtimeextend']}</b></td>

                        <td nowrap><b>{$value['t_drawerextend']}</b></td>
                        <td nowrap><b>{$value['commoditynameextend']}</b></td>
                        <td nowrap><b>{$value['totalandtaxextend']}</b></td>
                        <td nowrap><b>{$value['t_totalandtaxextend']}</b></td>
