...
<select name="{$ASSIGNED_USER_ID}" style="display:none;">
	<option value="">请选择</option>
	{foreach item=D from=$ALL_BILLINGCONTENT}
	<option {if $FIELD_MODEL->get('fieldvalue') eq $D['billingcontent'] }selected{/if} value="{$D['billingcontent']}">{$D['billingcontent']}</option>
	{/foreach}
</select>
