...
{strip}
    ...
    &nbsp;<input onclick="Vtiger_List_Js.showUserFieldEdit('index.php?module={$MODULE}&view=FieldAjax');" class="btn"
             ...
             <span>
                 {if Users_Privileges_Model::isPermitted("Newinvoice", 'DuplicatesHandling')}
    &nbsp;          <button  class="btn btn-primary" id="BatchSignature"
                   type="button">签名</button>
                 {/if}
             </span>
        </span>
        {if $ISPAGE}
