...
<div class='container-fluid editViewContainer'>
	...
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<tr><td class="fieldLabel medium"><label class="muted pull-right marginRight10px">文件</label></td><td class="fieldValue medium" colspan="3"><div class="row-fluid"><span class="span10">
				<input type="text" disabled="disabled" size="20" name="upfile" id="upfile">&nbsp;&nbsp; &nbsp; 
				<button type="button" class="btn btn-info" onclick="path.click()">上传</button>
				<!-- <input type="button" class="btn btn-info" value="上传" onclick="path.click()">   -->
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						...
						<div class="row-fluid">
							<span class="span10">
								{if (($SOURCE_MODULE=='ServiceContracts'||$SOURCE_MODULE=='ContractsAgreement'||$SOURCE_MODULE=='SupplierContracts'||$SOURCE_MODULE=='SuppContractsAgreement')&&$FIELD_MODEL->get('fieldDataType')=='picklist'&&$FIELD_MODEL->get('name')=='style')&&$IS_HT_ADMIN==0}
									{include file="uitypes/UniqueDropDownBox.tpl"|@vtemplate_path:$MODULE}
								{elseif $SOURCE_MODULE eq 'Vendors'}
									{include file="uitypes/VendorsPickList.tpl"|@vtemplate_path:$MODULE}
								{else}
