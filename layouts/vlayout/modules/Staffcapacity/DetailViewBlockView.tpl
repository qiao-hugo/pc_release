...
{strip}
	...
	{foreach key=KEY item=WEEKDATE from=$DATE_ARR}
		<table {if $WEEKDATE.show neq '1'}style="display:none;"{/if} class="{$KEY} weektable fallintotable1 table table-bordered blockContainer showInlineTable  detailview-table Duplicates">
	        <thead>
	        <tr>
	            <th class="blockHeader" colspan="4" >
	                ...
	                合同分成信息 <span class="label label-success">{vtranslate({$KEY},{$MODULE_NAME})}</span>
	            </th>
	        </tr>
	        </thead>
	        <tbody >
