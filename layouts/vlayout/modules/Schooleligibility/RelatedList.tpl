            ...
            <thead>
                <tr class="listViewHeaders">
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                        {if $HEADER_FIELD neq 't_schoolresumeid'}
                ...
                <tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' data-recordUrl='{$RELATED_RECORD->getDetailViewUrl()}'>
                    {foreach item=HEADER_FIELD key=FIELDNAME from=$RELATED_HEADERS}
                        {assign var=RELATED_HEADERNAME value=$FIELDNAME}
                            {if $RELATED_HEADERNAME eq 't_schoolresumeid'}
                                {assign var=t_schoolresumeid value=$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}
                            {else}
					...
					<td>
                        <div class="pull-right actions">
                            <span class="actionImages">
                                <a target="_blank" href="index.php?module=Schoolresume&view=Detail&record={$t_schoolresumeid}"><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="icon-th-list alignMiddle"></i></a>&nbsp;

                                <a target="_blank" href="{$RELATED_RECORD->getEditViewUrl()}"><i title="{vtranslate('编辑', $MODULE)}" class="icon-pencil alignMiddle"></i></a>&nbsp;
							</span>
                        </div>
                    </td>
                </tr>
