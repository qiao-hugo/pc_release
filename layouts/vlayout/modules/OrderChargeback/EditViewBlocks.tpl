...
<div class='editViewContainer container-fluid'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{/if}
			...
			<i style="color: #ff0000;font-size:20px;font-style: normal;font-weight:bold;">提退款单前，与服务合同相关的业务要提前完成，退款单完成后，合同将关闭不能进行任务操作;</i>
			</span>
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<table class="table table-bordered blockContainer showInlineTable {if $BLOCK_LABEL eq 'LBL_ADV'} tableadv{else if $BLOCK_LABEL eq 'LBL_CUSTOM_INFORMATION'}hide tablecust{/if} detailview-table">
			<thead>
			<tr>
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{if $FIELD_MODEL->get('uitype') neq "83"}
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19'}  {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->get('uitype') eq '20'} colspan="3"{/if}>
