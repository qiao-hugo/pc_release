...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		<span class="pull-left">
			<i style="color: #d14;font-size:20px;">提醒:外出类型为出差的,需要多次签到</i>
		</span>
		...
		{/foreach}
	<input id="VisitingOrder_editView_fieldName_destinationcode" type="hidden" class="input-large "  name="destinationcode" value="{$RECORD->get('destinationcode')}" >
	<div id="qqmap" style="height:400px;text-align:center;">
		<img id="mapPage" width="100%" height="100%" frameborder="0" src="">
	</div>
	...
	<script>
        window.addEventListener('message', function(event) {
            ...
            if (loc && loc.module == 'locationPicker') {
                $("#VisitingOrder_editView_fieldName_destination").val(loc.poiaddress+loc.poiname);
                $("#VisitingOrder_editView_fieldName_destinationcode").val(loc.latlng.lng+"***"+loc.latlng.lat);
            }
        }, false);
        $(function () {
			var destinationcode = $("#VisitingOrder_editView_fieldName_destinationcode").val();
			if(destinationcode){
			    latlngarr = destinationcode.split("***");
			    latlng = latlngarr[1]+','+latlngarr[0];
            }else{
			    latlng='39.8802147,116.415794';
            }
			var src = "https://apis.map.qq.com/ws/staticmap/v2/?center="+latlng+"&zoom=14&size=1680*300&maptype=roadmap&markers=size:large|color:red|label:k|"+latlng+"&key=YQSBZ-DN7WP-NWGDE-L7OWN-4ZYU2-GCBJU&labels=border:1|size:20|color:0xff0000|bgcolor:white|anchor:3|offset:0_-18|定位地点|"+latlng;
