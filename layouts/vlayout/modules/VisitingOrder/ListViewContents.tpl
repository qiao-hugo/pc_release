            ...
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue"  nowrap>
                    ...
                    {elseif $LISTVIEW_HEADER['columnname'] eq 'related_to'}
						<a class="btn-link" href=index.php?module={$LISTVIEW_ENTRY['modulename']}&view=Detail&record={$LISTVIEW_ENTRY['related_to_reference']} target=_blank>{$LISTVIEW_ENTRY['accountnamer']}</a>
                    {else}
            ...
            <td class="listViewEntryValue" >
                {if $LISTVIEW_HEADER@last}
                    ...
                    <div  style="width:90px">
                        ...
                        {/if}
						{if  $LISTVIEW_ENTRY['isstrangevisit'] eq 1}
							<i title="设为陌拜" class="icon-plane alignMiddle setStrangeVisitButton" data-id="{$LISTVIEW_ENTRY['id']}" data-flag="{$LISTVIEW_ENTRY['isstrangevisit']}"></i>
						{elseif $LISTVIEW_ENTRY['isstrangevisit'] eq 2}
							<i title="取消陌拜" class="icon-repeat alignMiddle setStrangeVisitButton" data-id="{$LISTVIEW_ENTRY['id']}" data-flag="{$LISTVIEW_ENTRY['isstrangevisit']}"></i>
						{/if}
					</div>
                {/if}
            </td>
