...
{strip}
    ...
    <input type="hidden" name="refillapplicationid" id="refillapplicationid"/>
    ...
    {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
        ...
        <table class="table table-bordered blockContainer showInlineTable">
            <tr>
                <th class="blockHeader" colspan="4">发票签收申请</th>
            </tr>
            <tr>
                ...
                <td class="fieldLabel {$WIDTHTYPE}">
                    {if $isReferenceField neq "reference"}<label class="muted pull-right marginRight10px">{/if}
                        ...
                        {else}
                           {if $FIELD_MODEL->getName()=='file' && $BILLPROPERTY=='BeforeHandSub'}<span class="redColor">*</span> {/if}{vtranslate($FIELD_MODEL->get('label'), $MODULE)}
                        {/if}
