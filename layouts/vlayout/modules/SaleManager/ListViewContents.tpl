...
<div class="listViewEntriesDiv contents-bottomscroll" style="overflow:auto;">
	...
	<table class="table listViewEntriesTable">
		...
		{if $LISTVIEW_FIELDS}
			{foreach key=fkey item=NOLISTVIEW_HEADER from=$LISTVIEW_FIELDS name=fieldview}
			<td class="listViewEntryValue"  nowrap>

                    {if $smarty.foreach.fieldview.index eq 0}
                        <a href='/index.php?module=Users&view=PreferenceDetail&record={$LISTVIEW_ENTRY['sid']}' target="_block">{$LISTVIEW_ENTRY[$LISTVIEW_HEADERS[$fkey]['columnname']]}</a>
                        {else}
                        {if $LISTVIEW_HEADERS[$fkey]['uitype'] eq 15 || $LISTVIEW_HEADERS[$fkey]['uitype'] eq 151 }
                            {vtranslate($LISTVIEW_ENTRY[$LISTVIEW_HEADERS[$fkey]['columnname']], $MODULE)}
                        {else}
                            {if $LISTVIEW_HEADERS[$fkey]['uitype'] eq 152}
                                {str_replace('#','',$LISTVIEW_ENTRY[$LISTVIEW_HEADERS[$fkey]['columnname']])}
                            {else}
                                {if $LISTVIEW_HEADERS[$fkey]['uitype'] eq 19 || $LISTVIEW_HEADERS[$fkey]['uitype'] eq 21}
                                    {mb_substr($LISTVIEW_ENTRY[$LISTVIEW_HEADERS[$fkey]['columnname']],0,20,'utf-8')}
                                {else}
                                    {$LISTVIEW_ENTRY[$LISTVIEW_HEADERS[$fkey]['columnname']]}
                                {/if}
                            {/if}
                        {/if}
                    {/if}
			...
			{else}
				{foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                    ...
                    {if $smarty.foreach.fieldview.index eq 0}
                        ...
                        <a href='index.php?module=Users&view=PreferenceDetail&record={$LISTVIEW_ENTRY['sid']}' target="view_window">{$LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']]}</a>
                        {else}
                        {if $LISTVIEW_HEADER['uitype'] eq 15 || $LISTVIEW_HEADER['uitype'] eq 151 }
                            {vtranslate($LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']], $MODULE)}
                        {else}
                            {if $LISTVIEW_HEADER['uitype'] eq 152}
                                {str_replace('#','',$LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']])}
                            {else}
                                {if $LISTVIEW_HEADER['uitype'] eq 19 || $LISTVIEW_HEADER['uitype'] eq 21}
                                    {mb_substr($LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']],0,20,'utf-8')}
                                {else}
