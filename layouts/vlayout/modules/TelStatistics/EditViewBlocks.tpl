...
<div class='container-fluid editViewContainer'>
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		<span class="pull-right">
				<button class="btn btn-info" type="reset" onclick="location.href='./index.php?module=TelStatistics&view=Import'"><strong>导入</strong></button>
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			<tr>
				...
				{if in_array($FIELD_MODEL->get('name'),array('total_telnumber','telduration','telnumber','useid','usercode'))}
					{continue}
				{/if}
					...
					<td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
						...
						<div class="row-fluid">
							<span class="span10">
								...
								{if $FIELD_MODEL->get('name') eq 'departmentid'}
									<button class="btn btn-small getUserData" style="background-color: #0B92D9" type="button" data-type="in" autocomplete="off"><span style="color:white;">搜索</span></button>
								{/if}
							</div>
						{/if}
					</td>
		...
		<br>
	{/foreach}
	<table class="table table-bordered blockContainer datatable"><thead><tr><th class="blockHeader" colspan="17"><b class="pull-right"><button class="btn btn-small batchSave" style="background-color: #0B92D9" type="button" data-type="in" autocomplete="off"><span style="color:white;">一键保存</span></button></b></th></tr></thead><tbody></tbody></table>
{/strip}