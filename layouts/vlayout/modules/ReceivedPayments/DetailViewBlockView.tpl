...
{strip}
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 {if $FIELD_MODEL->getName() eq relatetoid}
					<div  class = "fieldLabel"style="width: 400px;">
						{$STR}
					</div> 
					{/if}
	...
	<div class="widgetContainer_receivehistory" data-url="module=ReceivedPayments&view=Detail&mode=getReceivedPaymentsUseDetail&amp;record={$RECORD->getId()}" data-name="receivehistory">
		<div class="widget_contents"></div>
	</div>
	<div class="widgetContainer_receivehistory" data-url="module=ReceivedPayments&view=Detail&mode=getdetailinfo&amp;record={$RECORD->getId()}" data-name="receivehistory">
		<div class="widget_contents"></div>
	</div>
	...
	<table class="table table-bordered blockContainer showInlineTable" id="achievementallottable">
			<tr>
			<thead><th class="blockHeader" colspan="4"><span>分成明细</span></th></thead>
			</tr>
				<tr>
					<td style="text-align:center">所属公司</td>
					<td >业绩所属人</td>
					<td>比例</td>
					<td>分成金额</td>
					</tr>
				{foreach key=BLOCK_LABEL  item=BLOCK_FIELDS from=$ACHIEVEMENTALLOTDATA name="EditViewBlockLevelLoop"}
					<tr>
					<td  style="text-align:center">{$BLOCK_FIELDS['owncompanys']}</td>
					<td> {$BLOCK_FIELDS['receivedpaymentownid']}</td>
					<td>{$BLOCK_FIELDS['scalling']}%</td>
					<td>{$BLOCK_FIELDS['businessunit']}</td>
					</tr>	
				{/foreach}
	</table>
	{/if}
    {if $EXTRA_DATA neq ""}
        <table class="table table-bordered table-striped blockContainer showInlineTable" id = "">
            ...
            <tbody id="extra_body">
            {foreach item=extra_item  from=$EXTRA_DATA}
            <tr>
                <td>{$extra_item['extra_type']}</td>
                <td>{$extra_item['extra_price']}</td>
                <td>{$extra_item['extra_remark']}</td>
            </tr>
            {/foreach}
            </tbody>
        </table>
