...
{strip}
	...
	{/foreach}
	<table  class="table table-bordered blockContainer showInlineTable" style="margin-top: -20px;">
		...
		{if count($SPECIAL_OPERATORS)>0}
			{foreach from=$SPECIAL_OPERATORS item=SPECIAL_OPERATOR}
				<tr>
					<td>IF</td>
					<td>
						{vtranslate($SPECIAL_OPERATOR['basics_column'],$MODULE_NAME)}
					</td>
					<td>
						{$OPERATIONS[$SPECIAL_OPERATOR['basics_operator']]}
					</td>
					<td colspan="2">
						{$SPECIAL_OPERATOR['basics_value']}
					</td>
					<td>THEN</td>
					<td>
						{vtranslate($SPECIAL_OPERATOR['operate_column'],$MODULE_NAME)}
					</td>
					<td>
						{$SPECIAL_OPERATOR['operate_operator']}
					</td>
					<td colspan="4">
						{$SPECIAL_OPERATOR['operate_value']}
					</td>
				</tr>
			{/foreach}
		{/if}
