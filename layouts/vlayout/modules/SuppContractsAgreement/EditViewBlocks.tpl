			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				<td class="fieldLabel {$WIDTHTYPE}">
					{if $isReferenceField neq "reference"}<label class="muted pull-right marginRight10px">{/if}
						...
						{if $FIELD_MODEL->getName() eq 'receiptorid' }<span class="redColor" id="receiptoridMust" style="display: none">*</span>{/if}
						{if $isReferenceField eq "reference"}
							...
							{else}
								<label class="muted pull-right marginRight10px">
									{if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}
									{if $FIELD_MODEL->getName() eq 'vendorid' }<span class="redColor" id="vendoridMust" style="display: none">*</span>{/if}
