							...
							<ul class="dropdown-menu"></ul>
								...
								{*{foreach item="LISTVIEW_MASSACTION" from=$LISTVIEW_MASSACTIONS name=actionCount}
									...
									{if $smarty.foreach.actionCount.last eq true}
										<li class="divider"></li>
									{/if}
								{/foreach}<li id="{$MODULE}_listView_advancedAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_ADVANCEDACTIONS->getLabel())}"><a {if stripos($LISTVIEW_ADVANCEDACTIONS->getUrl(), 'javascript:')===0} href="javascript:void(0);" onclick='{$LISTVIEW_ADVANCEDA
								CTIONS->getUrl()|substr:strlen("javascript:")};'{else} href='{$LISTVIEW_ADVANCE
								DACTIONS->getUrl()}' {/if}>{vtranslate($LISTVIEW_ADVANCEDACTIONS->getLabel(), $MODULE)}</a></li>*}
								
								{if $LISTVIEW_LINKS['LISTVIEW']|@count gt 0}
									{foreach item=LISTVIEW_ADVANCEDACTIONS from=$LISTVIEW_LINKS['LISTVIEW']}
										{if $LISTVIEW_ADVANCEDACTIONS->getLabel() neq 'LBL_IMPORT'}
										<!--2015-1-8 wangbin-->
										{*<button onclick='{$LISTVIEW_ADVANCEDACTIONS->getUrl()}' class="btn" id="{$MODULE}_listView_advancedAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_ADVANCEDACTIONS->getLabel())}">{vtranslate($LISTVIEW_ADVANCEDACTIONS->getLabel(), $MODULE)}</button>*}
										{/if}
									{/foreach}
								{/if}
						{/if}
