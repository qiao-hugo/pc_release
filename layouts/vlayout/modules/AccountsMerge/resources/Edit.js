...
jQuery.Class("Vtiger_Edit_Js",{
	...
	setMultiReferenceFieldValue : function(container, params) {
	...
	registerClearReferenceSelectionEvent : function(container) {
		...
		container.find('.clearDate').on('click',function(e){
		...
		editViewForm.submit(function(e){
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
					//保存前验证提交内容
                    var related_to = $("input[name='related_to']").val();
					var accountid  = $("input[name='accountid']").val();
					if(related_to == accountid){
                        Vtiger_Helper_Js.showMessage({type: 'error', text: "同一个客户不可以合并!"});
                        return false;
					}
					if(related_to<=0 || accountid<=0){
                        Vtiger_Helper_Js.showMessage({type: 'error', text: "不正确的操作或非法操作!"});
                        return false;
					}
                    var params={};
                    params.data={
                        "module": "AccountsMerge",
                        "action": "BasicAjax",
                        "mode": "getOriginalAccountCheck",
                    ...
                    AppConnector.request(params).then(
                        ...
                        function(error){
                            Vtiger_Helper_Js.showMessage({type: 'error', text:'网络异常'});
                            isSuccess=true;
                        }
                    );
                    // 如果失败 则 阻止提交 isSuccess = true 是失败 默认失败
                    if(isSuccess){
                        return false;
					}
                    //Once the form is submiting add data attribute to that form element
                    //一次提交后给添加 attribute 属性 用来阻止重复提交。
                    editViewForm.data('submit', 'true');
                    //on submit form trigger the recordPreSave event
                    var recordPreSaveEvent = jQuery.Event(Vtiger_Edit_Js.recordPreSave);
                    editViewForm.trigger(recordPreSaveEvent, {'value' : 'edit'});
                    if(recordPreSaveEvent.isDefaultPrevented()) {
                        //判断是否为重复的id如果为真就会，阻止提交，不过好像没有生效。wangbin
					...
					//避免错误信息的固定导航栏下隐藏
					e.preventDefault();
					//app.formAlignmentAfterValidation(editViewForm); //调用自定义阻止提交事件回报错
				}
			}
		});
	},
			...
			afterUpload : function(data) {
                ...
                $('.filedelete').remove();
                 ...
                 //K('#file').val(data.result['name']);
                    ...
                    Vtiger_Helper_Js.showPnotify({text :'注意:文件名不能为空,不能有空格',title :'上传失败'});
                 }
			},
    ...
    accountsMergeClick:function(){
		$("#AccountsMerge_editView_fieldName_remark").attr("readonly","readonly");
		var remark = $("#AccountsMerge_editView_fieldName_remark").val();
        var splicingStr='';
		$("input[name='contacts'],input[name='consolidatedcalllist']").click(function () {
			var first = '';
			var two   = '';
            ...
            if($("input[name='consolidatedcalllist']").is(":checked")){
                two='拜访单';
			}
            var str = getAfterSplicingStr(remark,first,two);
            $("#AccountsMerge_editView_fieldName_remark").val(str);
        });
        function getAfterSplicingStr(defaults,first,two){
            if(first){
                defaults=defaults+first;
            }
            if(two && first){
                defaults=defaults+","+two;
            }else{
                defaults=defaults+two;
            }
            return defaults;
	...
	registerEvents: function(){
		...
		if(window.location.href.indexOf("VisitingOrder") == -1 && window.location.href.indexOf("Vacate") == -1 ) {
		...
		$('.active-result').each(function(i,index){if($(this).text().indexOf('][离职]')>1 && !$(this).hasClass('result-selected')){$(this).hide()}});
		
		$('.hasinactive').change(function(){
			var ooptionli=$(this).prev('.chzn-container').find('li');
			var selectvalue=$(this).val();
			if(selectvalue=='inactive'){
				ooptionli.each(function(i,index){if($(this).text().indexOf('][离职]')<1 && !$(this).hasClass('result-selected')){$(this).hide()}else{$(this).show()}});
			}else if(selectvalue=='active'){
				ooptionli.each(function(i,index){if($(this).text().indexOf('][离职]')>1 && !$(this).hasClass('result-selected')){$(this).hide()}else{$(this).show()}});
			}else{
				ooptionli.each(function(i,index){$(this).show()});
			}
			
		
		});
	//this.triggerDisplayTypeEvent();
	this.loadeditWidgets();
	    this.accountsMergeClick();
	}
});