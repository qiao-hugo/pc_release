                ...
                {if $ISROLE and $STAGERECORDID}
                   ...
                   -->*}
                    ...
                    {if !empty($DATA)}
                        ...
                        <input name="nowWorkFlowFlag" id="nowWorkFlowFlag" type="hidden" disabled  value="{$workflowstagesflag}"/>
                    {/if}
                    &nbsp;{if !in_array($workflowstagesflag,array('CFO','DEPUTY'))}
                        <span style="color: red">请合同管理员确认已收到代付款证明原件且​原件与代付款信息及上传的附件一致时，点击确认  &nbsp;&nbsp;&nbsp;</span>
                        <div class="btn-group">
                            <button type="button" class="btn stagesubmit">确认</button>
                        </div>
                      {else}
                        <div class="btn-group">
        ...
        <table class="table table-bordered equalSplit detailview-table ">
            ...
            {if ($ISROLE and $STAGERECORDID) || in_array($USER,$USERDEPARTMENT)}
           	<tr class="hide realremarkbutton" nowrap>
            	<td colspan="2"><textarea class="row-fluid remark" id="remarkvalue" required="required" placeholder="请输入备注信息" name="description" ></textarea></td>
                <td colspan="2">
                    <div class="fileUploadContainer" style="float: right" xmlns="http://www.w3.org/1999/html">
                        <div class="upload">
			...
			<tr>
                ...
                <th class="detailViewBlockHeader" nowrap>补充说明文件</th>
            ...
            {foreach key=index item=value from=$REMARKLIST}
                <tr>
                    ...
                    <td nowrap ><a href="index.php?module=Staypayment&action=DownloadFile&type=1&filename={base64_encode($value["attachmentsid"])}">{$value["name"]}</i></a></td>
                    <td nowrap >{$value["rejecttime"]}</td>
