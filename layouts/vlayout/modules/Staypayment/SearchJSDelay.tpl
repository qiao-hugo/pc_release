...
{strip}
    ...
    <script type="text/javascript">
        ...
        {if in_array($FIELD_NAME,array('contractid','staymentcode','accountid','payer','workflowstime','modulestatus','last_sign_time','isdelay'))}
            ...
            {if  $FIELD_MODEL->getFieldDataType() eq 'picklist' }
                {if $FIELD_NAME eq 'modulestatus'}
                    var field_{$FIELD_NAME|trim}_value = '<select id=\"BugFreeQuery_value999999\" style=\"width:100%\" class=\"chzn-select\" name=\"BugFreeQuery[value999999]\"><option value=\"\">选择一个选项</option><option value="c_complete">已签收</option><option value="n_complete">未签收</option></select>';
                {else}
        ...
        {/if}
        {/foreach}
        var field_isauto_type='boolean';
        var field_isauto_value = '<select id=\"BugFreeQuery_value999999\" style=\"width:100%;\" class=\"chzn-select\" name=\"BugFreeQuery[value999999]\"><option value=\"\">选择一个选项</option><option value=\"0\">否</option><option value=\"1\">是</option></select>';
        var field_changetime_type='date';
        var field_changetime_value = '<input type=\"text\"  size=\"16\" value=\"\" id=\"BugFreeQuery_value999999\" name=\"BugFreeQuery[value999999]\" autocomplete=\"off\">';
        ...
        var searchConditionTmp = '<tr class=\"SearchConditionRow\" id=\"SearchConditionRow999999\"><td><select id=\"BugFreeQuery_leftParenthesesName999999\" onchange=\"validateParentheses()\" style=\"width:48px;\" name=\"BugFreeQuery[leftParenthesesName999999]\"><option value=\"\" selected=\"selected\"></option><option value=\"(\">(</option></select></td><td><select id=\"BugFreeQuery_field999999\" onchange=\"updateQueryRow(999999);\" name=\"BugFreeQuery[field999999]\">
                {foreach key=FIELD_NAME item=FIELD_MODEL from=$SEARCHRECORD_STRUCTURE}{if in_array($FIELD_NAME,array('contractid','staymentcode','accountid','payer','workflowstime','modulestatus','last_sign_time','isdelay'))}<option value=\"{$FIELD_MODEL->get('column')}##{$FIELD_MODEL->get('uitype')}##{$FIELD_MODEL->get('id')}##{$FIELD_MODEL->getFieldDataType()}\" data=\"{$FIELD_NAME}\">
                {if $FIELD_MODEL->get('label') eq 'WorkflowsTime'}
                    代付款签收时间
                {elseif $FIELD_MODEL->get('label') eq 'modulestatus'}
                    代付款状态
                {else}
    ...
    <script type="text/javascript">
        ...
        function updateQueryValue(index,isKeepOldValue)
        {
            ...
            if(fieldType == 'datetime'||fieldType == 'date'){
                //console.log(1111);
                $('#'+searchParamsPreFix+'_value'+index).replaceWith('<input class="span9 dateField form_datetime" autocomplete="off" id="BugFreeQuery_value'+index+'" name="BugFreeQuery[value'+index+']" size="16" type="text" name="nowritetime" value=""  >');
