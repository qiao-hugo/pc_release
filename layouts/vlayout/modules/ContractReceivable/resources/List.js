...
Vtiger_List_Js("ContractReceivable_List_Js", {}, {
    ...
    exportReceivableOverdue:function(){
        $("body").on('click','#exportReceivableOverdue',function () {
            ...
            var urlParams = {"module":"ContractReceivable","action":"ChangeAjax","mode":"exportData","BugFreeQuery":form,'bussinesstype':bussinesstype};
            ...
            AppConnector.request(urlParams).then(
                function(data){
                    ...
                    if(data.success){
                        window.location.href='index.php?module=ContractReceivable&view=List&public=export&bussinesstype='+bussinesstype;
                    }else{
    ...
    submitconfim:function(){
        ...
        $('table').on('click','#PostQuery',function(){
            ...
            for(var i=0;i<formtable.length;i++){
                obj[formtable[i].name]=formtable[i].value;
            }
            var postData= {
                'module': app.getModuleName(),
                'action': 'ChangeAjax',
                'mode': 'getReceivableOverdueData',
                'department':$('#departmentids').val()
            };
            postData=$.extend(postData,obj);
            //$('#detailtable').empty();
            var progressIndicatorElement = jQuery.progressIndicator({
                'message' : '正在处理,请耐心等待哟',
                'position' : 'html',
            ...
            AppConnector.request(postData).then(
                function(data){
                    ...
                    $("#bartable").html(data.result);

                }
            )
        });
    },
    earlyWarningSetting:function(){
        var thisInstance=this;
        $('table').on('click','#PostData',function(){
            var formtable=$('#formtable').serializeArray();
            var obj={};
            for(var i=0;i<formtable.length;i++){
                obj[formtable[i].name]=formtable[i].value;
            ...
            var postData= {
                'module': app.getModuleName(),
                'action': 'ChangeAjax',
                'mode': 'earlyWarningSavedata',
                'rbexp':$('#alertchannels').val(),
                'Overduewarning':$('#alertchannels1').val()
            };
            postData=$.extend(postData,obj);
            var progressIndicatorElement = jQuery.progressIndicator({
                'message' : '正在处理,请耐心等待哟',
    ...
    collate : function() { //核对
        $('body').on("click", '.collate', function() { //单个核对
            var contractid = $(this).parents('tr').data('contractid');
            ...
            var html = '<table class="table" style="border-left:none;border-bottom:none;margin-top:20px;margin-bottom:0"><tbody>'+
                ...
                '<tr><td class="fieldLabel medium"><label class="muted pull-right marginRight10px"><span class="redColor" style="display: none;" id="remarkstar">*</span>备注:</label></td><td class="fieldValue medium" colspan="3"><div class="row-fluid"><span class="span10"><textarea id="remark" style="overflow-wrap:break-word;resize:none; height:100px;width:320px;"></textarea></span></div></td></tr>'+
                '</tbody></table>';
