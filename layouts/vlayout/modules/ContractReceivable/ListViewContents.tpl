...
{strip}
    ...
    <div id="pagehtml">
        ...
        <input type="hidden" id="bussinesstype" value="{$BUSSINESSTYPE}" />
            ...
            <div class="bottomscroll-div" >
                ...
                <table class="table listViewEntriesTable">
                    ...
                    {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
                        <tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY['id']}' data-mode='{$LISTVIEW_ENTRY["modulename"]}' data-recordUrl='index.php?module={$MODULE}&view=Detail&record={$LISTVIEW_ENTRY['id']}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" data-contractid="{$LISTVIEW_ENTRY['contractid_reference']}">
                            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                                <td class="listViewEntryValue"  nowrap>
                                    ...
                                    {elseif $LISTVIEW_HEADER['columnname'] eq 'collectionstatus' && uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE)=='有逾期'}
                                        <span style="color: red">{vtranslate(uitypeformat($LISTVIEW_HEADER,$LISTVIEW_ENTRY,$MODULE),$MODULE)}</span>
