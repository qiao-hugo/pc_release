            ...
            {foreach key=fkey item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS name=fieldview}
                <td class="listViewEntryValue"  nowrap>
                    {if $LISTVIEW_HEADER['columnname'] eq 'schoolrecruitid'}
                         <a class="btn-link" href='index.php?module=Schoolrecruit&view=Detail&record={$LISTVIEW_ENTRY['schoolrecruitid_reference']}' target="_blank">{vtranslate( $LISTVIEW_ENTRY[$LISTVIEW_HEADER['columnname']],$MODULE)}</a>
