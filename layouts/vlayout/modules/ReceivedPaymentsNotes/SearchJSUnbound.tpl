...
{strip}
    ...
    <script type="text/javascript">
        ...
        {foreach key=FIELD_NAME item=FIELD_MODEL from=$SEARCHRECORD_STRUCTURE_UNBOUND}
            var field_{$FIELD_NAME|trim}_type='{$FIELD_MODEL['fieldtype']}';
            {if  $FIELD_MODEL['fieldtype'] eq 'picklist' }
                var field_{$FIELD_NAME|trim}_value = '<select id=\"BugFreeQuery_value999999\" style=\"width:100%\" class=\"chzn-select\" name=\"BugFreeQuery[value999999]\"><option value=\"\">选择一个选项</option>{foreach key=PICKLIST item=pick_list from=$FIELD_MODEL['picklistvalues']}<option value=\"{$PICKLIST}\">{$pick_list}</option>{/foreach}</select>';
            {elseif $FIELD_MODEL['fieldtype'] eq 'owner'}
                {assign var=USERSLIST value=$FIELD_MODEL['picklistvalues']['userslist']}
            ...
            {elseif $FIELD_MODEL['fieldtype'] eq 'boolean'}
                var field_{$FIELD_NAME|trim}_value = '<select id=\"BugFreeQuery_value999999\" style=\"width:100%;\" class=\"chzn-select\" name=\"BugFreeQuery[value999999]\"><option value=\"\">选择一个选项</option><option value=\"否\">否</option><option value=\"是\">是</option></select>';
            {elseif $FIELD_MODEL['fieldtype'] eq 'currency'}
                var field_{$FIELD_NAME|trim}_value = '<input type=\"number\" style="margin-bottom: 0"  size=\"16\" value=\"\" id=\"BugFreeQuery_value999999\" name=\"BugFreeQuery[value999999]\" autocomplete=\"off\">';
            {else}
    ...
    <script type="text/javascript">
        ...
        function updateQueryValue(index,isKeepOldValue)
        {
            ...
            //if(fieldType == 'owner'||fieldType == 'picklist'){ //2015-05-06 young 判断下拉
               ...
               // $('.chzn-container').css({'margin-top':'-6px'});
            //}
            if(fieldType == 'datetime'||fieldType == 'date'||fieldType == 'dateequal'){
                //console.log(1111);
                $('#'+searchParamsPreFix+'_value'+index).replaceWith('<input class="span9 dateField form_datetime" id="BugFreeQuery_value'+index+'" autocomplete="off" name="BugFreeQuery[value'+index+']" size="16" type="text" name="nowritetime" value=""  >');
                if(fieldType == 'dateequal'){
                    $(".form_datetime").datepicker({format: 'yyyy-mm',autoclose:true,language: 'zh-CN',minView:'month',startView:1,minViewMode:1});
                }else{
