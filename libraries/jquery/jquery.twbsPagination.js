...
(function ($, window, document, undefined) {
    ...
    TwbsPagination.prototype = {
        ...
        buildListItems: function (pages) {
            var $listItems = $();
			$listItems = $listItems.add('<li class=disabled><a href=#>总记录:<small id="jscount">'+this.options.total+'</small>  页码:'+pages.currentPage+'/'+this.options.totalPages+' </a></li>');
		...
		//	if (this.options.jump) {
          //      $listItems = $listItems.add(this.buildItem('jump', $('#pageToJump').val()));
          //  }
			//$listItems = $listItems.add('<input type="text" style="width:50px;" id="pageToJump" class="pageToJump" value="1">');
        ...
        buildItem: function (type, page) {
            ...
            switch (type) {
                ...
                case 'last':
                    ...
                    break;
				case 'jump':
                    itemText = this.options.jump;
                    itemContainer.addClass(this.options.jumpClass);
                    break;	
                default:
                    break;
            }
        ...
        setupEvents: function () {
            ...
            this.$listContainer.find('li').each(function () {
				...
				/*
				$('#pageToJump').keydown(function (evt) {
						if(evt.which == 13){
							// Prevent click event if href is not set.//!base.options.href && evt.preventDefault();
							base.options.startPage=$('#pageToJump').val();
							$('#pageNumber').val($('#pageToJump').val());
							//$this.data('page')=$('#pageToJump').val();
							base.show(parseInt($('#pageToJump').val(), 10));
						}
				});*/
