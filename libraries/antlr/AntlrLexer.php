...
abstract class AntlrLexer extends BaseRecognizer{
	...
	public function matchAny($input=array()) {
		$this->input->consume();
	}
	 ...
	 */
	public function recover($re,$re2=array()) {
		$this->input->consume();
	}
	
	
	public function traceIn($ruleName, $ruleIndex,$inputSymbol=array())  {
		$inputSymbol = $this->input->LT(1)." line=".$this->getLine().":".$this->getCharPositionInLine();
		parent::traceIn($ruleName, $ruleIndex, $inputSymbol);
	}

	public function traceOut($ruleName, $ruleIndex,$inputSymbol=array())  {
		$inputSymbol = $this->input->LT(1)." line=".$this->getLine().":".$this->getCharPositionInLine();
