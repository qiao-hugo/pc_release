...
class RefundTimeoutAudit extends baseapp {
    public function one() {
        $id = $_REQUEST['id'];
        if (!empty($id)) {
            $params = array(
                'fieldname' => array(
                    'id' => $id,
                    'module' => 'RefundTimeoutAudit',
                    'record' => $id,
                ),
                'userid' => $this->userid
            );
            $result = $this->call('oneRefundTimeoutAudit', $params);
            $arr = array();
            ...
            $this->smarty->display('RefundTimeoutAudit/one.html');
        }
    }
