...
function check_login($email){
	...
	$params = array('email' => $email,"sessionid"=>$clientsessionid,'version' => "$version");
...
//		header("Location: http://m.crm.71360.com/index.php?module=VisitingOrder&action=vlist");
		global $apps_login_access_index_url;
		if(!empty($_REQUEST['backurl'])){
			$url=$apps_login_access_index_url.base64_decode($_REQUEST['backurl']);
		}else{
			$url=$apps_login_access_index_url."/index.php?module=VisitingOrder&action=vlist";
		}
		header("Location:".$url);
	}
}
