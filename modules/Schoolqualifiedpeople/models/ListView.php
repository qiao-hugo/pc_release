...
class Schoolqualifiedpeople_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$moduleName ='Schoolqualifiedpeople';
		...
		if(empty($orderBy) && empty($sortOrder)){
			...
			$orderBy = 'schoolqualifiedpeopleid';
		...
		// 这里过滤一下 is_train 这个变量
		$listQuery = str_replace('IF(is_train=1', 'IF(vtiger_schoolqualifiedpeople.is_train=1', $listQuery);
        $listQuery = preg_replace('/SELECT/', 'SELECT vtiger_schoolqualifiedpeople.schoolqualifiedid AS schoolqualifiedid, ', $listQuery, 1);
        //echo $listQuery;die();
		//echo $listQuery;die;
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolqualifiedpeopleid'];
			$listViewRecordModels[$rawData['schoolqualifiedpeopleid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('Schoolqualified','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' AND ( vtiger_schoolqualifiedpeople.p_reportsower in ('.implode(',',$where).') OR  vtiger_crmentity.smownerid in ('.implode(',',$where).') )';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' AND ( vtiger_schoolqualifiedpeople.p_reportsower '.$where . '  OR vtiger_crmentity.smownerid '.$where . ' ) ';
            }
        }
