...
class Potentials extends CRMEntity {
	...
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_potential'=>'potentialid','vtiger_potentialscf'=>'potentialid','vtiger_potential_detail'=>'potentialid');
	/**
	...
	var $list_fields = Array(
			'Potential'=>Array('vtiger_potential'=>'potentialname'),
			'Organization Name'=>Array('vtiger_potential'=>'related_to'),
			'Contact Name'=>Array('vtiger_potential'=>'contact_id'),
			'Sales Stage'=>Array('vtiger_potential'=>'sales_stage'),
			'Amount'=>Array('vtiger_potential'=>'amount'),
			'Expected Close Date'=>Array('vtiger_potential'=>'closingdate'),
	...
	function save_module($module)
	{
        ...
        $recordid = $this->id;
        $tarray=array();
        if(isset($_POST['potentialnames']) && !empty($_POST['potentialnames'])){
            $related_to  = isset($_POST['related_to']) && !empty($_POST['related_to']) ? $_POST['related_to'] : 0;
            foreach($_POST['potentialnames'] as $key=>$value){
                //如果该条内容的id 和 pid 都存在表 是 更新
                if($_POST['id'][$key]>0 && $_POST['pid'][$key]>0){
                    $uptarray=array($related_to,$_POST['potentialnames'][$key],$_POST['probabilitys'][$key],$_POST['salesstages'][$key],$_POST['budgetinterval'][$key],$_POST['budgetlockstart'][$key],$_POST['budgetlockend'][$key],$_POST['isannuallypay'][$key],$_POST['dockingrole'][$key],$_POST['docker'][$key],$_POST['projectdetails'][$key],date("Y-m-d H:i:s"),$_POST['id'][$key],$_POST['pid'][$key]);
                    $sql="UPDATE  vtiger_potential_detail SET  related_to=?,potentialnames=?,probabilitys=?,salesstages=?,budgetinterval=?,budgetlockstart=?,budgetlockend=?,isannuallypay=?,dockingrole=?,docker=?,projectdetails=?,modifiedtime=? WHERE potentialdetailid=?  AND potentialid =? ";
                    $this->db->pquery($sql,$uptarray);
                    //不存在指的是编辑时新增 或者 新增时新增
                }else{
                    $tarray[]=$recordid;
                    $tarray[]=$related_to;
                    $tarray[]=$_POST['potentialnames'][$key];
                    $tarray[]=$_POST['probabilitys'][$key];
                    $tarray[]=$_POST['salesstages'][$key];
                    $tarray[]=$_POST['budgetinterval'][$key];
                    $tarray[]=$_POST['budgetlockstart'][$key];
                    $tarray[]=$_POST['budgetlockend'][$key];
                    $tarray[]=$_POST['isannuallypay'][$key];
                    $tarray[]=$_POST['dockingrole'][$key];
                    $tarray[]=$_POST['docker'][$key];
                    $tarray[]=$_POST['projectdetails'][$key];
            ...
            $sql='INSERT INTO vtiger_potential_detail (potentialid,related_to,potentialnames,probabilitys,salesstages,budgetinterval,budgetlockstart,budgetlockend,isannuallypay,dockingrole,docker,projectdetails,createdtime,modifiedtime) VALUES '.$insertistr;
	...
	*/
    ...
    function retrieve_entity_info($record, $module){
		...
		//超过24小时不允许修改
		global $currentView;
		$where=getAccessibleUsers('Potentials','',true);
		if($where!='1=1'){
