...
class Potentials_Record_Model extends Vtiger_Record_Model {
    ...
    public function delPotentialDetailOne(Vtiger_Request $request){
        global $adb;
        $potentialdetailid=$request->get('potentialdetailid');
        if($potentialdetailid>0){
            $query=" UPDATE vtiger_potential_detail SET isdelete = 1 ,modifiedtime=? WHERE potentialdetailid= ? ";
            $result=$adb->pquery($query,array(date("Y-m-d H:i:s"),$potentialdetailid));
