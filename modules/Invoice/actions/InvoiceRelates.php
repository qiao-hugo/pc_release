...
class Invoice_InvoiceRelates_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
	    ...
	    $db=PearDatabase::getInstance();
            ...
            $sql = "SELECT
                        ...
                        IFNULL(vtiger_invoicerelatedreceive.invoiceid,'--') AS invoiceid,
                    ...
                    WHERE
                        vtiger_receivedpayments.relatetoid = {$recordId} ORDER BY invoice_no DESC,receivedid ASC";
        ...
        $query="SELECT
                    IFNULL(vtiger_invoice.billingtime,'--') AS billingtime,
                    IFNULL(
                        vtiger_servicecontracts.contract_no,
                        '--'
                    ) AS contract_no,
                    IFNULL(vtiger_invoice.invoice_no,'') as invoice_no,
                    IFNULL(vtiger_invoice.companyname,'') AS companyname,
                    vtiger_crmentity.createdtime,
                    IFNULL(vtiger_invoice.accountnumber,'') AS accountnumber,
                    crmid,
                ...
                LEFT JOIN vtiger_invoicerelatedreceive ON vtiger_invoicerelatedreceive.invoiceid = vtiger_invoice.invoiceid
                ...
                WHERE
                    (
                        ...
                        OR vtiger_invoice.contractid IS NULL
                    )
                AND vtiger_crmentity.smownerid = {$current_user->id}";

        $receivepaynolist = $db->run_query_allrecords($query);
        $return=array('accountname'=>$accountname[0]['accountname'],'id'=>$accountname[0]['accountid'],'billcontent'=>$accountname[0]['billingcontent']);
