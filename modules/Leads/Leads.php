...
class Leads extends CRMEntity {
	...
	*/
	function save_module($module)
	{
		// 更新分配时间
//		$leadid = $this->id;
//		$sql = "select * from vtiger_leaddetails where vtiger_leaddetails.assigner is not null AND allocatetime is null AND leadid=? LIMIT 1";
//		$sel_result = $this->db->pquery($sql, array($leadid));
//		$res_cnt = $this->db->num_rows($sel_result);
//		if($res_cnt > 0) {
//		    $sql = "update vtiger_leaddetails set allocatetime=? where leadid=? ";
//		    $this->db->pquery($sql, array(date("Y-m-d H:i:s"), $leadid));
//		}
	}
...
/* 	function getListButtons($app_strings) {
    ...
    function retrieve_entity_info($record, $module){
        ...
        global $currentView;
        $arr=array(/*'c_allocated',*/'c_transformation','c_cancelled','c_Related','c_Forced_Related');
        if(in_array($this->column_fields['assignerstatus'],$arr) && $currentView=='Edit'){
            throw new AppException('商机已处理不允许辑辑!');
            exit;
        }
	if($this->column_fields['leadcategroy']==2 && $currentView=='Edit'){
            throw new AppException('公海商机请先领取后再编辑!');
            exit;
        }
    }
}
