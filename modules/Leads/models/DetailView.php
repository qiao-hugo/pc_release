...
class Leads_DetailView_Model extends Vtiger_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
        ...
        $arr=array('c_transformation','c_cancelled','c_Related','c_Forced_Related');
		if(Users_Privileges_Model::isPermitted($moduleModel->getName(), 'ConvertLead', $recordModel->getId()) &&
            Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())&&
            !in_array($recordModel->entity->column_fields['assignerstatus'],$arr)) {
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
...
//        if(Users_Privileges_Model::isPermitted($moduleModel->getName(), 'ConvertLead', $recordModel->getId()) &&
//            Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())&&
//            $recordModel->entity->column_fields['assignerstatus']!='c_Forced_Related'&&
//            $recordModel->entity->column_fields['assignerstatus']!='c_cancelled') {
        if(Users_Privileges_Model::isPermitted($moduleModel->getName(), 'ConvertLead', $recordModel->getId()) &&
            Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())&&
            !in_array($recordModel->entity->column_fields['assignerstatus'],$arr)) {
            $basicActionLink = array(
        ...
        if(Users_Privileges_Model::isPermitted($moduleModel->getName(), 'ConvertLead', $recordModel->getId()) && Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())&&$recordModel->entity->column_fields['assignerstatus']!='c_Forced_Related'&&$recordModel->entity->column_fields['assignerstatus']=='c_cancelled' ||
            Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())&&$recordModel->entity->column_fields['assignerstatus']!='c_Forced_Related'&&$recordModel->entity->column_fields['assignerstatus']=='c_cancelled'
        ) {
            $basicActionLink = array(
                'linktype' => 'DETAILVIEWBASIC',
                'linklabel' => 'LBL_ACTIVATION_LEAD',
                'linkurl' => 'Javascript:Leads_Detail_Js.convertLead("'.$recordModel->getConvertLeadUrl().'",this);',
                'linkicon' => ''
            );
            $linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
