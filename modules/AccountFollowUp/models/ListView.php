...
class AccountFollowUp_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
        ...
        $listQuery.= " AND vtiger_modcomments.modulename='Accounts' AND vtiger_modcomments.related_to>0";
		...
		while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['modcommentsid'];
            $rawData['accountid'] = $rawData['related_to_reference'];
			$listViewRecordModels[$rawData['modcommentsid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('AccountFollowUp','List',true);
            if($where!='1=1'){
                $where=array_intersect($where,$userid);
                $where=empty($where)?array(-1):$where;
                $listQuery .= ' and vtiger_modcomments.creatorid in ('.implode(',',$where).')';
            }
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                $listQuery .= ' and vtiger_modcomments.creatorid'.$where;
            }
        }
        return $listQuery;
    }
    public function getListViewCount() {
        ...
        $listQuery.=' LIMIT 10000';//该表数据量太大显示那么多也没有用,还影响性能只显示1万条
        /*echo $listQuery;
        exit;*/
        $listResult = $db->pquery($listQuery, array());
