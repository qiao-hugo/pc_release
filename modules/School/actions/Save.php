...
class School_Save_Action extends Vtiger_Action_Controller {
	...
	public function saveRecord($request) {
        ...
        //招聘学校点编辑信息时不冲掉原来的学校地址 gaocl add 2018/03/08
		...
		$record = $request->get('record');
		if (empty($record)) {
			$contactsuser = $request->get('contactsuser');
			$position     = $request->get('position');
			$gendertype   = $request->get('gendertype');
			$phone        = $request->get('phone');
			$email        = $request->get('email');
			$data = array(
				'schoolcontactsname'=>$contactsuser,
				'position'=>$position,
				'gendertype'=>$gendertype,
				'phone'=>$phone,
				'email'=>$email,
				'schoolid'=>$recordModel->getId()
			);
			global $adb;
			$divideNames = array_keys($data);
			$divideValues = array_values($data);
			$adb->pquery('INSERT INTO `vtiger_schoolcontacts` ('. implode(',', $divideNames).') VALUES ('. generateQuestionMarks($divideValues) .')',$divideValues);
		}
	...
	public function getDetailViewUrl($id) {
		return 'index.php?module=School&view=Detail&record='.$id;
	}
