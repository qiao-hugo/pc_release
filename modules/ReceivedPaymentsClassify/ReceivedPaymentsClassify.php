...
class ReceivedPaymentsClassify extends CRMEntity {
    ...
    var $list_fields = Array (
        ...
        'Rel Id'=> Array('ReceivedPaymentsClassify', 'unit_price'),
        'Related to'=>Array('ReceivedPaymentsClassify','relatetoid'),

    	//wangbin 2015-1-14 合同回款列表增加字段
    	'description'=>Array('ReceivedPaymentsClassify','overdue'),
    	'RELATE DEPART'=>Array('ReceivedPaymentsClassify','createid'),
        //'Status'=>Array('receivedpayments','status'),
    	'Reality_date'=>Array('ReceivedPaymentsClassify','reality_date'),
    	'Modifieddate'=>Array('ReceivedPaymentsClassify','modifiedtime'),
    	'Mfr PartNo'=>Array('ReceivedPaymentsClassify','createtime'),
    	);
    ...
    var $search_fields = Array(
        /* Format: Field Label => Array(tablename, columnname) */
        // tablename should not have prefix 'vtiger_'
        'Rel Name'=> Array('ReceivedPaymentsClassify', 'relmodule'),
        'Rel Operate'=> Array('ReceivedPaymentsClassify', 'reloperate'),
        'Rel Id'=> Array('ReceivedPaymentsClassify', 'unit_price'),
        'Related to'=>Array('ReceivedPaymentsClassify','relatetoid')
    );
   ...
   function save_module($module) {
      ...
      if($serviceid>0){  //合同id存在
         $flag = 1;
         //20170303周海 匹配了回款合同 需要更新客户的垫款 减
         // 查到对应的客户id
         $sql = "select sc_related_to from vtiger_servicecontracts where servicecontractsid=? limit 1";
         $sel_result = $this->db->pquery($sql, array($serviceid));
         $row = $this->db->query_result_rowdata($sel_result, 0);
         //print_r($row);die;
         /*if (count($row) > 0) {
            Accounts_Record_Model::setAdvancesmoney($row['sc_related_to'], - $_REQUEST['unit_price'], '(回款编辑匹配合同)');
         }*/
      }
      ...
      /*$sql = "update vtiger_receivedpayments set ismatchdepart=? where receivedpaymentsid=?";
      $this->db->pquery($sql, array($flag, $ReceivedPaymentid));*/
   ...
   //end
    ...
    function vtlib_handler($modulename, $event_type) {
        ...
        if($event_type == 'module.postinstall') {
			$projectTaskResult = $adb->pquery('SELECT tabid FROM vtiger_tab WHERE name=?', array('ReceivedPaymentsClassify'));
