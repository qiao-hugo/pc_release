...
class ReceivedPaymentsClassify_Import_View extends Vtiger_Import_View {
...
//        return $viewer->view('ImportBasicStep.tpl', 'ReceivedPayments');
        return $viewer->view('NewImportBasicStep.tpl', 'ReceivedPaymentsClassify');
    }
    ...
    function uploadAndParse(Vtiger_Request $request) {
        if(Import_Utils_Helper::validateFileUpload($request)) {
            ...
            return $viewer->view('ImportAdvanced.tpl', 'ReceivedPaymentsClassify');
        } else {
            $this->importBasicStep($request);
        }
    }
