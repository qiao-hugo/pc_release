...
class ReceivedPaymentsClassify_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
        ...
        if ($is_collate) {
            ...
            $sql = "UPDATE vtiger_receivedpayments SET paymentchannel=?, paymentcode=?, owncompany=?, reality_date=?, modifiedtime=?, checkid=? WHERE receivedpaymentsid=?";
            $adb->pquery($sql,
                [
	...
	// 	   echo $request->get('isguarantee') ;die;
		   ...
		   //end
            ...
            if (!empty($receivepayid)) {
                $receivepayment_data = Vtiger_Record_Model::getInstanceById($receivepayid, 'ReceivedPaymentsClassify');
