...
class SuppContractsAgreement_BasicAjax_Action extends Vtiger_Action_Controller {
	...
	function getAccount(Vtiger_Request $request){
        ...
        do {
            ...
            $dataResult=array('flag'=>true,'type'=>$row['type'],'rebateList'=>$rebateList,'accountid'=>$row['vendorid'],'accountname'=>$row['vendorname'],'total'=>$row['total'],'contract_name'=>$row['contract_name'],'paymentclause'=>$row['paymentclause'],'bankaccount'=>$row['bankaccount'],'bankname'=>$row['bankname'],'bankcode'=>$row['bankcode'],'banknumber'=>$row['banknumber'],'banklist'=>$row['banklist']);
        }while(0);
