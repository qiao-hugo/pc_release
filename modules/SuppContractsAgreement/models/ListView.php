...
class SuppContractsAgreement_ListView_Model extends Vtiger_ListView_Model {


	//根据参数显示数据
	public function getListViewEntries($pagingModel,$searchField=null) {
		$db = PearDatabase::getInstance();
		$moduleName ='SuppContractsAgreement';
		...
		if(empty($orderBy) && empty($sortOrder)){
			...
			$orderBy = 'vtiger_suppcontractsagreement.contractsagreementid';
			$sortOrder = 'DESC';
		}
    ...
    public function getUserWhere(){
		...
		if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('SuppContractsAgreement','List',true);
            if($where!='1=1'){
            ...
            if($where!='1=1'){
                $listQuery .= ' and (vtiger_crmentity.smownerid '.$where.$invoicecompany=' OR '.getAccessibleCompany('vtiger_suppliercontracts.companycode','SupplierContracts').')';
            }
        }
