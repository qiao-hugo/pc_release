 ...
 */
class UserManger_Field_Model extends Vtiger_Field_Model {
	...
	public function getFieldDataType() {
		if($this->get('uitype') == 99){
			return 'password';
		}else if(in_array($this->get('uitype'), array(32, 115,130))) {
            return 'picklist';
        ...
        } else if($this->get('uitype') == 70) {
		    if($this->get('column') =='leavedate'){
                return 'Datetime';
            }
        }
	...
	public function getPicklistValues() {
		if($this->get('uitype') == 32) {
        ...
        else if($this->get('uitype') == '130'){
            $db = PearDatabase::getInstance();
            if($this->getFieldName()=='departmentid'){
                $query = "SELECT departmentid AS 'keys',departmentname AS 'values' FROM vtiger_departments";
            }else if($this->getFieldName()=='roleid'){
                $query = "SELECT roleid AS 'keys',rolename AS 'values' FROM vtiger_role WHERE  roleid<>'H1'";
            }else{
                return array();
            }
            $result = $db->pquery($query, array());
            ...
            $fieldPickListValues = array();
            for($i=0; $i<$num_rows; $i++) {
                $keys = $db->query_result($result,$i,'keys');
                $values = $db->query_result($result,$i,'values');
                $fieldPickListValues[$keys] =$values;
            }
            return $fieldPickListValues;
        }else if($this->get('uitype') == '98'){
            $db = PearDatabase::getInstance();
           if($this->getFieldName()=='roleid'){
                $query = "SELECT roleid AS 'keys',rolename AS 'values' FROM vtiger_role WHERE  roleid<>'H1'";
            }else{
                return array();
            }
	 ...
	 */
     public function getAllRoles($true=true){
		...
		foreach ($roleModels as $roleId=>$roleModel) {
			...
			$jobcategory = $roleModel->getcategory();
			$roles[$roleName] = array('id'=>$roleId,'jobcategory'=>$jobcategory);
		}
		return $roles;
    }
	/**
