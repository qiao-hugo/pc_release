<?php
class UserManger_Cacheinfo_Action extends Vtiger_Action_Controller {
    function __construct() {
        parent::__construct();
        $this->exposeMethod('makeAllCache');
    ...
    public function makeAllCache(Vtiger_Request $request) {
       if(!Users_Privileges_Model::isPermitted('UserManger', 'EditView')){
           $response = new Vtiger_Response();
           $response->setResult('没有权限操作');
           $response->emit();
           exit;
       }
        ...
        foreach($tabPermissions as $key=>$profiles){
            ...
            foreach($profiles as $profile){
                if(empty($fields[$key]['readonly'])){
                    ...
                    if(!empty($modulePermission[$profile]['readonly'])){
    ...
    public function cacheDependency($arr){
        if(is_array($arr)&&count($arr)>0){
            ...
            if(!empty($arr)){
                $fun='make'.$arr;
                call_user_func(array('UserManger_Cacheinfo_Action', $fun));
            }else{
