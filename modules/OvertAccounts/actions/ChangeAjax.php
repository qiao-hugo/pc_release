<?php
class OvertAccounts_ChangeAjax_Action extends Vtiger_Action_Controller {
    function __construct() {
        parent::__construct();
        $this->exposeMethod('setAdvancesmoney');
        $this->exposeMethod('getAccountReportList');
        $this->exposeMethod('cleanFallToovertTime');
    }
	...
	public function process(Vtiger_Request $request) {
		...
		*/
        ...
        if($accountlabeling=='qudaobu' && ($type=='OVERT' && $entity['accountcategory']!=2 || $type=='TEMPORARY' && $entity['accountcategory']==2 )){
    ...
    public function cleanFallToovertTime(Vtiger_Request $request){
        $recordId=$request->get('record');
        $recordModel=Vtiger_Record_Model::getCleanInstance('OvertAccounts');
        if($recordModel->exportGrouprt('Accounts','TOOVERTTIME')){
            global $adb;
            $adb->pquery('UPDATE `vtiger_account` SET fall_toovert_time=null WHERE accountid=?',array($recordId));
            $array[]=array('fieldname'=>'accountcategory','prevalue'=>'5天限制', 'postvalue'=>'解除');
            $this->setModTracker($recordId,$array);
        }
    }
}
