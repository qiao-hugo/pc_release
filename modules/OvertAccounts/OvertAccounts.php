...
// OvertAccount is used to store vtiger_account information.
class OvertAccounts extends CRMEntity {
	var $log;
	var $db;
	...
	function OvertAccounts() {
		$this->log =LoggerManager::getLogger('account');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('OvertAccounts');
	}
	...
	*/
	...
	function get_history($id)
	{
		...
		return getHistory('OvertAccounts',$query,$id);
	}
...
//		$entityIds = implode(',', $entityIds);
	...
	* Returns Export OvertAccounts Query.
	*/
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery("OvertAccounts", "detail_view");
		...
		$query .= $this->getNonAdminAccessControlQuery('OvertAccounts',$current_user);
	...
	*/
	function getAccountHierarchy($id) {
		...
		foreach ($this->list_fields_name as $fieldname=>$colname) {
			if(getFieldVisibilityPermission('OvertAccounts', $current_user->id, $colname) == '0') {
		...
		foreach($accounts_list as $account_id => $account_info) {
			...
			$hasRecordViewAccess = (is_admin($current_user)) || (isPermitted('OvertAccounts', 'DetailView', $account_id) == 'yes');
			...
			foreach ($this->list_fields_name as $fieldname=>$colname) {
				...
				if(!$hasRecordViewAccess && $colname != 'accountname') {
					$account_info_data[] = '';
				} else if(getFieldVisibilityPermission('OvertAccounts', $current_user->id, $colname) == '0') {
					...
					if ($colname == 'accountname') {
						...
						// - to show the hierarchy of the OvertAccounts
	...
	/**
	* Function to Recursively get all the upper OvertAccounts of a given Account
	* @param  integer   $id      		- accountid
	...
	*/
	function __getParentOvertAccounts($id, &$parent_accounts, &$encountered_accounts) {
		global $log, $adb;
        $log->debug("Entering __getParentOvertAccounts(".$id.",".$parent_accounts.") method ...");
		...
		if ($adb->num_rows($res) > 0 &&
			...
			$this->__getParentOvertAccounts($parentid,$parent_accounts,$encountered_accounts);
		}
	...
	function retrieve_entity_info($record, $module){
        ...
        //if($this->column_fields['accountcategory']==2) {
            ...
            if(!in_array($current_user->roleid,array("H83","H84","H85"))){
                $this->column_fields['phone'] = "领取后才能查看";
                $this->column_fields['mobile'] = "领取后才能查看";
            }
		//}
		if($this->column_fields['accountcategory']!=2){
			$where=getAccessibleUsers('OvertAccounts','',true);
