...
class VisitingOrder_Detail_View extends Vtiger_Detail_View {
...
function preProcess(Vtiger_Request $request, $display=true) {
	...
	function showModuleDetailView(Vtiger_Request $request) {
		...
		if($recordModel->get('outobjective')!='出差'){
            // 获取拜访单的签到信息
            $sql = "SELECT
				u.last_name,
				v.signtime,
		...
		foreach ($visitsingArr as $key=>$value) {
            ...
            foreach ($fileData as $fileid){
                ...
                $fileurl[] = 'http://'.$_SERVER['HTTP_HOST'].'/index.php?module=VisitingOrder&action=ChangeAjax&mode=showImg&filename='.$fileid;
            }
		...
		$strangeVisitHistory = $recordModel->strangeVisitHistory($recordId);
		...
		$destinationcode = explode("***",$recordModel->get("destinationcode"));
		$viewer->assign("LATLNG",$destinationcode[1].','.$destinationcode[0]);
		$viewer->assign("LNGLAT",$destinationcode[0].','.$destinationcode[1]);

		$viewer->assign('VISITSINGS', $t_data);
		$viewer->assign('STRANGEVISITHISTORY', $strangeVisitHistory);
