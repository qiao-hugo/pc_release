...
class Users_Privileges_Model extends Users_Record_Model {
	...
	public function hasModulePermission($tabId) {
		...
		//$moduleModel = Vtiger_Module_Model::getInstance($tabId);//暂时不处理后期代码全部使用缓存的模块信息
		//&& $moduleModel->isActive() 确认模块是否可用
		global $current_user;
		return ($current_user->is_admin=='on' || in_array($tabId,$current_user->profile_tabs_permission));
	}
	...
	public function hasModuleActionPermissionN($module,$action){
		$currentUser = vglobal('current_user');
		require("crmcache/actionPermission".$currentUser->roleid.".php");
		
		
		return ($this->isAdminUser()||$actionPermission[$module.'/'.$action]);
	}
	/**
	 ...
	 * @return Users_Privilege_Model object
	 */
	public static function getInstanceById($userId, $module = NULL, $flag = false) {
		if (empty($userId))
			return null;
		$currentUser = vglobal('current_user');
		$user=CRMEntity::getInstance("Users");
		$user->checkUserprivileges($userId);
		...
		$valueMap['secondroleid'] = $second_user_roles;
		...
		$valueMap['current_user_departments'] = $current_user_departments;
		$valueMap['current_user_parent_role_seq'] = $current_user_parent_role_seq;
		$valueMap['current_user_parent_departments'] = $current_user_parent_departments;
		$valueMap['user_manager'] = $user_manager;//负责部门
		
		$valueMap['isproductmanager'] = $isproductmanager;//@TODO
     ...
     * @return Boolean
     */
    public static function isReportPermissions($moduleName, $actionName) {
        $currentUser = vglobal('current_user');
        $currentUserId = $currentUser->id;
        global $adb;
        $sql = "select * from vtiger_exportmanage where module=? and classname=? and userid=? and deleted=0" ;
        $result = $adb->pquery($sql, array($moduleName,$actionName,$currentUserId));;
