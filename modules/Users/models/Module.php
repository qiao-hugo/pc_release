...
class Users_Module_Model extends Vtiger_Module_Model {
	...
	public function getDefaultUrl() {
		return 'index.php?module='.$this->get('name').'&parent=Settings&view='.$this->getDefaultViewName();
	}
	...
	//用户名和工号重复校验 By Joe @20150415
	public function checkDuplicateUser($querytype,$fieldvalue){
		$db = PearDatabase::getInstance();
		if($querytype=='user_name'){
			...
			$query = "SELECT user_name FROM vtiger_users WHERE status='Active' and usercode = ?";
			$result = $db->pquery($query, array($fieldvalue));
			if($db->num_rows($result) > 0){
				return true;
			}
