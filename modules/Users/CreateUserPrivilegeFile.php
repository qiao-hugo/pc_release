...
/** Creates a file with all the user, user-role,user-profile, user-groups informations 
  * 创建用户，用户角色，用户组，用户配置，用户部门，核心文件，通过用户的配置文件达到权限控制
...
function createUserPrivilegesfile($userid){
	...
	if($handle){
		...
		if($user_focus->is_admin == 'on'){	//管理员无需权限
		...
		/*	$newbuf .= "\$user_info=".constructSingleStringKeyValueArray($userInfo).";\n";
			...
			$secondroleid=$userInfo['secondroleid'];
			...
			//print_r($subRoleAndUsers);
			$def_org_share=getDefaultSharingAction();
			$parentRoles=getParentRole($user_role);

			/** young.yang@71360.com
			 * @history 2014-08-25 新增部门级别的数据缓存
			 */
			
			$user_department=$userInfo['departmentid'];
			$user_department_info=getDepartmentInformation($user_department);//获取用户的子部门
			$user_manager_department=userManage($userid);//负责部门

			$isproductmanager=getProductManager($userid);
			
			$newbuf .= "\$current_user_departments='".$user_department."';\n";
			$newbuf .= "\n";
			$newbuf .= "\$current_user_parent_departments='".$user_department_info[$user_department][1]."';\n";
			$newbuf .= "\n";
			$newbuf .= "\$user_manager='".$user_manager_department."';\n";
			$newbuf .= "\n";
			$newbuf .= "\$isproductmanager='".$isproductmanager."';\n";
			$newbuf .= "\n";
			$newbuf .= "\$current_user_roles='".$user_role."';\n";
			$newbuf .= "\n";
			$newbuf .= "\$second_user_roles='".$secondroleid."';\n";//加入多个角色的选择
			$newbuf .= "\n";
			...
			$newbuf .= "\$profileActionPermission=null;\n";
			$newbuf .="\n";		
			...
			//$newbuf .="\n";	
			//$newbuf .= "\$subordinate_department_users=".constructTwoDimensionalCharIntSingleArray($user_department_info).";\n";
			$newbuf .="\n";		
			$newbuf .= "\$user_info=".constructSingleStringKeyValueArray($userInfo).";\n";
...
function userManage($id){
	global $adb;
	$sql="select (select parentdepartment from 
...
department from vtiger_departmentusers where userid=?";
	...
	if($adb->num_rows($result)){
		return $adb->query_result($result, 0,'department');
	}
	return '';
}
/**
 ...
 * @param unknown $id
 */
function getProductManager($id){
	global $adb;
	$sql="select productid from vtiger_products where find_in_set(?,replace(productman,' |##| ',','))";
	$result=$adb->pquery($sql,array($id));
	if($adb->num_rows($result)){
		return 1;
	}
