...
Class Users_PreferenceEdit_View extends Vtiger_Edit_View {
	...
	public function checkPermission(Vtiger_Request $request) {
        ...
        $result=$adb->pquery('SELECT id FROM `vtiger_user2setting` WHERE FIND_IN_SET(?,userid) AND FIND_IN_SET(1,setting)',array($userid));

        if(($currentUserModel->isAdminUser() == true || $currentUserModel->get('id') == $record || $adb->num_rows($result)>0)) {
			return true;
		} else {
	...
	public function process(Vtiger_Request $request) {
		...
		$viewer->assign('USER_MODEL', Users_Record_Model::getCurrentUserModel());
		$viewer->assign('SHOWADMIN', $recordModel->personalAuthority('Users','SHOWADMIN'));
