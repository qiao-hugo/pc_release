...
class SeparateInto_ListView_Model extends Vtiger_ListView_Model {


	//根据参数显示数据
	public function getListViewEntries($pagingModel) {
		$db = PearDatabase::getInstance();
		$moduleName ='SeparateInto';
		...
		if(empty($orderBy) && empty($sortOrder)){
			...
			$orderBy = 'vtiger_separateinto.separateintoid';
			$sortOrder = 'DESC';
		}
		...
		while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['separateintoid'];

			$listViewRecordModels[$rawData['separateintoid']] = $rawData;
		}
    ...
    public function getUserWhere(){
		...
		if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('SeparateInto','List',true);
            if($where!='1=1'){
