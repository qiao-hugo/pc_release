...
class BusinessOpportunities_selectAjax_Action extends Vtiger_Action_Controller {
    ...
    public function getdata(Vtiger_Request $request){
        ...
        $new_sql = "SELECT count(1) AS value,sourcecategory AS name FROM vtiger_leaddetails LEFT JOIN vtiger_crmentity ON vtiger_leaddetails.leadid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted=0 AND (sourcecategory!='' AND sourcecategory IS NOT NULL) {$tempdate} {$where} GROUP BY sourcecategory";
