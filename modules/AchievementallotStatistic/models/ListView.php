 ...
 */
class AchievementallotStatistic_ListView_Model extends Vtiger_ListView_Model {


	public function getListViewEntries($pagingModel) {
		$db = PearDatabase::getInstance();
        $moduleName = 'AchievementallotStatistic';
        $orderBy = $this->getForSql('orderby');
        $sortOrder = $this->getForSql('sortorder');
        if(empty($orderBy) && empty($sortOrder)){
            $orderBy = 'vtiger_achievementallot_statistic.achievementmonth DESC,vtiger_achievementallot_statistic.achievementallotid ';
            $sortOrder = 'DESC';
        }
	...
	public function getUserWhere(){
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){
            ...
            $where=getAccessibleUsers('AchievementallotStatistic','List',true);
            if($where!='1=1'){
                $query=' and vtiger_achievementallot_statistic.receivedpaymentownid in ('.implode(',',$where).')  AND vtiger_achievementallot_statistic.departmentid in (\''.implode("','",$departments).'\')';
            }else{
                $query=' AND vtiger_achievementallot_statistic.departmentid in (\''.implode("','",$departments).'\')';;
            }
            $listQuery .= $query;
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                $listQuery .= ' and vtiger_achievementallot_statistic.receivedpaymentownid '.$where;
            }
        }
