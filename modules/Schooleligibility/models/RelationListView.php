...
*定义管理语句
*/
class Schooleligibility_RelationListView_Model extends Vtiger_RelationListView_Model {
	static $relatedquerylist = array(

		'Schoolinterqua'=>"SELECT vtiger_schoolinterqua.schoolinterquaid, vtiger_schoolresume.`name` AS r_name, IF ( vtiger_schoolresume.gendertype = 'MALE', '男', '女') AS r_gendertype, vtiger_schoolresume.telephone AS r_telephone, vtiger_schoolresume.email AS r_email, vtiger_schoolinterqua.qis_train, vtiger_schoolinterqua.qtraindate, vtiger_schoolinterqua.qis_sssessment, vtiger_schoolinterqua.qis_trainquali, vtiger_schoolresume.schoolresumeid AS t_schoolresumeid  FROM vtiger_schoolinterqua INNER JOIN vtiger_schoolresume ON vtiger_schoolinterqua.schoolresumeid = vtiger_schoolresume.schoolresumeid WHERE schooleligibilityid =?"
    );
	...
	public function getEntries_implement($pagingModel) {
		...
		for($i=0; $i< $db->num_rows($result); $i++ ) {
            ...
            } else if ($relationModule->get('name') == 'Schoolinterqua') {
            	$record->setId($row['schoolinterquaid']);
				$relatedRecordList[$row['schoolinterquaid']] = $record;
            } else {
