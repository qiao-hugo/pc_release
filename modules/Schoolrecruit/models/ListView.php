...
class Schoolrecruit_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolrecruitid'];
			$listViewRecordModels[$rawData['schoolrecruitid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('Schoolrecruit','List', true);
            if($where!='1=1'){
        ...
        // 陪同人也可以看
        $listQuery .= " OR find_in_set( '".$current_user->id."', REPLACE(accompany, ' |##| ', ',') ) ";
