...
class VisitAccountContract_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		if(empty($orderBy) && empty($sortOrder)){
			...
			$orderBy = 'visitaccountcontractid';
		...
		while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['visitaccountcontractid'];
			$listViewRecordModels[$rawData['visitaccountcontractid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('VisitAccountContract','List', true);
            if($where!='1=1'){
