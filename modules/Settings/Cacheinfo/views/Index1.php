...
class Settings_Cacheinfo_Index_View extends Settings_Vtiger_Index_View {
	...
	public function process(Vtiger_Request $request) {
		...
		//缓存分组与部门数据共享规则
		$sql1='SELECT * FROM `vtiger_datashare_module_rel` RIGHT JOIN `vtiger_datashare_role2depart` on vtiger_datashare_module_rel.shareid=vtiger_datashare_role2depart.shareid';
		...
		$sql = "select id,last_name,status,reports_to_id FROM vtiger_users where reports_to_id>0";
		$result = $adb->pquery($sql);
		$noOfresult = $adb->num_rows($result);
		$reports=array();
	...
	$reports= get_mychilddep($reports);
		...
		if($currentModule=='Cacheinfo'){
			require_once($root_directory.'includes/runtime/Cache.php');
			Vtiger_Cache::clear();
