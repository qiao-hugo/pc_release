...
class Settings_Vtiger_Module_Model extends Vtiger_Base_Model {
     ...
     * @throws Exception
     */
    public function getFieldId($modulename){
        global $adb;
        $result=$adb->pquery('SELECT fieldid FROM `vtiger_settings_field` WHERE modulename=? LIMIT 1',array($modulename));
        $resultData=$adb->raw_query_result_rowdata($result,0);
        return $resultData['fieldid'];
    }
}
