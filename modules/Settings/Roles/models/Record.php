...
class Settings_Roles_Record_Model extends Settings_Vtiger_Record_Model {
	...
	public function getChildren() {
		...
		if(!$this->children) {
			...
			$sql = 'SELECT * FROM vtiger_role WHERE parentrole LIKE ? AND depth = ? order by jobcategory DESC';
    ...
    public function getjobcategory(){
        ...
        $db = PearDatabase::getInstance();
        $query = 'SELECT * FROM vtiger_jobcategory';
        ...
        $jobcategory = array();
        for($i=0; $i<$num_rows; $i++) {
            $jobcategory[] = $db->raw_query_result_rowdata($result,$i);
        }
        return $jobcategory;
    }
...
//	}
	...
	public function save() {
		...
		if($mode == 'edit') {
			$sql = 'UPDATE vtiger_role SET rolename=?, parentrole=?, depth=?, allowassignedrecordsto=?,jobcategory=? WHERE roleid=?';
			$params = array($this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'),$this->getcategory(), $roleId);
			$db->pquery($sql, $params);
		} else {
			$sql = 'INSERT INTO vtiger_role(roleid, rolename, parentrole, depth, allowassignedrecordsto,jobcategory) VALUES (?,?,?,?,?,?)';
			$params = array($roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'),$this->getcategory());
			$db->pquery($sql, $params);
	...
	public function delete($transferToRole) {
		...
		$db->pquery('UPDATE `vtiger_usermanger` SET roleid=? WHERE roleid=?', array($transferRoleId, $roleId));
	 ...
	 * @return <Array> list of Role models <Settings_Roles_Record_Model>
	 */
	public static function getAllToArray($baseRole = false) {
		$db = PearDatabase::getInstance();
		$params = array();
		...
		$roles = array();
		for ($i=0; $i<$noOfRoles; ++$i) {
			$role = $db->query_result_rowdata($result, $i);
