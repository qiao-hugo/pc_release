...
class Settings_SharingAccess_RuleMember_Model extends Vtiger_Base_Model {
	...
	public static function getInstance($qualifiedId) {
		...
		if($type=='Department'){
			...
			if($db->num_rows($result)) {
				$row = $db->query_result_rowdata($result, 0);
				$qualifiedId = 'Department:'.$row['departmentid'];
        ...
        if($type=='companyid'){
            $companyids='AA';
            if(!empty($memberId)){
                $companyids=explode(',',$memberId);
                $companyid=implode("','",$companyids);
                $query='SELECT GROUP_CONCAT(invoicecompany) as companyfullname FROM vtiger_invoicecompany WHERE companycode in(\''.$companyid.'\') ';
                $result=$db->pquery($query,array());
                if($db->num_rows($result)){
                    $companyfullname=$db->query_result($result, 0, 'companyfullname');
                }
            }
            $rule = new self();
            return $rule->set('id', $companyids)->set('name', $companyfullname);
        }
