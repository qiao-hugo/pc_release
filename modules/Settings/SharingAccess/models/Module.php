...
class Settings_SharingAccess_Module_Model extends Vtiger_Module_Model {
	...
	public static function getAll($editable=false) {
		...
		$query = 'SELECT * FROM vtiger_def_org_share INNER JOIN vtiger_tab ON vtiger_tab.tabid = vtiger_def_org_share.tabid';
		$params = array();
		if($editable) {
			$query .= ' WHERE editstatus = ?';
    ...
    public static function getCompayId(){
        global $adb;
        $query='SELECT companycode as company_codeno,invoicecompany as companyfullname FROM vtiger_invoicecompany GROUP BY companycode';
