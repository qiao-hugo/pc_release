...
class Schoolassessmentpeople_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$moduleName ='Schoolassessmentpeople';
		...
		$listQuery = preg_replace('/SELECT/', 'SELECT vtiger_schoolassessmentpeople.assessmentid AS assessmentid, ', $listQuery, 1);
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['assessmentpeopleid'];
			$listViewRecordModels[$rawData['assessmentpeopleid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $listQuery .= ' and vtiger_schoolassessmentpeople.p_assessownerid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_schoolassessmentpeople.p_assessownerid '.$where . ' ';
            }
        }
