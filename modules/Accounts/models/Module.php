...
class Accounts_Module_Model extends Vtiger_Module_Model {
	...
	public function getSideBarLinks($linkParams) {
		...
		*/
        ...
        $quickLink[] = array(
            'linktype' => 'SIDEBARLINK',
            'linklabel' => '掉公海设置',
            'linkurl' => $this->getListViewUrl().'&report=2',
            'linkicon' => '',
        );
        }
    ...
    /**
     * 搜索使用， cxh 2019 - 11 -29 字段 应为如果是 filter=changeHistory 没有走条件 所以新建条件
    ...
    public function getSearchFields(){
            ...
            if(empty($this->searchfields)){
                ...
                foreach($moduleBlockFields as $moduleFields){
                    foreach($moduleFields as $moduleField){
                        ...
                        if($moduleField->get('name')=='accountname'){
                            $this->searchfields[$moduleField->get('name')] = $moduleField;
                        }else if($moduleField->get('name')=='assigned_user_id'){

                            $moduleField->name='olsuser';
                            $moduleField->label='olsUser';
                            $moduleField->table='vtiger_accountsmowneridhistory';
                            $moduleField->column='vtiger_accountsmowneridhistory.oldsmownerid';
                            $this->searchfields['olsuser']=$moduleField;

                            $moduleFieldB=clone $moduleField;
                            $moduleFieldB->id+=1;
                            $moduleFieldB->name='newuser';
                            $moduleFieldB->label='newUser';
                            $moduleFieldB->table='vtiger_accountsmowneridhistory';
                            $moduleFieldB->column='vtiger_accountsmowneridhistory.newsmownerid';
                            $this->searchfields['newuser']=$moduleFieldB;

                            $moduleFieldC=clone $moduleField;
                            $moduleFieldC->id+=1;
                            $moduleFieldC->name='modiuser';
                            $moduleFieldC->label='modiUser';
                            $moduleFieldC->table='vtiger_accountsmowneridhistory';
                            $moduleFieldC->column='vtiger_accountsmowneridhistory.modifiedby';
                            $this->searchfields['modiuser']=$moduleFieldC;

                        }elseif($moduleField->get('name')=='createdtime'){
                            $moduleField->table='vtiger_accountsmowneridhistory';
                            $moduleField->label='CreatedTime';
                            $moduleField->column='vtiger_accountsmowneridhistory.createdtime';
                            $this->searchfields[$moduleField->get('name')] = $moduleField;
        ...
        return  $this->wpjam_array_push($this->searchfields,array('intentionality'=>$this->searchfields['intentionality']),'accountrank');
    }
	/**
     ...
     */
    public function searchRecordAPP($searchValue, $parentId=false, $parentModule=false, $relatedModule=false,$isPermission=false){
        if(!empty($searchValue) && empty($parentId) && empty($parentModule)) {
            $matchingRecords = $this->getSearchResultApp($searchValue, $this->getName(),$isPermission);
        }
     ...
     * @throws Exception
     */
    private function  getSearchResultApp($searchKey, $module=false,$isPermission=false){
        $db = PearDatabase::getInstance();
        ...
        if(!$isPermission){
            ...
            if($where!='1=1'){
                $query .= ' and ( (vtiger_crmentity.smownerid '.$where.$this->getShareAccount().') OR  vtiger_account.serviceid '.$where.' )';
            }
        }
    ...
    public function getListViewUrl($overtFlag=0) {
        if($overtFlag == 1){
            //公海客户
            return 'index.php?module=OvertAccounts&view='.$this->getListViewName();
        }else{
            //非公海客户
     ...
     * @return array|string
     */
    public function getAllShareAccount(){
        global $adb,$current_user;
        $userRecordModel = Users_Record_Model::getCleanInstance("Users");
//        if(!$userRecordModel->isChannelUser($current_user->departmentid)){
//            return '';
//        }

        $userIds = array_keys($userRecordModel->getAccessibleUsers());
        array_push($userIds,$current_user->id);
        $result=$adb->pquery('SELECT accountid FROM vtiger_shareaccount WHERE userid in ('.implode(",",$userIds).') AND sharestatus=1',array());
        $accountList='';
        if($adb->num_rows($result)){
            while($rowdata=$adb->fetch_array($result)){$accountList[]=$rowdata['accountid'];}
    ...
    public function getShareAccountUserIds($accountid){
        global $adb,$current_user;
        $result=$adb->pquery('SELECT userid FROM vtiger_shareaccount WHERE accountid=? AND sharestatus=1',array($accountid));
        $userids='';
        if($adb->num_rows($result)){
            while($rowdata=$adb->fetch_array($result)){$userids[]=$rowdata['userid'];}
        }
        return $userids;
    }
}
