...
class Accounts_RelationListView_Model extends Vtiger_RelationListView_Model {
	...
	'ServiceContracts'=>'SELECT vtiger_crmentity.crmid,vtiger_servicecontracts.contract_no,vtiger_servicecontracts.modulestatus,vtiger_servicecontracts.productname,vtiger_servicecontracts.total,vtiger_servicecontracts.signdate,vtiger_servicecontracts.firstreceivepaydate,SUM(vtiger_receivedpayments.unit_price) as repay FROM vtiger_servicecontracts INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_servicecontracts.servicecontractsid  LEFT JOIN vtiger_receivedpayments on (vtiger_servicecontracts.servicecontractsid = vtiger_receivedpayments.relatetoid AND vtiger_receivedpayments.deleted=0) WHERE vtiger_crmentity.deleted =0 AND vtiger_servicecontracts.sc_related_to=? ',
    ...
    'Potentials'=>'SELECT  vtiger_potential_detail.potentialdetailid AS crmid,vtiger_users.last_name,vtiger_potential.potential_no,vtiger_potential.customertypeproperty,vtiger_potential.cooperatedepartments,vtiger_potential_detail.potentialnames,vtiger_potential_detail.salesstages, vtiger_potential_detail.budgetinterval,CONCAT(vtiger_potential_detail.budgetlockstart,"~",vtiger_potential_detail.budgetlockend) as duration,IF ( vtiger_potential_detail.isannuallypay = 1, "是", "否" ) as isannuallypay,vtiger_potential_detail.dockingrole,vtiger_potential_detail.docker,vtiger_potential_detail.projectdetails,vtiger_potential.potentialid FROM  vtiger_potential_detail LEFT JOIN  vtiger_potential ON vtiger_potential.potentialid= vtiger_potential_detail.potentialid LEFT JOIN  vtiger_crmentity ON  vtiger_crmentity.crmid=vtiger_potential.potentialid LEFT JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.smownerid    WHERE  vtiger_crmentity.deleted=0 AND  vtiger_potential_detail.isdelete=0  AND  vtiger_potential_detail.related_to= ? '
    );
	...
	public function getEntries($pagingModel){
        ...
        $queryend='';
        if($where!='1=1'){
            $queryend=' and (vtiger_crmentity.smownerid in ('.implode(',',$where).') or vtiger_servicecontracts.receiveid in ('.implode(',',$where).') OR vtiger_servicecontracts.signid IN('.implode(',',$where).')) ';
        }
        $relatedquerylist['ServiceContracts'].=$queryend.' AND NOT EXISTS(SELECT 1 FROM vtiger_activationcode WHERE vtiger_activationcode.contractid=vtiger_servicecontracts.servicecontractsid AND vtiger_activationcode.`status` in(0,1) AND vtiger_activationcode.isbcustomer=1) GROUP BY vtiger_servicecontracts.contract_no';
		/*if(in_array($current_user->roleid,array('H82','H81','H80','H79','H101')) && !in_array($current_user->id,array(1121,43))){
            $relatedquerylist['ServiceContracts']='SELECT vtiger_crmentity.crmid,vtiger_servicecontracts.contract_no,vtiger_servicecontracts.modulestatus,vtiger_servicecontracts.productname,vtiger_servicecontracts.total,vtiger_servicecontracts.signdate,vtiger_servicecontracts.firstreceivepaydate,SUM(vtiger_receivedpayments.unit_price) as repay FROM vtiger_servicecontracts INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_servicecontracts.servicecontractsid  LEFT JOIN vtiger_receivedpayments on vtiger_servicecontracts.servicecontractsid=vtiger_receivedpayments.relatetoid WHERE vtiger_crmentity.deleted =0 AND vtiger_servicecontracts.sc_related_to=? AND vtiger_crmentity.smownerid='.$current_user->id.'';
        }*/
