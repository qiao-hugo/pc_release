...
class Accounts_Detail_View extends Vtiger_Detail_View {
	...
	public function getActivities(Vtiger_Request $request) {
		...
		if($currentUserPriviligesModel->hasModulePermission($moduleModel->getId())) {
            ...
            */
			...
			$recentActivities = Accounts_Record_Model::getservicecomments($parentRecordId, $pagingModel);
            $recentActivitiesandsmowner = Accounts_Record_Model::getservicecommentsandsmower($parentRecordId, $pagingModel);
			$RECENT_HEADS = Accounts_Record_Model::getheads($parentRecordId, $pagingModel);
    ...
    public function preProcess(Vtiger_Request $request, $display=true){
        ...
        //根据关联参数执行
        if(in_array($mode,array('getActivities'))) {//直接die
	...
	function showModuleDetailView(Vtiger_Request $request) {
			...
			$moduleName = $request->getModule();
			//start
			//Accounts_Module_Model::accountLevel($recordId);
			//end
    ...
    function showModuleSummaryView($request) {
        ...
        $viewer->assign('NAME_FLAG', Accounts_Record_Model::getsupperaccountupdate('accountname'));
        $viewer->assign('RANK_FLAG', Accounts_Record_Model::getsupperaccountupdate('accountrank'));
    
        $viewer->assign('MODULE_NAME', $moduleName);
        $viewer->assign('IS_AJAX_ENABLED', $this->isAjaxEnabled($recordModel));
        $viewer->assign('SUMMARY_RECORD_STRUCTURE', $recordStrucure->getStructure());
        //$viewer->assign('RELATED_ACTIVITIES', $this->getActivities($request));
        $viewer->assign('RELATED_ACTIVITIES', $this->RELATED_ACTIVITIES($recordId));
        //$viewer->assign('RANKLIMIT', $recordModel->getRankLimit());//详情页字段编辑已取消
    ...
    function showRecentComments(Vtiger_Request $request) {
        ...
        if(false && $userid == $serviceid['serviceid']){//如果当前登录用户是该客户的客服，就去查找任务包
            $if_updateTask1 = true;//已停用
        ...
        $viewer->assign('ACCOUNTINTENTIONALITY',ModComments_Record_Model::getAccountIntentionality($accountid));
    ...
    public function RELATED_ACTIVITIES($recordId){
        return '<div class="widgetContainer_H" data-url="module=Accounts&view=Detail&record='.$recordId.'&mode=getActivities&page=1&limit=5" data-name="LBL_UPDATES"><div class="widget_contents"></div></div>';
    }
}
