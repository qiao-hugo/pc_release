...
class Accounts_CheckDuplicate_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
        ...
        $strlength = mb_strlen($data['last_name'],'utf-8');
        $last_name =  mb_substr($data['last_name'],$strlength-1,$strlength,'utf-8');
        $name='**'.$last_name.'【'.$data['departmentname'].'】，客户等级：'.$accuntank;
        $str=' '.$name;
		...
		if (!$result) {
			...
			$result = array('success'=>true, 'message'=>vtranslate('客户在临时区，请联系负责人：', $moduleName).$str);
		}else if($result===1){
            $result = array('success'=>true, 'message'=>vtranslate('客户在正常保护区，请联系负责人：', $moduleName).$str);
        }else{
