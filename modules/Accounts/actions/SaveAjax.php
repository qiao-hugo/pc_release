        ...
        if($request->get('field')=='accountname' || $request->get('field')=='accountrank'){
            ...
            do {
                ...
                if ($request->get('field') == 'accountname') {
                    ...
                    $TRecrodModel=Vtiger_Record_Model::getCleanInstance('TyunWebBuyService');
                    global $tyunweburl,$sault;
                    $tyunweburl1=$tyunweburl.'api/app/tcloud-account/v1.0.0/account/updateCustomerName';
                    $time=time().'123';
                    $sault1=$time.$sault;
                    $token=md5($sault1);
                    $curlset=array(CURLOPT_HTTPHEADER=>array(
                        ...
                        "S-Request-Time:".$time));
                    $rebinddata=json_encode(array("accountId"=>$recordId,"accountName"=>$value));
                    $TRecrodModel->https_request($tyunweburl1,$rebinddata,$curlset);
                }
                ...
                $recordModel = Vtiger_Record_Model::getInstanceById($recordId, 'Accounts',true);
