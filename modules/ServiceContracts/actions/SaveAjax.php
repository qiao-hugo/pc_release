...
class ServiceContracts_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function process(Vtiger_Request $request) {
        ...
        if(!$recordModel->personalAuthority('ServiceContracts','Received')){
            $result[$request->get('field')] = array('value' => $request->get('value'),
                'display_value' => '<span class="label label-warning">没有权限</span>');
        ...
        if($recordId){
            if($request->get('field')=='invoicecompany' && !$recordModel->canChangeMainContract($request)){
                $result[$request->get('field')] = array('value' => $request->get('value'),
                    'display_value' => '<span class="label label-warning">当前合同已经申请开票，合同主体不可变更</span>');
                $result['_recordLabel'] = $recordModel->getName();
                $result['_recordId'] = $recordModel->getId();
                $response = new Vtiger_Response();
                $response->setEmitType(Vtiger_Response::$EMIT_JSON);
    ...
    public function saveRecord($request) {
        ...
        if(($moduleModel->exportGrouprt('ServiceContracts','Received') && (($request->get('field')=='modulestatus'&&$recordModel->entity->column_fields['modulestatus']!=Workflows_Module_Model::$moudulestatus['c_complete'])||$request->get('field')!='modulestatus')) or (in_array($recordModel->get('modulestatus'),array('已发放','a_normal','b_check','b_actioning','c_stamp'))
                && $recordModel->get('assigned_user_id')==$current_user->id
                && empty($sc_related_to) && $request->get('field')=='sc_related_to')){
            //合成完成后不允许修改
            $recordModel->save();
    ...
    public function checkPermission(Vtiger_Request $request) {
        ...
        if($request->get('field')!='sc_related_to'){
