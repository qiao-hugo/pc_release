...
class circleSeal {
	...
	public function __construct($str ='', $rad = 75, $rmwidth = 6, $strad = 24, $stang = 0, $crang = 0, $fsize = 16, $inrad =0){
		$this->sealString	= empty($str) ? "财务服务合同" : $str;
		$this->strMaxLeng	= 12;
		$this->sealRadius	= $rad;
	...
	private function drawStart(){
        ...
        $zyz='专用章';
        $charset = mb_detect_encoding($zyz);
        if($charset != 'UTF-8'){
            $zyz = mb_convert_encoding($zyz, 'UTF-8', 'GBK');
        }
        imagettftext($this->img, $this->fontSize-2, 0, $this->sealRadius/2, $this->sealRadius*2-$this->sealRadius/2, $this->backGround, $this->font, $zyz);
        imagettftext($this->img, $this->fontSize/1.8, 0, $this->sealRadius/2, $this->sealRadius*2-$this->sealRadius/4, $this->backGround, $this->font, date('Y-m-d'));
	}
