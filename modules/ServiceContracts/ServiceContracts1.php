...
//                $this->db->pquery("DELETE  FROM   vtiger_servicecontracts_update_receivedpayments_divide_tmp  WHERE  servicecontractsid = ? ",array($record));
            ...
            case 'CHANGESMOWNER'://更改合同领取人打回
                ...
                $this->db->pquery("UPDATE vtiger_servicecontracts SET modulestatus=backstatus WHERE servicecontractsid=?",array($record));
                break;
            default :
                $this->db->pquery("DELETE FROM `vtiger_salesorderworkflowstages` WHERE vtiger_salesorderworkflowstages.salesorderid=? AND vtiger_salesorderworkflowstages.modulename='ServiceContracts' AND vtiger_salesorderworkflowstages.workflowsid=?",array($record,$workflowsid));
...
//                $result = $recordModel->getModule()->doCancel($entity['contract_no']);
        ...
        if($currentflag=='CHANGESMOWNER'){
            ...
            if($recordModel->get("modulestatus")=='已发放' && $servicenum){
                echo json_encode(array('success'=>false,'msg'=>'你已领取的合同已超出合同领取份额，可将现有合同归还后再次审批'));
                exit();
            }
		}
	...
	function retrieve_entity_info($record, $module){
		...
		if($where!='1=1'){
			//编辑或详细视图状态下非管理员的权限验证
                //$query='SELECT 1 FROM vtiger_shareaccount WHERE vtiger_shareaccount.userid in('.implode(',',$where).') and vtiger_shareaccount.sharestatus=1 AND vtiger_shareaccount.accountid=?';
                $query='SELECT 1 FROM vtiger_shareaccount WHERE vtiger_shareaccount.userid='.$current_user->id.' and vtiger_shareaccount.sharestatus=1 AND vtiger_shareaccount.accountid=?';
                ...
                if(!in_array($this->column_fields['assigned_user_id'],$where) && !in_array($this->column_fields['Receiveid'],$where) && /*!in_array($this->column_fields['receiptorid'],$where) &&*/ !in_array($this->column_fields['Signid'],$where) && $flag && !getAccessibleCompany('','',false,-1,$this->column_fields['companycode']) && $invoicecompanyflag){
