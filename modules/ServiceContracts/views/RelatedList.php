...
class ServiceContracts_RelatedList_View extends Vtiger_RelatedList_View {
	function process(Vtiger_Request $request) {
         ...
         * 判断是否有下载权限
         */
        $viewer->assign('CAN_DOWNLOAD', 0);
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);//module相关的数据
		if($moduleModel->exportGrouprt('ServiceContracts','Download')){   //权限验证
            $viewer->assign('CAN_DOWNLOAD', 1);
        }

		//vmatefile 权限
        $viewer->assign('V_CAN_DOWNLOAD', 0);
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);//module相关的数据
        if($moduleModel->exportGrouprt('Vmatefiles','Download')){   //权限验证
            $viewer->assign('V_CAN_DOWNLOAD', 1);
        }

        $viewer->assign('V_VIEW', 0);
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);//module相关的数据
        if($moduleModel->exportGrouprt('Vmatefiles','View')){   //权限验证
            $viewer->assign('V_VIEW', 1);
        }

        if($relatedModuleName == 'Files'||$relatedModuleName == 'Vmatefiles') {
			// 判断该用户是否有权限 附件签收权限
	        global $current_user;
	        ...
	        global $current_user;
            $IS_ROLEID=true;
            if(!in_array($current_user->id,array(1121,43)) && in_array($current_user->roleid,array('H101','H79','H80','H81','H82'))){
                $IS_ROLEID=false;
            }
            //$viewer->assign('IS_ROLEID', !in_array($current_user->roleid,array('H101','H79','H80','H81','H82')));
            $viewer->assign('IS_ROLEID', $IS_ROLEID);
        ...
        if ($relatedModuleName == 'Files') {
            $collate_fit = 0;
            $contractInfo = $parentRecordModel->getData();
            if ($contractInfo['modulestatus'] == 'c_complete') {
                if ($contractInfo['last_collate_status'] == 'fit') {
                    $collate_fit = 1;
                } elseif (empty($contractInfo['last_collate_status']) && $contractInfo['first_collate_status'] == 'fit') {
                    $collate_fit = 1;
                }
            }
            $viewer->assign('collate_fit', $collate_fit);
        }
