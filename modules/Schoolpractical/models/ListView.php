...
class Schoolpractical_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolpracticalid'];
			$listViewRecordModels[$rawData['schoolpracticalid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('Schooladopt','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and vtiger_schoolpractical.instructor in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_schoolpractical.instructor '.$where . ' ';
            }
        }
