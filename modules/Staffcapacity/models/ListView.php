...
class Staffcapacity_ListView_Model extends Vtiger_ListView_Model {

	public function getListViewEntries($pagingModel) {
        $db = PearDatabase::getInstance();

        $moduleName = 'Staffcapacity';
       ...
       // echo $listQuery;die();
        ...
        while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['staffcapacityid'];
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('Staffcapacity','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and vtiger_staffcapacity.businessid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_staffcapacity.businessid '.$where . ' ';
            }
        }
