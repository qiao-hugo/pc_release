...
class Schoolemploypeople_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$listQuery = preg_replace('/SELECT/', 'SELECT vtiger_schoolemploypeople.schoolemployid, ', $listQuery, 1);
		$listQuery = preg_replace('/vtiger_schoolemploypeople.p_assessownerid,/', '(select last_name from vtiger_users where id=vtiger_schoolemploypeople.p_assessownerid) AS p_assessownerid, vtiger_schoolemploypeople.p_assessownerid AS t_assessownerid,', $listQuery, 1);
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolemploypeopleid'];
			$listViewRecordModels[$rawData['schoolemploypeopleid']] = $rawData;
		}
    ...
    public function getListViewHeaders() {
        ...
        if(!empty($sourceModule)){
           return $queryGenerator->getModule()->getPopupFields();
        	...
        	//print_r($queryGenerator->getModule());die;
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('School','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and vtiger_schoolemploypeople.p_assessownerid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_schoolemploypeople.p_assessownerid '.$where . ' ';
            }
        }
