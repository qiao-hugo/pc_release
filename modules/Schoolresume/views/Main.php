...
class Schoolresume_Main_View extends Import_Main_View{



    public static function import($request, $user) {
        $importController = new Schoolresume_Main_View($request, $user);
        $importController->saveMap(); //模版另存为;
        $fileReadStatus = $importController->copyFromFileToDB();//状态判断; 文件总条数插入条数
        /*var_dump($fileReadStatus);
        exit;*/
        if($fileReadStatus) {
            $importController->queueDataImport(); //vtiger_import_queue 中插入数据相关数据
        }
        $isImportScheduled = $importController->request->get('is_scheduled'); //是否超过导入最大条数; null
    ...
    public function triggerImport($batchImport=false) {
        $importInfo = Import_Queue_Action::getImportInfo($this->request->get('module'), $this->user); //vtiger_import_queue 导入脚本处理状态; 返回一个数组;
        ...
        $importDataController->importData(); //跟新 import1表
        Import_Queue_Action::updateStatus($importInfo['id'], Import_Queue_Action::$IMPORT_STATUS_HALTED);//3
        $importInfo = Import_Queue_Action::getImportInfo($this->request->get('module'), $this->user);//vtiger_import_queue
        self::showImportStatus($importInfo, $this->user);
    }

    public static function showImportStatus($importInfo, $user) {
        if($importInfo == null) {
            Import_Utils_Helper::showErrorPage(vtranslate('ERR_IMPORT_INTERRUPTED', 'Import'));  //importinfo vtiger_import_queue
            exit;
        }
        ...
        if(method_exists($focus, 'getImportStatusCount')) {
            ...
            $importStatusCount = $importDataController->getImportStatusCount();      //返回vtiger_import_1 statues 导入状态;
        }
    ...
    public static function showCurrentStatus($importInfo, $importStatusCount, $continueImport) {
        ...
        $viewer->view('ImportStatus.tpl', 'Schoolresume');
    }
    ...
    public static function showResult($importInfo, $importStatusCount) {
        ...
        $viewer->view('ImportResult.tpl', 'Schoolresume');
    }
    ...
    public static function showScheduledStatus($importInfo) {
        ...
        $viewer->view('ImportSchedule.tpl', 'Schoolresume');
    }
    ...
    public function copyFromFileToDB() {
        $fileReader = Import_Utils_Helper::getFileReader($this->request, $this->user);//返回获取文件处理的；
        $fileReader->read();
        $fileReader->deleteFile();
        if($fileReader->getStatus() == 'success') {    //打开文件状态
    ...
    public static function deleteMap($request) {
        ...
        $viewer->view('Import_Saved_Maps.tpl', 'Schoolresume');
    }
}
