...
class Schoolvisit_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$index = 0;
		//ob_end_clean(); //移动端 这里必须要清除一下(可能是自己本地的原因，添加到这里也不影响) 输出缓存 周海 
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolvisitid'];
			$listViewRecordModels[$rawData['schoolvisitid']] = $rawData;
		}
		//ob_end_clean();
		//print_r($tt);die;
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            //$listQuery .= ' and vtiger_crmentity.smownerid in ('.implode(',',$where).')';
            $listQuery .= ' and (vtiger_schoolvisit.extractid in ('.implode(',',$where).') OR find_in_set('.$current_user->id.', REPLACE(vtiger_schoolvisit.accompany,\' |##| \', \',\') ))';
        }else{
