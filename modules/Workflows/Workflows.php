...
class Workflows extends CRMEntity {
    var $db, $log; // Used in class functions of CRMEntity

    var $table_name = 'vtiger_workflows';
    var $table_index= 'workflowsid';
     ...
     */
    var $customFieldTable = Array('vtiger_workflowscf', 'workflowsid');
    var $entity_table = "vtiger_crmentity";
    /**
     * Mandatory for Saving, Include tables related to this module.
     */
    var $tab_name = Array('vtiger_crmentity', 'vtiger_workflows', 'vtiger_workflowscf');
    ...
    var $tab_name_index = Array(
		...
		'vtiger_workflows'   => 'workflowsid',
	    'vtiger_workflowscf' => 'workflowsid');
    ...
    // tablename should not have prefix 'vtiger_'
		'Workflow Name'=> Array('workflows', 'workflowsname'),
		'Is Contract'=> Array('workflows', 'iscontract'),
		'Is Content'=>Array('workflows','iscontent'),
		'Is Attachment'=>Array('workflows','isattachment'),
		'Is Chargeback'=>Array('workflows','ischargeback'),
    	'Templetsid'=>Array('workflowscf','templetsid'),
		'Assigned To' => Array('crmentity','smownerid')
    );
    var $list_fields_name = Array(
    /* Format: Field Label => fieldname */
		'Workflow Name'=> 'workflowsname',
		'Is Contract'=> 'iscontract',
		'Is Content'=>'iscontent',
		'Is Attachment'=>'isattachment',
		'Is Chargeback'=>'ischargeback',
    	'Templetsid'=>'templetsid',
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view from list view (Fieldname)
	var $list_link_field = 'workflowsname';
	...
	var $popup_fields = Array('workflowsname');

	// Placeholder for sort fields - All the fields will be initialized for Sorting through initSortFields
	var $sortby_fields = Array();

	// For Alphabetical search
	var $def_basicsearch_col = 'workflowsname';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'workflowsname';

	// Required Information for enabling Import feature
	var $required_fields = Array('workflowsname'=>1);

	// Callback function list during Importing
	var $special_functions = Array('set_import_assigned_user');

	var $default_order_by = 'workflowsname';
	var $default_sort_order='ASC';
	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	//var $mandatory_fields = Array('createdtime', 'modifiedtime', 'workflowsname', 'workflowsid', 'assigned_user_id');
	var $mandatory_fields = Array('assigned_user_id', 'createdtime', 'modifiedtime', 'workflowsname');
	
	var $relatedmodule_list=array('WorkflowStages');
	//'Potentials','Quotes'
	var $relatedmodule_fields=array(
	'WorkflowStages'=>array('workflowstagesname'=>'WORKFLOWSTAGES Name','handleaction'=>'Handleaction','workflowsid'=>'Related to','sequence'=>'sequence')
	);
	...
	function create_export_query($where)
	{
		...
		$sql = getPermittedFieldsQuery('workflows', "detail_view");
	...
	function vtlib_handler($modulename, $event_type) {
		if($event_type == 'module.postinstall') {
			...
			$projectsResult = $adb->pquery('SELECT tabid FROM vtiger_tab WHERE name=?', array('Workflows'));
			...
			if($modcommentsModuleInstance && file_exists('modules/ModComments/ModComments.php')) {
				...
				if(class_exists('ModComments')) ModComments::addWidgetTo(array('Workflows'));
			}
			*/
    ...
    function delete_related_module($module, $crmid, $with_module, $with_crmid) {
         if (!in_array($with_module, array('WorkflowStages'))) {
        ...
        foreach($with_crmid as $relcrmid) {
            ...
            $child->mode='edit';
            $child->column_fields['workflowsid']='';
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("WorkflowStages"=>"vtiger_workflowstages","Documents"=>"vtiger_senotesrel","Attachments"=>"vtiger_seattachmentsrel");
