...
class SaleManager_List_View extends Vtiger_List_View {
    function process (Vtiger_Request $request) {

        $viewer = $this->getViewer($request);
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);//module相关的数据
        if($request->get('public')=='AuditSettings'){
            $viewer->assign('ROLES',$moduleModel->getRole());
            $viewer->assign('ACCOUNTROLES',$moduleModel->getAccountRole());
