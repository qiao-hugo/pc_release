...
class DepaSalestargetdetail_ListView_Model extends Vtiger_ListView_Model {
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $listQuery .= ' and vtiger_depasalestargetdetail.createid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_depasalestargetdetail.createid '.$where . ' ';
            }
        }
