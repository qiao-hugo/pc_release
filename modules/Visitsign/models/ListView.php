...
class Visitsign_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		while($rawData=$db->fetch_array($listResult)) {
            //$rawData['id'] = $rawData['visitsignid'];
            $rawData['issign'] = $rawData['issign'] == '1' ? '是' : '否';
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('Visitsign','List',true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and (vtiger_visitsign.userid in ('.implode(',',$where).') OR find_in_set('.$current_user->id.', REPLACE(vtiger_visitsign.userid,\' |##| \', \',\') ))';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
            	//$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and (vtiger_visitsign.userid '.$where . ' OR find_in_set('.$current_user->id.', REPLACE(vtiger_visitsign.userid,\' |##| \', \',\') ))';
            }
        }
