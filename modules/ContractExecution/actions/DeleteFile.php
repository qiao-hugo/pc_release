...
class ContractExecution_DeleteFile_Action extends Vtiger_Action_Controller {
    ...
    public function process(Vtiger_Request $request) {
        ...
        if($fileid>0){
            if(!empty($recordid)&& $recordid>0) {
                $recordModel = Vtiger_Record_Model::getInstanceById($recordid, 'ContractsAgreement');
                $modulestatus=$recordModel->entity->column_fields['modulestatus'];
                if(!in_array($modulestatus,array('a_exception','a_normal'))){
                    echo json_encode(array('success'=>false,'result'=>array('msg'=>'当前状态不允许删除')));
                    exit;
                }
            }
            ...
            if($adb->num_rows($result)) {
                ...
                //if(unlink($root_directory.DIRECTORY_SEPARATOR.$filePath.$savedFile)){
                ...
                echo json_encode(array('success'=>true,'result'=>array('msg'=>'文件删除成功'.$fileDetails['attachmentsid'])));
                exit;
                //}
