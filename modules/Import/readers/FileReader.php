...
class Import_FileReader_Reader {
	...
	public function createTable() {
		...
		$tableName = Import_Utils_Helper::getDbTableName($this->user);//创建一个自定义表格;
		$fieldMapping = $this->request->get('field_mapping');
        $moduleFields = $this->moduleModel->getFields();
       // $columnsListQuery = 'id INT PRIMARY KEY AUTO_INCREMENT, status INT DEFAULT 0, recordid INT'; //status 字段重复
        $columnsListQuery = 'id INT PRIMARY KEY AUTO_INCREMENT, statuses INT DEFAULT 0, recordid INT';
