...
class Import_Data_Action extends Vtiger_Action_Controller {
	...
	public function importData() {
		...
		$this->updateModuleSequenceNumber();//跟新特殊前缀标题;
	}

	public function initializeImport() {
		$lockInfo = Import_Lock_Action::isLockedForModule($this->module); //vtiger_import_locks查看锁定状态;  SELECT * FROM vtiger_import_locks WHERE tabid=?
		if ($lockInfo != null) {
	...
	public function updateImportStatus($entryId, $entityInfo) {
		...
		$recordId = $entityInfo['id'];
		/*if (!empty($entityInfo['id'])) {
			$entityIdComponents = vtws_getIdComponents($entityInfo['id']);
			$recordId = $entityIdComponents[1];
		}*/
		$adb->pquery('UPDATE ' . Import_Utils_Helper::getDbTableName($this->user) . ' SET statuses=?, recordid=? WHERE id=?',
				array($entityInfo['status'], $recordId, $entryId)); //跟新了导入文件表
	}
	...
	public function createRecords() {
		...
		$moduleObjectId = $moduleMeta->getEntityId();//读取vtiger_ws_entity 的id;
		$moduleFields = $moduleMeta->getModuleFields();//回款表的字段
		$tableName = Import_Utils_Helper::getDbTableName($this->user);
		$sql = 'SELECT * FROM ' . $tableName . ' WHERE statuses = '. Import_Data_Action::$IMPORT_RECORD_NONE;
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if(method_exists($focus, 'importRecord')) {
                ...
                if (!empty($mergeType) && $mergeType != Import_Utils_Helper::$AUTO_MERGE_NONE) {
					...
					$createRecord = true;//创建数据
				}

				if ($createRecord) {
					$fieldData = $this->transformForImport($fieldData, $moduleMeta);//继续处理导入文件的值;
                    if($fieldData == null) {
						$entityInfo = null;
					} else {
                        try{
                            $tmp1=$this->user->db->database->raiseErrorFn;
                            $tmp2=$this->user->db->database->transOff;
                            $this->user->db->database->raiseErrorFn=null;
                            $this->user->db->database->transOff=false;
                            $entityInfo = vtws_create($moduleName, $fieldData, $this->user);
                            $this->user->db->database->raiseErrorFn=$tmp1;
                            $this->user->db->database->transOff=$tmp2;
                        } catch (Exception $e){
			...
			if ($fieldInstance->getFieldDataType() == 'owner') {
				...
				if (!empty($fieldValue)) {
					...
					if (count($fieldValueDetails) > 1) {
						...
						$entityId = getEntityId($referenceModuleName, $entityLabel);//根据关联表label获取字段id;
					} else {
	...
	public function getImportStatusCount() {
		...
		$result = $adb->query('SELECT statuses FROM '.$tableName);
		...
		if($result) {
			...
			for($i=0; $i<$noOfRows; ++$i) {
				$status = $adb->query_result($result, $i, 'statuses');
				if(self::$IMPORT_RECORD_NONE == $status) {
					...
					switch($status) {
						case self::$IMPORT_RECORD_CREATED	:	$statusCount['CREATED']++;//1
																break;
						case self::$IMPORT_RECORD_SKIPPED	:	$statusCount['SKIPPED']++;//2
																break;
						case self::$IMPORT_RECORD_UPDATED	:	$statusCount['UPDATED']++;//3
																break;
						case self::$IMPORT_RECORD_MERGED	:	$statusCount['MERGED']++;//4
																break;
					}
				}
