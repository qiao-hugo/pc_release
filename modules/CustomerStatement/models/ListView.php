...
class CustomerStatement_ListView_Model extends Vtiger_ListView_Model {
	//根据参数显示数据
	public function getListViewEntries($pagingModel) {
        $db = PearDatabase::getInstance();
        $moduleName = 'CustomerStatement';
        $orderBy = $this->getForSql('orderby');
        $sortOrder = $this->getForSql('sortorder');

        if(empty($orderBy) && empty($sortOrder)){
            $orderBy = 'vtiger_customerstatement.customerstatementid';
            $sortOrder = 'DESC';
        }
        $this->getSearchWhere();
...
//        echo $listQuery;die;
        ...
        while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['customerstatementid'];
            $rawData['deleted'] = ($current_user->is_admin=='on')?1:0;
            $listViewRecordModels[$rawData['customerstatementid']] = $rawData;
        }
