...
class Approval_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		if($request->get('relationOperation') && $parentModuleName) {
			...
			$data['createid'] = $request->get('createid');
			$data['relationid'] = $request->get('relationid');

			global $adb;
			$current_id = $adb->getUniqueID("vtiger_approval");
			$data['approvalid'] = $current_id;

			$divideNames = array_keys($data);
			$divideValues = array_values($data);
			// 快速添加数据
			
			//echo 'INSERT INTO `vtiger_approval` ('. implode(',', $divideNames).') VALUES ('. generateQuestionMarks($divideValues) .')';
			/*print_r($divideValues);
			exit;*/
			$adb->pquery('INSERT INTO `vtiger_approval` ('. implode(',', $divideNames).') VALUES ('. generateQuestionMarks($divideValues) .')',$divideValues);
