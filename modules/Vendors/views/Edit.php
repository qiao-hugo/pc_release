...
Class Vendors_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ($vendortype == 'medium') {
			...
			$viewer->assign('SELECT_MAINPLATFORM', explode(' # ', $mainplatform));
			$viewer->assign('IS_VENDORTYPE', 1);
		}
        $vendorRecordModel=Vtiger_Record_Model::getCleanInstance("Vendors");

		// 获取充值平台数据
		$viewer->assign('RECHARGEPLATFORM_DATA', $vendorRecordModel->getRechargeplatform());

		// 获取产品数据
		$viewer->assign('PRODUCT_DATA', $vendorRecordModel->getProducts());

		// 获取产品返点数据
    	$vendorsrebateData = $vendorRecordModel->getVendorsrebate($record);
