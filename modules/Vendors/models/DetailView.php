...
class Vendors_DetailView_Model extends Vtiger_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		global $current_user;
		if($column_fields['assigned_user_id']==$current_user->id && empty($column_fields['workflowsid']) && $this->getFileStyle($recordModel->getId())) {
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'LBL_SET_VENDORSTATE',
				'linkurl' => '',
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
		}
        if($column_fields['assigned_user_id']==$current_user->id) {
            $basicActionLink = array(
                'linktype' => 'DETAILVIEWBASIC',
                'linklabel' => 'LBL_SET_VENDORSCORE',
                'linkurl' => '',
                'linkicon' => ''
            );
            $linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
        ...
        if(array_key_exists($column_fields['assigned_user_id'], $accessibleUsers) || $current_user->is_admin=='on') {
            $basicActionLink = array(
                'linktype' => 'DETAILVIEWBASIC',
                'linklabel' => 'LBL_CHANGESMOWNER',
                'linkurl' => '',
                'linkicon' => ''
            );
            $linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
    ...
    {
        $query='SELECT `name` FROM vtiger_files WHERE description=\'Vendors\' AND relationid=? AND delflag=0';
