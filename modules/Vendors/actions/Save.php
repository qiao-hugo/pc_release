...
class Vendors_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		$mainplatform = $request->get('mainplatform');
		// 主要产品
		if ($vendortype == 'medium' && is_array($mainplatform)) {
			$mainplatform = implode(' # ', $mainplatform);
		} else {
			$mainplatform = str_replace(' # ', '', $mainplatform);
		}
		$request->set('mainplatform', $mainplatform);
        $vendorname=$request->get('vendorname');
        $vendorname=preg_replace('/^(\s|\x{3000}|\x{00a0}|\x{0020}|&nbsp;)+|(\s|\x{3000}|\x{00a0}|\x{0020}|&nbsp;)+$/u','',$vendorname);
