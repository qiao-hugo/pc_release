...
class Schoolinterqua_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolinterquaid'];
			$listViewRecordModels[$rawData['schoolinterquaid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        /*if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $where=getAccessibleUsers('Schoolinterqua','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and vtiger_schoolrecruit.createuserid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_schoolrecruit.createuserid '.$where . ' ';
            }
        }*/
