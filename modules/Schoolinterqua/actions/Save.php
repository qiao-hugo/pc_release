...
class Schoolinterqua_Save_Action extends Vtiger_Action_Controller {
	...
	public function saveRecord($request) {
		...
		if($request->get('relationOperation')) {
			$request->set('sourceRecord', $request->get('sourceRecord'));
		}
		$recordModel->save();
		...
		if($request->get('relationOperation')) {
			...
			//print_r($relationModel);die;
			$relationModel->addRelation($parentRecordId, $relatedRecordId);

			// 更新 schooleligibilityid
			global $adb;
			$sql = "update vtiger_schoolinterqua set schooleligibilityid=? where schoolinterquaid=?";
			$adb->pquery($sql, array($parentRecordId, $relatedRecordId));
			$sql = "update vtiger_schoolresume set is_linterqua=1 where schoolresumeid=?";
			$adb->pquery($sql, array($request->get('schoolresumeid')));
		}
		return $recordModel;
	}
	...
	public function getDetailViewUrl($id) {
		return 'index.php?module=Schoolinterqua&view=Detail&record='.$id;
	}
