...
class Files_DownloadFile_Action extends Vtiger_Save_Action {
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($batch == 1) {
            ...
            if (!empty($records)) {
                global $adb;
                $result = $adb->pquery("SELECT * FROM vtiger_files WHERE attachmentsid IN (" . $records . ")", []);
            ...
            if ($record > 0) {
                global $adb;
                $result = $adb->pquery("SELECT * FROM vtiger_files WHERE attachmentsid=?", array($record));
