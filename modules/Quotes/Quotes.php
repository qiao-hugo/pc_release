...
class Quotes extends CRMEntity {
	...
	var $tab_name = Array('vtiger_crmentity','vtiger_quotes','vtiger_quotescf');
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_quotes'=>'quoteid','vtiger_quotescf'=>'quoteid');
	/**
...
// 		}
		...
		$result = $adb->pquery("SELECT vtiger_crmentity.smownerid, vtiger_products.productname,vtiger_products.productid  FROM vtiger_products LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid WHERE vtiger_crmentity.deleted = 0 AND vtiger_crmentity.crmid > 0 and vtiger_products.productid=?",array($this->column_fields['productid']));
		if($adb->num_rows($result)){
			$product=$adb->query_result_rowdata($result,0);
