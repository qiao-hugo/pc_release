...
class ContractTemplate_FileUpload_Action extends Vtiger_FileUpload_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		if($files['name'] != '' && $files['size'] > 0){
			...
			$upload_status = move_uploaded_file($filetmp_name, $upload_file_path . $current_id . "_" .$newfilename);
            if(!$upload_status){
			...
			$params2 = array($current_id, $filename, $model,$filetype, $upload_file_path,$current_user->id,date('Y-m-d H:i:s'),$newfilename);
