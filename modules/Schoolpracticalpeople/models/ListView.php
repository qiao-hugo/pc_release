...
class Schoolpracticalpeople_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$moduleName ='Schoolpracticalpeople';
		...
		if(empty($orderBy) && empty($sortOrder)){
			...
			$orderBy = 'schoolpracticalpeopleid';
		...
		$listQuery = preg_replace('/SELECT/', 'SELECT vtiger_schoolpracticalpeople.practicalpeopleid AS practicalpeopleid, ', $listQuery, 1);
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schoolpracticalpeopleid'];
			$listViewRecordModels[$rawData['schoolpracticalpeopleid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $listQuery .= ' and vtiger_schoolpracticalpeople.instructor in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_schoolpracticalpeople.instructor '.$where . ' ';
            }
        }
