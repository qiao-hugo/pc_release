...
class Matchreceivements_BasicAjax_Action extends Vtiger_Action_Controller {
	function __construct() {
        ...
        $this->exposeMethod('getSplitServiceContracts');
        $this->exposeMethod('splitBatchReceive');
        $this->exposeMethod('getServiceInfoByStayId');
	}
        ...
        $contractInfo=$adb->query_result_rowdata($contractInfo,0);
           ...
           /* while ($dtaRows=$adb->fetch_array($results)){
            ...
            if(!empty($matchdate)){
                ...
                $sql="select createtime from vtiger_receivedpayments_notes where receivedpaymentsid=? order by receivedpaymentsnotesid desc";
                $result=$adb->pquery($sql,array($receivepayid));
                $matchdate=$adb->query_result_rowdata($result,0,'createtime');
                if(!$matchdate){
...
//            $achievementmonth=$this->getAchievementmonth($reality_date,$matchdate);
            ...
            $achievementmonth=$this->getAchievementmonthWithContract($reality_date,$matchdate,$contractid,$receivepayid);
            ...
            if($isMoreYears==1){
                 ...
                 for ($i=0;$i<2;$i++){
                    ...
                    //多年单循环拆单。
                     ...
                     $datavalue['effectiverefund']=abs($effectiverefund);
        ...
        if(!$this->judgeIsNewOrder($rp['activationcodeid'])){
    ...
    public function VRZCalculationAchievement($rp,$contractid,$receivepayid,$shareuser,$total,$currentid,$remark,$adb,$matchdate=0,$salesorderid=0){
        ...
        if($rp['productid']==2226512){
            $achievementtype='newadd';
            // 新增业务
        }else if(strpos($rp['contract_no'],"VRZ") || strpos($rp['contract_no'],"TYUNZHXT")){
            $achievementtype='newadd';
            //续费业务
    ...
    public function noSaaSCalculationAchievement($rp,$contractid,$receivepayid,$shareuser,$total,$currentid,$remark,$adb,$matchdate=0,$salesorderid=0){
        ...
        //查询业绩是否下单  查询已扣减成本金额
        if($rp['multitype']==1){
            $sqlsalesorder=" SELECT costreduction,performanceoftime,salesorderid  FROM vtiger_salesorder as so,vtiger_salesorderrayment as sd   WHERE  so.servicecontractsid=? AND sd.receivedpaymentsid=? AND sd.salesorderid=so.salesorderid  AND  so.iscancel=0 LIMIT 1  ";
            $salesorderInfo=$adb->pquery($sqlsalesorder,array($contractid,$receivepayid));
        }else{
            $sqlsalesorder=" SELECT  costreduction,performanceoftime,salesorderid  FROM vtiger_salesorder WHERE  servicecontractsid=? AND iscancel=0 LIMIT 1  ";
            $salesorderInfo=$adb->pquery($sqlsalesorder,array($contractid));
        }
        $salesorderInfo=$adb->query_result_rowdata($salesorderInfo,0);
        if(empty($salesorderInfo['performanceoftime'])){
            ...
            //查询回款时间，必须是当月（计算业绩的月份）
            $reality_dateYm = date('Y-m',strtotime($reality_date));
            //查询工单时间，必须是2号之前提的
            //查询---RAYMENT_MATCH----审核时间是否是3号之前
            $workflowsSql = 'select auditortime from vtiger_salesorderworkflowstages where workflowstagesflag="RAYMENT_MATCH" and isaction=2 and modulename="SalesOrder" and salesorderid=?';
            $workflows = $adb->pquery($workflowsSql, array($salesorderInfo['salesorderid']));
            $auditortime = $adb->query_result($workflows,0,'auditortime');
            $auditortimeYm = date('Y-m', strtotime($auditortime));
            $matchdateYm = date('Y-m', strtotime($matchdate));
            $day = date('d', strtotime($auditortime));
            global $log;
            $log->info('工单id：'.$salesorderInfo['salesorderid'].' 对应回款收款时间：'.$reality_date.'  RAYMENT_MATCH节点审核时间：'.$auditortime.' 回款匹配时间：'.$matchdate);
            if((strtotime($auditortimeYm) == strtotime("+1 months",strtotime($reality_dateYm)) && $day<=21) || strtotime($auditortimeYm) == strtotime($reality_dateYm)){
                $achievementmonth=$reality_dateYm;
            }else{
                $achievementmonth=$this->getspecialAchievementmonth($matchdate,$reality_date,$matchdate);
            }
            $log->info('工单id：'.$salesorderInfo['salesorderid'].' 业绩的月份：'.$achievementmonth);
        }
    ...
    public function tsiteCalculationAchievement($rp,$contractid,$receivepayid,$shareuser,$total,$currentid,$remark,$adb,$matchdate=0,$salesorderid=0){
        ...
        //查询业绩是否下单  查询已扣减成本金额
        if(0<$salesorderid){
            $sqlsalesorder=" SELECT costreduction,performanceoftime,modulestatus,salesorderid FROM vtiger_salesorder WHERE  salesorderid=? AND iscancel=0 LIMIT 1  ";
            $salesorderInfo=$adb->pquery($sqlsalesorder,array($salesorderid));
        }else{
            if($rp['multitype']==1){
                $sqlsalesorder=" SELECT costreduction,performanceoftime,modulestatus,salesorderid  FROM vtiger_salesorder as so,vtiger_salesorderrayment as sd   WHERE  so.servicecontractsid=? AND sd.receivedpaymentsid=? AND sd.salesorderid=so.salesorderid  AND  so.iscancel=0 LIMIT 1  ";
                $salesorderInfo=$adb->pquery($sqlsalesorder,array($contractid,$receivepayid));
            }else{
                $sqlsalesorder=" SELECT  costreduction,performanceoftime,modulestatus,salesorderid  FROM vtiger_salesorder WHERE  servicecontractsid=? AND iscancel=0 LIMIT 1  ";
                $salesorderInfo=$adb->pquery($sqlsalesorder,array($contractid));
            }
        }
        ...
        if(empty($salesorderInfo['performanceoftime']) || $salesorderInfo['modulestatus']=='c_lackpayment'){
            ...
            // global $log;
            // $log->info('工单id：'.$salesorderInfo['salesorderid'].' 对应回款收款时间：'.$reality_date.'  RAYMENT_MATCH节点审核时间：'.$auditortime.' 回款匹配时间：'.$matchdate);
            // if((strtotime($auditortimeYm) == strtotime("+1 months",strtotime($reality_dateYm)) && $day<=3) || strtotime($auditortimeYm) == strtotime($reality_dateYm)){
            //     $achievementmonth=$reality_dateYm;
            // }else{
            //     $achievementmonth=$this->getspecialAchievementmonth($matchdate,$reality_date,$matchdate);
            // }
    ...
    /**
     * 非TYun的产品没给的产品走这里计算以工单为单位
     */
    public function othersExtraCalculationAchievementsalesorder($rp,$contractid,$receivepayid,$shareuser,$total,$currentid,$remark,$adb,$matchdate=0,$salesorderid=0){
        $generatedamount=0;
        //②查询非Tyun类年限
        $queryc=" SELECT FLOOR(MAX(vtiger_salesorderproductsrel.agelife)/12) as agelife FROM vtiger_salesorderproductsrel WHERE vtiger_salesorderproductsrel.salesorderid= ? AND multistatus in(0,1) LIMIT 1";
        $resultdatapayments=$adb->pquery($queryc,array($salesorderid));
        ...
        $remark.="非T云类通用公式";
        //人力成本   工单外采成本  额外成本
        $queryc="SELECT IFNULL(sum(vtiger_salesorderproductsrel.costing),0) AS costing,IFNULL(sum(vtiger_salesorderproductsrel.purchasemount),0) AS purchasemount,IFNULL(sum(vtiger_salesorderproductsrel.extracost),0) AS extracost FROM vtiger_salesorderproductsrel WHERE vtiger_salesorderproductsrel.salesorderid =?";
        $costingdata=$adb->pquery($queryc,array($salesorderid));
        $costingdata=$adb->query_result_rowdata($costingdata,0);
        // 有关 沙龙 外采 媒介充值 其他
        $queryc="SELECT extra_type,extra_price FROM vtiger_receivedpayments_extra WHERE vtiger_receivedpayments_extra.receivementid=? ";
        $otherdata=$adb->pquery($queryc,array($receivepayid));
        ...
        $costpriceTaoCan=0;
        if(!empty($rp['productid'])){
            $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid  WHERE vtiger_salesorderproductsrel.salesorderid =? AND multistatus=3 LIMIT  1 ";
            $product=$adb->pquery($sql,array($salesorderid));
            while ($rowsDat=$adb->fetch_array($product)) {
                if ((strpos($rp['contract_no'], "XF") != false || $rp['servicecontractstype'] != '新增')) {
                    $marketingPriceTaoCan += $rowsDat['renewalfee'] * $rowsDat['agelife'];
        ...
        $costpriceExtra=0;
        if(!empty($rp['extraproductid'])){
            $sql="SELECT FLOOR(vtiger_salesorderproductsrel.agelife/12) as agelife,vtiger_products.realprice,vtiger_products.unit_price,vtiger_products.renewalfee,vtiger_products.renewalcost FROM vtiger_salesorderproductsrel  LEFT JOIN  vtiger_products  ON  vtiger_products.productid=vtiger_salesorderproductsrel.productcomboid  WHERE vtiger_salesorderproductsrel.salesorderid =?";
            $product=$adb->pquery($sql,array($salesorderid));
        ...
        $sqlsalesorder=" SELECT  costreduction,performanceoftime  FROM vtiger_salesorder WHERE  salesorderid=? AND iscancel=0 LIMIT 1  ";
        $salesorderInfo=$adb->pquery($sqlsalesorder,array($salesorderid));
        ...
        while ($rowDatas=$adb->fetch_array($resultdatas)){
            ...
            $remark.="工单总成本（".$worksheetcost."）"."已扣减成本（".$salesorderInfo['costreduction']."）";
            //到账业绩
            $surplusCostreduction=$worksheetcost-$salesorderInfo['costreduction'];//剩余所成本扣除数
            $surplusCostreduction=$surplusCostreduction>0?$surplusCostreduction:0;
            $arriveachievement=$total-$surplusCostreduction;
            $arriveachievement=$arriveachievement>0?$arriveachievement:0;//际
            $arriveachievement=$arriveachievement*$scalling/100;
            $arriveachievement=$arriveachievement-$otherdatas['extra_price']*$scalling/100;
            if($arriveachievement< 0 ){
                $arriveachievement=0;
        ...
        //如果已减工单成本 + 目前回款金额 大于等于总成本说明 工单成本已经减完了 则直接把已减成本改成 总成本就ok了
        if($total+$salesorderInfo['costreduction']>$worksheetcost){
            //更新已减成本
            $updatesql = " UPDATE vtiger_salesorder SET costreduction=? WHERE salesorderid =? ";
            $adb->pquery($updatesql,array($worksheetcost,$salesorderid));
        }else{
            //更新已减成本
            $updatesql = " UPDATE vtiger_salesorder SET costreduction=costreduction+? WHERE salesorderid =? ";
            $adb->pquery($updatesql,array($total,$salesorderid));
        }
        return array("datavalue"=>$returndata,"insertValueStr"=>$insertValueStr,'isTyun'=>0);
    }
    ...
    {
        if($this->isClassBAccount($contractid)){//是否是B类客户如果是B类客户则不生成业绩
            return ;
        }
        $adb = PearDatabase::getInstance();
        ...
        if(!empty($insertValueStr) && isset($type) && $type!=1){
            ...
            $achievementmonth=$this->getAchievementmonthWithContract($rp['reality_date'],$rp['matchdate'],$contractid,$receivepayid);
            $deleteStastic_sql='DELETE  FROM  vtiger_achievementallot_statistic WHERE arriveachievement>=0 and achievementmonth=? and  receivedpaymentsid = ?';
            $adb->pquery($deleteStastic_sql,array($achievementmonth,$receivepayid));
            //插入做销售业绩明细表数据
            $adb->pquery($addStastic_sql.$insertValueStr,array($datavalue));
            //插入汇总表数据
            $sqlquery=" SELECT achievementallotid,receivedpaymentownid,reality_date FROM vtiger_achievementallot_statistic WHERE receivedpaymentsid=? AND achievementmonth > 0 ";
            $result = $adb->pquery($sqlquery,array($receivepayid));

            $sql="update vtiger_receivedpayments set ischeckachievement=1 where receivedpaymentsid=?";
            $adb->pquery($sql,array($receivepayid));
            ...
            while ($rowdatas=$adb->fetch_array($result)){
                ...
                if($adb->num_rows($userResult)){//离职人员如系统客户已交接，则业绩提成核算给客户接收人；如果未交接，则离职员工无业绩提成，只核算团队业绩提成；要以离职具体日期计算，即如果是6.28离职，而回款是6.26，那么还是要核算业绩提成的【需确认系统规则是否按此设置】【包括交付暂扣的补发】
            ...
            //如果有有效数据 插入处理汇总表数据//排除掉离职后回款的员工业绩
            if(!empty($newArray)){
                //AchievementSummary_Record_Model::newAchievementSummary($newArray);
            }
            $sql='UPDATE vtiger_achievementallot_statistic SET userfullname=(SELECT last_name FROM vtiger_users WHERE id=receivedpaymentownid) WHERE receivedpaymentsid=?';
            $adb->pquery($sql,array($receivepayid));
    ...
    public function commonInsertAchievementallotStatisticjioaben($receivepayid=0,$total=0,$shareuser=0,$currentid=0,$contractid=0,$params=0,$matchdate=0){
        ...
        if(!empty($insertValueStr) && isset($type) && $type!=1){
            ...
            $recordModel = Matchreceivements_Record_Model::getCleanInstance("Matchreceivements");
            $recordModel->matchToRanking($receivepayid,true);
        }
    }
    /**
     *
    ...
    public  function rechargeCalculation(&$rp,&$contractid,&$receivepayid,&$shareuser,&$total,$currentid,&$remark,&$adb,&$params,$matchdate=0,$salesorderid=0){
        ...
        $costdeduction=$costprice;
        $achievementtype='newadd';
        $sql=" SELECT ra.refillapplicationid,GROUP_CONCAT(ra.receivedpaymentsid) AS receivedpaymentsidstr,r.exchangerate,r.receivementcurrencytype,SUM(ra.refillapptotal) AS refillapptotal FROM vtiger_refillapprayment AS ra,vtiger_receivedpayments AS r WHERE ra.refillapplicationid =? AND ra.receivedpaymentsid=r.receivedpaymentsid AND ra.deleted = 0 AND ra.refillapptotal > 0 ";
        $resultData=$adb->pquery($sql,array($params['refillapplicationid']));
        $resultData=$adb->query_result_rowdata($resultData,0);
        
        //充值申请单毛利 * 回款利率的和
        $sql=" SELECT SUM(prestoreadrate * ? * 0.06) as taxation,SUM(prestoreadrate * ? * 0.9) as prestoreadratecost,SUM(transferamount * ?) as transferamount FROM vtiger_rechargesheet  WHERE refillapplicationid= ? ";
        $totalgrossprofit=$adb->pquery($sql,array($resultData['exchangerate'],$resultData['exchangerate'],$resultData['exchangerate'],$params['refillapplicationid']));
        $totalgrossprofit=$adb->query_result_rowdata($totalgrossprofit,0);
        
        //税费----美金时没有税费
        $taxation = 0;
        if($resultData['receivementcurrencytype'] == '人民币'){
            $taxation = $totalgrossprofit['taxation'];
        }
        
        //预算广告费成本
        $prestoreadratecost = $totalgrossprofit['prestoreadratecost'];
        //应收款金额
        $transferamount = $resultData['refillapptotal'] * $resultData['exchangerate']; //$rp['unit_price'];//$totalgrossprofit['transferamount'];
        //毛利 = 回款-总成本===总成本=税费+预算广告费成本
        $totalgrossprofit= $transferamount - ($taxation + $prestoreadratecost);
        //日志
        global $log;
        $log->info($params['refillapplicationid'].'--refillapplicationid：业绩sql '.$sql.' refillapplicationid='.$params['refillapplicationid'].' 汇率：'.$resultData['exchangerate']);
        $log->info($params['refillapplicationid'].'--refillapplicationid：回款，税费，预算广告费成本，毛利 '.$transferamount.' '.$taxation.' '.$prestoreadratecost.' '.$totalgrossprofit.' 汇率：'.$resultData['exchangerate']);
        ...
        while ($rowDatas=$adb->fetch_array($resultdatas)){
            ...
            //判断分成人是否是中小体系---H3，非中小不生成业绩
            $parentdepartmentArr = !empty($Department['parentdepartment']) ? explode('::', $Department['parentdepartment']) : [];
            global $log;//日志
            $log->info($receivedpaymentownid.'--receivedpaymentownid:判断是否是中小体系：'.json_encode($parentdepartmentArr));
            if(!in_array('H3', $parentdepartmentArr)){
                $log->info($receivedpaymentownid.'--receivedpaymentownid:是非中小体系不生成业绩');
                continue;
            }
    ...
    public function process(Vtiger_Request $request) {
        ...
        do {
            ...
            if($contractid && $staypaymentid && $resultdata['setype']=='ServiceContracts'){
                $contractResult  = $adb->pquery("select sc_related_to from vtiger_servicecontracts where servicecontractsid=?",array($contractid));
                $staypaymentResult = $adb->pquery("select accountid from vtiger_staypayment where staypaymentid=?",array($staypaymentid));
                $contractData = $adb->fetchByAssoc($contractResult,0);
                $staypaymentData = $adb->fetchByAssoc($staypaymentResult,0);
                if($contractData['sc_related_to']!=$staypaymentData['accountid']){
                    $datad=array('flag'=>false,'msg'=>'代付款客户和合同不匹配，请切换');
                    break;
                }
            }

            $result = $adb->pquery("select old_receivedpaymentsid,ancestor_receivedpaymentsid,unit_price from vtiger_receivedpayments where receivedpaymentsid=? limit 1",array($receivepayid));
            $receivedPaymentData = $adb->fetchByAssoc($result,0);
            if($staypaymentid){
                $result = $adb->pquery("select contractid,surplusmoney,staypaymenttype from vtiger_staypayment where staypaymentid=?",array($staypaymentid));
                $data = $adb->fetchByAssoc($result,0);
                if($contractid!=$data['contractid']){
                    $datad=array('flag'=>false,'msg'=>'合同和代付款不匹配');
                    break;
                }
                if($data['surplusmoney']<$receivedPaymentData['unit_price'] && $data['staypaymenttype']=='fixation'){
                    $datad=array('flag'=>false,'msg'=>'回款金额应该小于等于代付款金额');
                    break;
                }
            }

            if($resultdata['setype']=='SupplierContracts'){
                $sql = "UPDATE vtiger_receivedpayments SET modulename='SupplierContracts',ismatchdepart=1,ismanualmatch=1,matchdate='" . date('Y-m-d') . "',relatetoid = ?,newdepartmentid=?,staypaymentid=? WHERE receivedpaymentsid = ?";
            ...
            if($receivedPaymentData['old_receivedpaymentsid']){
                $contractRecordModel = Vtiger_Record_Model::getInstanceById($contractid,'ServiceContracts');
                ...
                if (count($data)>1  || (count($data)==1 && $data[0]!=$subjectId)){
                    $datad=array('flag'=>false,'msg'=>'同一笔回款拆分成多笔回款，只能匹配到同一客户或者供应商下');
                    break;
                }
            }
            ...
            if($recordModelContract->get('contract_type')=='T云WEB版'&&!Matchreceivements_Record_Model::isPreContractMatched($contractid)){
                $datad=array('flag'=>false,'msg'=>'上份合同回款未完成，不允许匹配!');
                break;
            }

            $contractTotal=$recordModelContract->get('total');//合同金额

            $query="SELECT sum(unit_price) AS unit_price,sum(standardmoney) AS standardmoney FROM vtiger_receivedpayments WHERE relatetoid=? AND receivedstatus='normal' AND deleted=0";
            $resultdata=$adb->pquery($query,array($contractid));
            $totalReceivedpayments=0;

            if($adb->num_rows($resultdata)>0){
                $contractdata=$adb->query_result_rowdata($resultdata,0);
                $totalReceivedpayments=$contractdata['standardmoney'];
            }
            $query="SELECT standardmoney FROM vtiger_receivedpayments WHERE receivedpaymentsid=?";
            $resultaa=$adb->pquery($query,array($receivepayid));
            $total1=$resultaa->fields['standardmoney'];//用原币金额来匹配合同
            $totalReceivedpayments=bcadd($total1,$totalReceivedpayments,2);
            ...
            if($receivedstatus=='deposit'){
                $sql = "UPDATE vtiger_receivedpayments SET modulename='ServiceContracts',receivedstatus='deposit',ismatchdepart=1,ismanualmatch=1,allowinvoicetotal=0, allowreceiptamount=unit_price, matchdate='" . date('Y-m-d') . "',relatetoid = ?,newdepartmentid=?,staypaymentid=? WHERE receivedpaymentsid = ?";
                $adb->pquery($sql, array($contractid, $user_departments,$staypaymentid, $receivepayid));
                $modtrackerBasicId = $adb->getUniqueID("vtiger_modtracker_basic");
            ...
            global $kaililongdepartment;
            $deparr=$departmentinfo[$kaililongdepartment];
            if ($shareuser == 1 && in_array($currentUser->departmentid,$deparr)) {
                //共享商务独占业绩
        ...
        SELECT NULL ,owncompanys,receivedpaymentownid,scalling,servicecontractid,?,?*(scalling/100),'" . date('Y-m-d') . "',signdempart FROM vtiger_servicecontracts_divide WHERE servicecontractid = ? ";
            ...
            if ($receivepayid && $contractid && $total) {
                ...
                Matchreceivements_Record_Model::recordLog(array($sql,$contractid,$user_departments,$staypaymentid,$currentid,$receivepayid));
                ...
                $receivedModel->recordReceivedpayment($receivepayid,$contract_no,$contractid,'匹配',$staypaymentid,$currentid);
                ...
                //向t云发送付款信息
                $recordModelContract->payAfterMatch($contractid,$total,true,$currentid);
                ...
                $notestype=$request->get('notestype')?$request->get('notestype'):'notestype1';
        ...
        if($source&&$source=='app'){
            //从app那里直接来的直接把结果返回
            Matchreceivements_Record_Model::recordLog(array('app匹配'.json_encode($datad)),'app_match');
            return $datad;
        }
        if($datad['flag']){
            //匹配成功，发邮件
     ...
     * 是否是B类客户
     */
    public function isClassBAccount($contractid){
        global $adb;
        $query='SELECT 1 FROM vtiger_activationcode WHERE `status` in(0,1) AND isbcustomer=1 AND contractid=?';
        if($adb->num_rows($adb->pquery($query,array($contractid)))){
            return true;
        }
        return false;
    }

    public function getServiceInfoByStayId(Vtiger_Request $request){
        global $adb;
        $stayId=$request->get('record');
        $sql="select vtiger_staypayment.staypaymentid,vtiger_staypayment.contractid,vtiger_servicecontracts.contract_no from vtiger_staypayment left join vtiger_servicecontracts on vtiger_staypayment.contractid=vtiger_servicecontracts.servicecontractsid where vtiger_staypayment.staypaymentid=?";
        $result=$adb->pquery($sql,array($stayId));
        $retrun=array('flag'=>true);
        $retrun['staypaymentid']=$adb->query_result($result,0,'staypaymentid');
        $retrun['contractid']=$adb->query_result($result,0,'contractid');
        $retrun['contract_no']=$adb->query_result($result,0,'contract_no');
        $retrun['modulename']='ServiceContracts';
        $retrun['realoperate']=setoperate($retrun['contractid'],$retrun['modulename']);
        $response = new Vtiger_Response();
        $response->setResult($retrun);
        $response->emit();
    }
     ...
     * @return bool
     */
    public function judgeIsNewOrder($activationcodeidId){
        global $adb;
        $sql="SELECT activationcodeid from vtiger_activationcode  WHERE createdtime>='2021-05-01' and activationcodeid=".$activationcodeidId;
     ...
     * @param $contractid
     */
    public function getAchievementmonthWithContract($reality_date,$matchdate,$contractid,$receivepayid){
        Matchreceivements_Record_Model::recordLog(array('开始算业绩业绩月份',$reality_date,$matchdate,$contractid,$receivepayid),'autoAchievementallot');
        global $adb;
        $closingDate=Vtiger_Record_Model::getInstanceById(2434264,'ClosingDate');
        ...
        $matchTime=$matchTime?$matchTime:$matchdate.' 09:00:59';
        $madate=date("Y-m").'-'.$date.' 09:00:59';//最后签收截止时间
        Matchreceivements_Record_Model::recordLog(array('开始算业绩业绩月份1',$reality_date,$matchTime,$contractid,$receivepayid),'autoAchievementallot');
        if($reality_dateYm==date('Y-m',strtotime("-1 month"))){
            ...
            if($isLastMonth){
                ...
                $contractdate=date("Y-m").'-03 18:00:59';
                if(strtotime($signfor_date)<=strtotime($contractdate)){
                    //签订时间在配置时间前，就是上个月的
                    $isLastMonth=true;
                }else{
                    $isLastMonth=false;
                ...
                if($isLastMonth){
                    ...
                    if($isLastMonth){
                        ...
                        if($adb->num_rows($result)>0){
                            ...
                            //代付款签收时间
                            if(strtotime($workflowstime)<=strtotime($contractdate)){
                                //签订时间在配置时间前，就是上个月的
                                $isLastMonth=true;
                            }else{
                                $isLastMonth=false;
