           ...
           /* while ($dtaRows=$adb->fetch_array($results)){
            ...
            while ($rowDatas=$adb->fetch_array($resultdatas)){
                ...
                if(in_array($rp['servicecontractstype'],array('upgrade')) && $falg){
                    ...
                    $costdeduction=$productdatas['allonecostrenewprice'];
                    $dividecostdeduction=$costdeduction*$scalling/100;
	...
	$staypaymentid = $request->get("staypaymentid");
        ...
        do {
            ...
            if($receivedstatus=='deposit'){
                $sql = "UPDATE vtiger_receivedpayments SET modulename='ServiceContracts',receivedstatus='deposit',ismatchdepart=1,ismanualmatch=1,matchdate='" . date('Y-m-d') . "',relatetoid = ?,newdepartmentid=?,staypaymentid=? WHERE receivedpaymentsid = ?";
                $adb->pquery($sql, array($contractid, $user_departments,$staypaymentid, $receivepayid));
                $modtrackerBasicId = $adb->getUniqueID("vtiger_modtracker_basic");
...
SELECT NULL ,owncompanys,receivedpaymentownid,scalling,servicecontractid,?,?*(scalling/100),'" . date('Y-m-d') . "',signdempart FROM vtiger_servicecontracts_divide WHERE servicecontractid = ? ";
            ...
            $sql = "UPDATE vtiger_receivedpayments SET modulename='ServiceContracts',receivedstatus='normal',ismatchdepart=1,ismanualmatch=1,matchdate='" . date('Y-m-d') . "',relatetoid = ?,newdepartmentid=?,staypaymentid=? WHERE receivedpaymentsid = ?";
