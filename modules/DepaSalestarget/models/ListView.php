...
class DepaSalestarget_ListView_Model extends Vtiger_ListView_Model {

	public function getListViewEntries($pagingModel) {
        $db = PearDatabase::getInstance();

        $moduleName = 'DepaSalestarget';
        ...
        while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['salestargetid'];
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  //20150525 柳林刚 加入
            ...
            $where=getAccessibleUsers('DepaSalestarget','List', true);
            if($where!='1=1'){
            ...
            $listQuery .= ' and vtiger_depasalestarget.createid in ('.implode(',',$where).')';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and vtiger_depasalestarget.createid '.$where . ' ';
            }
        }
