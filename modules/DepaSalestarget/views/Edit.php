<?php
Class DepaSalestarget_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		// 查看 销售目标是否可以修改
		if (!empty($record)) {
			$sql = "select * from vtiger_depasalestarget where salestargetid=? LIMIT 1";
			...
			if ($res_cnt > 0) {
				...
				$sel_result = $db->pquery($sql, array($row['createid']));
				$res_cnt = $db->num_rows($sel_result);
				if ($res_cnt > 0) {
					$row = $db->query_result_rowdata($sel_result, 0);
