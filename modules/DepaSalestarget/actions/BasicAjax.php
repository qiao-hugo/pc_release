...
class DepaSalestarget_BasicAjax_Action extends Vtiger_Action_Controller {
	...
	public function weebSalesSubmit(Vtiger_Request $request) {
		...
		$sql = "update vtiger_depasalestargetdetail set " . implode(',', $tt) . " where salestargetdetailid=?";
	...
	public function isCheckTow(Vtiger_Request $request) {
		...
		$sql = "select * from vtiger_depasalestarget where year=? AND month=? AND department=? ";

		$db=PearDatabase::getInstance();
		$sel_result = $db->pquery($sql, array($year, $month, $department));
		...
		if($res_cnt > 0) {
			...
			if ($row['salestargetid'] == $recordId) {  //可以修改
				...
				$datas['message'] = "该部门".$month."月份周报已存在";
			}
		} 
	...
	public function weebSalesReturn(Vtiger_Request $request) {
		...
		$sql = "SELECT
				...
				FROM
					vtiger_depasalestarget
				LEFT JOIN vtiger_depasalestargetdetail ON vtiger_depasalestarget.salestargetid = vtiger_depasalestargetdetail.salestargetid
				LEFT JOIN vtiger_users ON vtiger_depasalestarget.createid = vtiger_users.id
				WHERE
					vtiger_depasalestargetdetail.salestargetdetailid = ?
				LIMIT 1";
		...
		if ($res_cnt > 0) {
			...
			if ($row['reports_to_id'] == $current_user->id) {
				// 修改
				$sql = "update vtiger_depasalestargetdetail set ismodify=0 where salestargetdetailid=?";
