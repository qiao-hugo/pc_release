...
class DepaSalestarget extends CRMEntity {
	var $log;
	var $db;
	var $table_name = "vtiger_depasalestarget";
    ...
    var $tab_name = Array('vtiger_depasalestarget');
    var $tab_name_index = Array('vtiger_depasalestarget'=>'salestargetid');
	/**
	...
	function DepaSalestarget() {
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('DepaSalestarget');
	}
	...
	*/
	function save_module($module) {
        ...
        if(!empty($_POST['inserti'])){
            ...
            $sql = "select * from vtiger_depasalestarget where salestargetid=?";
            ...
            foreach($_POST['inserti'] as $key=>$value){
                ...
                $tarray['createid']=$current_user->id;
            ...
            // 判断是否已经有超过4条了
            $sql = "select * from vtiger_depasalestargetdetail where salestargetid=?";
			$db=PearDatabase::getInstance();
			$sel_result = $db->pquery($sql, array($record));
			$res_cnt = $db->num_rows($sel_result);
			if ($res_cnt < 4) {
				$sql="INSERT INTO vtiger_depasalestargetdetail(" . implode(',', $keySql) . ") VALUES{$invalue}";
            	$db->pquery($sql, array());
			}
            // 更新创建时间
            $sql = "update vtiger_depasalestarget set createtime=? where salestargetid=?";
        ...
        $sql = "select * from vtiger_depasalestargetdetail where salestargetid=? AND ismodify='1'";
        $db=$this->db;
        $sel_result = $db->pquery($sql, array($recordid));
        $res_cnt = $db->num_rows($sel_result);
        if ($res_cnt > 0) {
            $sql="update vtiger_depasalestarget set ismodify=1 where salestargetid=?";
