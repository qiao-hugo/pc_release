...
class AutoTask_BasicAjax_Action extends Vtiger_Action_Controller
{
    ...
    function copyreceive($mailcopyby,$copygroups,$accountli){
        ...
        if($mailcopyby =='0'){
            ...
            for ($i=0;$i<$adb->num_rows($receivedresult);$i++){
                //$copyid[]=$adb->fetchByAssoc($receivedresult)['userid'];
            ...
            foreach($copygroups as $val2){
              //  echo $val2."<br>";
                if( in_array($val2,$limitedrolea)){
                    ...
                    //$copyid[] = $adb->fetchByAssoc($receivedresult)['serviceid'];
					$aa=$adb->fetchByAssoc($receivedresult);
					$copyid[]=$aa['serviceid'];
                  //  die('客服');
    ...
    function getAudittpl(Vtiger_Request $request)
    {
        ...
        $remarkcommen = $request->get('remarkcommen');  //从客户跟进信息过来的跟进信息
...
//通过$autoworkflowentityid 查找当前前台的任务包id:
    ...
    function submitaudit(Vtiger_Request $request)
    {
        ...
        $crmid = $request->get('crmid'); //合同id;
        $taskremark = $request->get('taskremark'); // 备注内容
        //$autoworkflowid = $request->get('autoflowid'); // 工作流id;
        $autoworkflowid = $request->get('flowid'); // 后台工作流id;
        $autoworkflowtaskid = $request->get('autoflowtaskid'); // 后台任务节点id;
        ...
        $currenttaskprimaryid = $currenttask['autoworkflowtaskentityid']; //前台任务节点id
        $flowentityid = $currenttask['autoworkflowentityid'];//前台工作流id;
        ...
        if (($currenttask['isaction'] !== "2" && $currenttask['process_from'] == "") || $currenttask['isaction'] == "1") { // 只有当前节点是激活状态或者是第一个没有关闭的节点才能审核；
            $saveremarsksql = "UPDATE vtiger_autoworkflowtaskentitys SET mail_report = ?, pauseaudit = ?, taskremark = ?, auditedtime = ?,auditorid=?,isaction=? WHERE autoworkflowid = ? AND autoworkflowtaskid = ? AND crmid=?";
                ...
                if($issendit == "on"){ //如果勾选发送邮件选项；
                   ...
                   // echo $returnmailstring;die;
                    ...
                    if($returnmailstring == "true"){
                       ...
                       $adb->pquery($insertsql,array('young@trueland',$receiveidstring,$copyidstring,$subject,$mailcontext,$userid,$currenttaskprimaryid,$custom_rece,$custom_copy,$module,$moduleid,$date,$sting_file));
                        $maidetail_ID =  $adb->getLastInsertID();
                   }else{
                        $mail_success='2';
                    //邮箱发送失败的处理,待完善
...
//                关闭当前节点,激活下一个节点;
                $saveremarsksql1 = "UPDATE vtiger_autoworkflowtaskentitys SET mail_ID= ?,mail_report = ?, pauseaudit = ?, taskremark = ?, auditedtime = ?,auditorid=?,isaction=? WHERE autoworkflowid = ? AND autoworkflowtaskid = ? AND crmid=?";
                $adb->pquery($saveremarsksql1, array(
                    "$maidetail_ID",
                    "$mail_success",
                    "0",
                    $taskremark,
                ...
                // 审核完成后在这里跟新列表的状态 （可以独立）
                $sel_isaction = "UPDATE vtiger_autoworkflowentitys aa SET taskentityname = ( SELECT GROUP_CONCAT( cc.autoworkflowtaskname SEPARATOR '<br>' ) AS taskname FROM vtiger_autoworkflowtaskentitys cc WHERE cc.autoworkflowentityid = ? AND isaction = '1' ) WHERE aa.autoworkflowentityid = ?";
                $adb->pquery($sel_isaction,array($flowentityid,$flowentityid));
            }
        }
    }
...
//注意这里有一点点修改？？？？                关闭当前节点,激活下一个节点;
            ...
            // 审核完成后在这里跟新列表的状态 （可以独立）
            $sel_isaction = "UPDATE vtiger_autoworkflowentitys aa SET taskentityname = ( SELECT GROUP_CONCAT( cc.autoworkflowtaskname SEPARATOR '<br>' ) AS taskname FROM vtiger_autoworkflowtaskentitys cc WHERE cc.autoworkflowentityid = ? AND isaction = '1' ) WHERE aa.autoworkflowentityid = ?";
            $adb->pquery($sel_isaction,array($autoworkflowentityid,$autoworkflowentityid));
        }
    }
    ...
    public function IDC_follow($accountid){
        global $adb;
        $select_qiantai_tasksql =  "SELECT vtiger_autoworkflowtaskentitys.* FROM vtiger_autoworkflowtaskentitys  WHERE vtiger_autoworkflowtaskentitys.accountid = ? AND vtiger_autoworkflowtaskentitys.isaction = ? AND vtiger_autoworkflowtaskentitys.relationmodule=?";
        $result = $adb->pquery($select_qiantai_tasksql,array($accountid,1,IdcRecords));
        return $adb->fetchByAssoc($result,0);
    }

    //wangbin 客户被客服分配时候关联（生成工作流、以及关联工单、客服分配模块）
    public  function Service_Comments($accountid){
        global $adb;
        $select_qiantai_tasksql = "SELECT vtiger_autoworkflowtaskentitys.* FROM vtiger_autoworkflowtaskentitys  WHERE vtiger_autoworkflowtaskentitys.accountid = ? AND vtiger_autoworkflowtaskentitys.isaction = ? AND vtiger_autoworkflowtaskentitys.relationmodule=?";
        $result = $adb->pquery($select_qiantai_tasksql,array($accountid,1,ServiceAssignRule));
        return $adb->fetchByAssoc($result,0);
    }

    //
    public function  service_assign($accountid){
        //根据合同下的客户信息读取 商务 客服        提单人 等信息
        global $adb;
        $sel_smo_servi_sql = "SELECT vtiger_crmentity.smownerid, ( SELECT vtiger_servicecomments.serviceid FROM vtiger_servicecomments WHERE related_to = ? AND assigntype = 'accountby' LIMIT 1 ) AS service FROM vtiger_crmentity WHERE vtiger_crmentity.crmid = ?";
        $sel_smo_servi_result = $adb->pquery($sel_smo_servi_sql,array($accountid,$accountid));
        if($adb->num_rows($sel_smo_servi_result)>0){
            $temp_smo_ser = $adb->fetchByAssoc($sel_smo_servi_result,0);
            $smownerid = $temp_smo_ser['smownerid']; //客户负责人
            $service =   $temp_smo_ser['service']; //客户跟进的客服
            $update_flowentity_sql = 'UPDATE vtiger_autoworkflowentitys aa SET service = ?,assigner=? WHERE aa.accountid = ? ';
            $adb->pquery($update_flowentity_sql,array($service,$smownerid,$accountid));
            if($service>0){//如果这个客户已经分配了客服，就直接审核掉客服分配这个节点；
                $qiantai_taskArr = AutoTask_BasicAjax_Action::Service_Comments($accountid);
                if(!empty($qiantai_taskArr)){
                    AutoTask_BasicAjax_Action::closeCurrent_openNext($qiantai_taskArr,"");
                }
            }
        }
    }
