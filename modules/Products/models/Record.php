...
class Products_Record_Model extends Vtiger_Record_Model {
	...
	public static  function getRelateProduct($record,$module='Potentials'){
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT vtiger_products.*, vtiger_productcf.notecontent FROM vtiger_products INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid LEFT JOIN vtiger_productcf ON vtiger_products.productid = vtiger_productcf.productid WHERE vtiger_crmentity.deleted = 0 AND vtiger_products.productid IN ( SELECT productid FROM vtiger_seproductsrel WHERE vtiger_seproductsrel.setype = '$module' AND crmid = ?)", array($record));
		$productlist=array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$productid=$db->query_result($result, $i, 'productid');
			$productname=$db->query_result($result, $i, 'productname');
			$notecontent=$db->query_result($result, $i, 'notecontent');
			$realprice=$db->query_result($result, $i, 'realprice');
			$productlist[]=array('productid'=>$productid,'productname'=>$productname,'notecontent'=>$notecontent,'realprice'=>$realprice);
		}
		return $productlist;
	}
	/**
