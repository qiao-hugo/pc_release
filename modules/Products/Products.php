...
class Products extends CRMEntity {
	...
	//var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_products'=>'productid','vtiger_productcf'=>'productid','vtiger_seproductsrel'=>'productid');
    var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_products'=>'productid','vtiger_productcf'=>'productid');
    ...
    var $unit_price;
	var $relatedmodule_list=array('ProductBundles');
	var $relatedmodule_fields=array('Products'=>array('productname'=>'产品名称','product_no'=>'产品编号','realprice'=>'成本','unit_price'=>'市场价','minmarketprice'=>'最低市场价','startdate'=>'开始时间','enddate'=>'结束时间','customer'=>'客服','isdisplay'=>'是否显示'));
	...
	function save_module($module){
	    //2015-07-02 添加产品规格wangbin
	    $productid =  $this->id; //产品id record
	    
	    $standardname = $_REQUEST['standardname'];//规格名称
	    $singleprice= $_REQUEST['singleprice'];//单价
	    $realyprice = $_REQUEST['realyprice'];//市场价
	    $rule = $_REQUEST['rule'];//规格
	    
	   // $deletestandardsql = "DELETE FROM vtiger_products_standard WHERE productid = ?";
        $deletestandardsql = "UPDATE vtiger_products_standard SET `delete`=1 WHERE productid =?";
        $this->db->pquery("$deletestandardsql",array($productid));
	    $insertstandardsql = "INSERT INTO vtiger_products_standard ( productid, standardname, realprice, singleprice, standardvalue) VALUES(?,?,?,?,?)";
	    for ($i=0;$i<count($standardname);++$i){
	        $this->db->pquery("$insertstandardsql",array($productid,$standardname[$i],$realyprice[$i],$singleprice[$i],$rule[$i]));
	    }
	    
	    
	    //2015-07-03 添加产品套餐信息
	    $packageproductids = $_REQUEST['packagepro'];  //套餐内产品id
	    $defaultstand = $_REQUEST['defaultstand'];   //套餐产品的默认规格
        $choosablestand =$_REQUEST['choosablestand'];//可选规格
        $defaultcost = $_REQUEST['defaultcost'];//默认成本
        $years = $_REQUEST['years']; //年限

        $delepackagesql = "DELETE FROM  `vtiger_seproductsrel` WHERE productid = ?";
        $insertpackagesql = "INSERT INTO `vtiger_seproductsrel` (crmid,productid,setype,defaultstand,choosablestand,years,defaultcost) VALUES(?,?,?,?,?,?,?)";
        $this->db->pquery($delepackagesql,array($productid));
        for($j=0;$j<count($packageproductids);$j++){
            //implode("##",$choosablestand[$packageproductids[$j]]);
           // var_dump(array($packageproductids[$j],$productid,"Products",$defaultstand[$j],(implode("##",$choosablestand[$packageproductids[$j]])),$years[$j],$defaultcost[$j]));die;
            $this->db->pquery($insertpackagesql,array($packageproductids[$j],$productid,"Products",$defaultstand[$j],(implode("##",$choosablestand[$packageproductids[$j]])),$years[$j],$defaultcost[$j]));
        }
...
// 		$this->updateUnitPrice();
		...
		if(!empty($_POST['formid'])){
			$this->db->pquery('INSERT INTO `vtiger_customer_modulefields` (`relateid`,`relatedmodule`,`addtime`,`addby`,`formid`) VALUES(?,?,?,?,?) on DUPLICATE key UPDATE formid=?,addtime=?',array($this->id,'Products',date('Ymd'),$current_user->last_name,$_POST['formid'],$_POST['formid'],date('Ymd')));
		}
