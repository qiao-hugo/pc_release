...
class Knowledge_List_View extends Vtiger_List_View {
	...
	function process (Vtiger_Request $request) {
            ...
            $viewer->assign('PUBLIC', $request->get('public'));
            $moduleModel = Vtiger_Module_Model::getInstance($moduleName);//module相关的数据
            $viewer->assign('IS_MANAGER', 0);
            if($moduleModel->exportGrouprt('Knowledge','undercarriage')){   //权限验证
                $viewer->assign('IS_MANAGER', 1);
            }
            $viewer->assign('CATEGORY',Knowledge_Record_Model::getlistCategory());//调用右侧列表
