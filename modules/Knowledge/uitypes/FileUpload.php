<?php
class Knowledge_FileUpload_UIType extends Vtiger_FileUpload_UIType {
        ...
        if(!empty($newvalue[0])){
            ...
            foreach($newvalue as $val){
                ...
                if(count($val)>1){
                    ...
                    $str.='<a href=index.php?module='.$currentModule.'&action=DownloadFile&filename='.urlencode(base64_encode($fileid)).' target="_blank">'.implode('##',$val).'</a><span title="双击进行修改">&nbsp;&nbsp;&nbsp;&nbsp;</span><br>';
                }else{
                    $str.=$val[0];
                }
            }
