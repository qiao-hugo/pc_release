<?php
Class Schoolinterview_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		//$schoolresumeid = $request->get('schoolresumeid');
		$schoolresumeid = $recordModel->entity->column_fields['schoolresumeid'];
		if (!empty($schoolresumeid)) {
			$schoolresume = Schoolresume_Record_Model::getSchoolresumeInfo($schoolresumeid);
			$viewer->assign('SCHOOL_RESUME', $schoolresume);
			$viewer->assign('schoolresumeid', $schoolresumeid);
		}
		/*global $adb;;
		$sql = "SELECT schoolresumeid FROM vtiger_schoolresume WHERE schoolresumeid=?";
		$sel_result = $adb->pquery($sql, array($request->get('schoolresumeid')));
