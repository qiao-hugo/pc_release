...
class ContractsAgreement_List_View extends Vtiger_KList_View {
    function process (Vtiger_Request $request)
    {
        ...
        if($strPublic == 'dempartConfirm') {//非标合同部门负责审核设置
            ...
            if (!$moduleModel->exportGrouprt('ContractsAgreement', 'dempartConfirm')) {   //权限验证
                return;
            }
            $viewer = $this->getViewer($request);
            $viewer->assign('USER', ReceivedPayments_Record_Model::getuserinfo(" AND vtiger_users.`status`='Active'"));
            $viewer->assign('RECOEDS', ServiceContracts_Record_Model::getAuditsettings('ContractsAgreement'));
