 ...
 */
class ContractsAgreement_Record_Model extends Vtiger_Record_Model {
...
/**
    ...
    public function checkAccountid($request){
        ...
        $result=$adb->pquery('SELECT sc_related_to FROM vtiger_servicecontracts WHERE servicecontractsid=?',array($servicecontractsid));
        $sc_related_to=$result->fields['sc_related_to'];
        $false=false;
        if($sc_related_to>0){
            if($sc_related_to!=$accountid){
                $false=true;
            }
        }
        return $false;
    ...
    public function checkOrderIsCancel($request){
        ...
        $db = PearDatabase::getInstance();
        $result = $db->pquery("select ispay,modulestatus,signaturetype,contract_type from vtiger_servicecontracts where servicecontractsid=? ",array($request->get("servicecontractsid")));
        $row = $db->fetchByAssoc($result,0);
        if($row['signaturetype']=='eleccontract' && $row['ispay']==1 && $row['modulestatus']=='c_complete' &&
            $request->get("supplementarytype")=='tovoid' && in_array($row['contract_type'],$configcontracttypeNameTYUN)){
            return true;
        }
        return false;
    ...
    public function isSignContractsAgreement($contractId){
        $db = PearDatabase::getInstance();
        $result = $db->pquery("SELECT
	a.modulestatus,
	a.newservicecontractsid,
	b.modulestatus AS bmodulestatus 
FROM
	vtiger_contractsagreement a
	LEFT JOIN vtiger_servicecontracts b ON a.newservicecontractsid = b.servicecontractsid 
	LEFT JOIN vtiger_crmentity c ON c.crmid = a.contractsagreementid 
WHERE
	a.servicecontractsid =?
    AND c.deleted=0
	AND a.supplementarytype = 'stagepay'",array($contractId));
        if(!$db->num_rows($result)){
            return array("success"=>0,'msg'=>'请先上传分期付款协议');
        }
        while ($row =$db->fetchByAssoc($result)){
            if($row['modulestatus']!='c_complete'){
                return array("success"=>0,'msg'=>'分期付款协议暂未审核通过');
            }
            if(!in_array($row['bmodulestatus'],array('c_complete','c_recovered'))){
                return array("success"=>0,'msg'=>'请先签收或者归还分期付款协议');
            }
        }
        return array("success"=>1,'msg'=>'成功');
    }

    public function recordContractDelaySign($servicecontractsid,$sideagreement,$modulestatus){
        global $adb;
        //记录签收时候
        $oldModuleStatus='';
//        if($sideagreement){
//            $result = $adb->pquery("select a.servicecontractsid,b.modulestatus from  vtiger_contractsagreement a
//  left join vtiger_servicecontracts b on a.servicecontractsid=b.servicecontractsid where a.newservicecontractsid=?",array($servicecontractsid));
//            $row = $adb->fetchByAssoc($result,0);
//            $servicecontractsid = $row['servicecontractsid'];
//            $oldModuleStatus = $row['modulestatus'];
//        }

        //合同签收时 记录合同签收时间
        $result = $adb->pquery("select * from vtiger_contractdelaysign where servicecontractsid=?",array($servicecontractsid));
        if($adb->num_rows($result)){
            $row = $adb->fetchByAssoc($result,0);
            $isdelay=0;
            if(strtotime($row['lastsigndate'])<time()){
                $isdelay = 1;
            }
            if($row['contractsignstatus']=='signed'){
                return;
            }

            if($modulestatus=='c_complete'){
                $sql = "update vtiger_contractdelaysign set signdate=?,isdelay=?,hetongstatus='c_complete',contractsignstatus='signed' where  servicecontractsid=?";
                //签收则修改
                $adb->pquery($sql,array(date("Y-m-d H:i:s"),$isdelay,$servicecontractsid));
