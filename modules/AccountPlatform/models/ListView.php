...
class AccountPlatform_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		if(!empty($specialSearchWhere)){
            $accountplatformids=" AND vtiger_accountplatform.accountplatformid IN(SELECT vtiger_accountplatform_detail.accountplatformid FROM vtiger_accountplatform_detail WHERE 1=1  ".$specialSearchWhere." GROUP BY vtiger_accountplatform_detail.accountplatformid )";
        }
        $listQuery = $this->getQuery();
        $listQuery=str_replace('vtiger_accountplatform.modulestatus,','vtiger_accountplatform.modulestatus,(SELECT vtiger_users.email1 FROM vtiger_users WHERE vtiger_users.id=vtiger_crmentity.smownerid LIMIT 1) as email,',$listQuery);
		...
		while($rawData=$db->fetch_array($listResult)) {
            $rawData['id'] = $rawData['accountplatformid'];
			$listViewRecordModels[$rawData['accountplatformid']] = $rawData;
		}
            ...
            if(isset($BugFreeQuery['BugFreeQuery[queryRowOrder]'])){
                ...
                if(is_array($SearchConditionRow)&&!empty($SearchConditionRow)){
                    foreach($SearchConditionRow as $key=>$val){
                        ...
                        if($searchKey!='department' && !strpos($searchKey,'tiger_accountplatform.accountplatform') && !strpos($searchKey,'tiger_accountplatform.idaccount') ){
                            if(($key+2)==$counts){
            ...
            if(isset($BugFreeQuery['BugFreeQuery[queryRowOrder]'])){
                ...
                if(is_array($SearchConditionRow)&&!empty($SearchConditionRow)){
                    foreach($SearchConditionRow as $key=>$val){
                        ...
                        if(strpos($searchKey,'tiger_accountplatform.accountplatform')){
                            $searchWhere.=" AND vtiger_accountplatform_detail.accountplatform like '%".$searchValue."%'";
                        }
                        if(strpos($searchKey,'tiger_accountplatform.idaccount') ){
                            $searchWhere.=" AND vtiger_accountplatform_detail.idaccount like '%".$searchValue."%'";
                        }
                    }
                }
            }
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if($sourceModule=='RefillApplication'){
            $date=date("Y-m-d");
            $listQuery.=" AND vtiger_accountplatform.modulestatus='c_complete' 
                          AND vtiger_accountplatform.effectivestartaccount>='".$date."' 
                          AND vtiger_accountplatform.effectiveendaccount<='".$date."'";
        }else{
            $accountQuery=' OR EXISTS(SELECT 1 FROM vtiger_crmentity as crmtable WHERE crmtable.crmid=vtiger_accountplatform.accountid AND crmtable.smownerid='.$current_user->id.')';
            if (!empty($searchDepartment) && $searchDepartment != 'H1') {
                $userid = getDepartmentUser($searchDepartment);
                $where = getAccessibleUsers('AccountPlatform', 'List', true);
                if ($where != '1=1') {
    ...
    public function getListViewCount() {
        ...
        $accountplatformids='';
        if(!empty($specialSearchWhere)){
            $accountplatformids=" AND  vtiger_accountplatform.accountplatformid IN(SELECT accountplatformid FROM vtiger_accountplatform_detail WHERE 1=1  ".$specialSearchWhere.")";
        }
        //用户条件
