...
class Vtiger_FieldAjax_View extends Vtiger_Index_View {
	...
	function process (Vtiger_Request $request) {		
		...
		if($mode=='updatesort'){
			...
			if($fieldtype=='reset'){
                $listViewModel->getSelectFields(array(),'reset');
                exit;
            }
			if(!empty($feilds)){
				$feilds=implode(',',$request->get('fieldList'));
		...
		foreach($listViewHeaders as $field){
...
<div> 
    <table class="dialog-table" id="CustomSetTable" style="width:550px"> 
     <tbody> 
      ...
      <tr> 
       <td style="text-align:center" colspan="5"> <input type="submit" style="width:60px" value="确定" /> <input type="reset" style="width:60px" value="还原" /> <br /> </td> 
      </tr> 
     </tbody>
    </table>
   </div>
