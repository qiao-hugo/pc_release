...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function getWorkflows(Vtiger_Request $request){
		...
		foreach($model as $key=>$val){
			...
			$models[$workflowsid.$val['sequence']][$key]=$val;  //将 workflowstagesid 换成 sequence为兼容自动生成的节点没有 workflowstagesid =0；
		}
        /*if($isaction==0){
		...
		$viewer->assign('WORKFLOWSID',$workflowsid);
    ...
    public static function exportGrouprt($module,$classname,$id=0){
        if($id==0)
        {
            global $current_user;
            $id = $current_user->id;
