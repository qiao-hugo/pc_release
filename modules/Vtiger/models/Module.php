...
class Vtiger_Module_Model extends Vtiger_Module {
	...
	//创建模块实体
	public function getCRM_instance(){
		if(empty($this->CRMEntityModule)){
			$this->CRMEntityModule=CRMEntity::getInstance($this->get('name'));
		}
		return $this->CRMEntityModule;
	...
	public function isTrackingEnabled() {
		...
		return ($this->isActive() && $this->get('withupdate'));
	}

	/**
	 * 模块是否可评论(列表已关闭 by Joe)
	 * @return boolean
	 */
	...
	public function deleteRecord(Vtiger_Record_Model $recordModel) {
		...
		$focus = CRMEntity::getInstance($moduleName);
		//1.关联
        //删除前置事件steel 2017-09-01
        if(method_exists($focus, 'deleteBefore')) {
            $focus->deleteBefore($moduleName,$recordModel->getId());
        }
		if(in_array('vtiger_crmentity',$focus->tab_name)){
			$focus->trash($moduleName, $recordModel->getId());
		}else{
        	$focus->real_deleted($moduleName,$recordModel->getId());
        }
         ...
         * 删除后置事件
         */
        if(method_exists($focus, 'deleteAfter')) {
            $focus->deleteAfter($moduleName,$recordModel->getId());
        }
	...
	public function getModuleMeta($userModel = false) {
		...
		if(empty($this->moduleMeta)){
			if(empty($userModel)) {
				$userModel = Users_Record_Model::getCurrentUserModel();//获取当前用户信息，字段，module，entity
			}
	 ...
	 */
	public function getFields($empty=false) {
	...
	//设置当前模块关联菜单
	public function makeRelatedurl($id){
		$CRM_instance=$this->getCRM_instance();
		$relatedmodule_list=$CRM_instance->relatedmodule_list;
		$relationModels = array();
		//$relationModelClassName = Vtiger_Loader::getComponentClassName('Model', 'Relation', $this->get('name'));
		$realoperate='&realoperate='.setoperate($id,$this->get('name'));
		if(!empty($relatedmodule_list)){
			foreach($relatedmodule_list as $relatemodulename) {
				if (isPermitted($relatemodulename,'DetailView')!='yes') {
					continue;
				}
				$relationModels[] = array(
						'linklabel' => $relatemodulename,
						'linkurl' => 'module='.$this->get('name').'&relatedModule='.$relatemodulename.'&view=Detail&record='.$id.'&mode=showRelatedList'.$realoperate,
				);	
			}
		}
	...
	public static function getInstanceFromArray($valueArray) {
		...
		$name=$instance->name;
		/*if($name=='Webmails'){$name="Emails";}$instance->CRMEntityModule=CRMEntity::getInstance($name);*/
		return $instance;
	}
	 ...
	 * 获取crm所有的modules
	 * @param <array> $presence 可以理解为查询条件
	...
	public static function getAll($presence = array(), $restrictedModulesList = array()) {
		$db = PearDatabase::getInstance();
		self::preModuleInitialize2();	// 初始化
        $moduleModels = Vtiger_Cache::get('vtiger', 'modules');   // 命名空间和名称，这里是获取系统自己的modules
        ...
        if(!$moduleModels){
           ...
           /*  $query = 'SELECT * FROM vtiger_tab';
			...
			$presence=changetoarray($presence);
            //$result = $db->pquery($query, $params);
            //$noOfModules = $db->num_rows($result);
           // for($i=0; $i<$noOfModules; ++$i) {
			foreach($modelinfo as $row){
               // $row = $db->query_result_rowdata($result, $i);
			   if(!empty($presence) && !in_array($row['presence'],$presence)){
					continue;
			   }
                $moduleModels[$row['tabid']] = self::getInstanceFromArray($row);
                Vtiger_Cache::set('module',$row['tabid'], $moduleModels[$row['tabid']]);//两种方式表id和module名称
                Vtiger_Cache::set('module',$row['name'], $moduleModels[$row['tabid']]);
            }
			// 如果是有条件的查询，那么将不做整体的缓存替换. @TODO  presence默认传递了0,2不知道是做少的
            if(!$presence){
                Vtiger_Cache::set('vtiger', 'modules',$moduleModels);
            }
        }
		// 进一步判断，读取到的数据是否在presence中，在module缓存之后使用
	...
	public static function getEntityModules() {
		...
		$moduleModels = Vtiger_Cache::get('zdcrm_','EntityModules');
        if(!$moduleModels){
            $presence = array(0, 2);
            ...
            Vtiger_Cache::set('zdcrm_','EntityModules',$moduleModels);
        }
		return $moduleModels;
	}
	...
	/*
	 * 初始化实体字段的缓存，如果为空那么选择所有的，
	 * @TODO 为毛在这里每次都要cache all这个
	 */
	 ...
	 * @return <boolean>
	 */
	public function isPermitted($actionName,$record='') {
		return ($this->isActive() && Users_Privileges_Model::isPermitted($this->getName(), $actionName,$record));
	}
...
//			);
    ...
    */ 
     ...
     * 生成二维码
     */
    public function getQRcode($url,$matrixPointSize = 4,$errorCorrectionLevel = 'L'){
        ob_clean();
     ...
     * @return string
     */
    public function base64encode($v){
        //加入乱字符开始
     ...
     * @return int|mixed
     */
    public function base64decode($v){
    ...
    public function exportGrouprt($module,$classname,$id=0){
        ...
        return $recordModel->personalAuthority($module,$classname,$id);
    }
}
