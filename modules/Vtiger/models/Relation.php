...
class Vtiger_Relation_Model extends Vtiger_Base_Model{
	...
	public static function getAllRelations($parentModuleModel, $selected = true, $onlyActive = true) {
        ...
        $query .= ' AND vtiger_relatedlists.name NOT IN ('.generateQuestionMarks($skipReltionsList).') ORDER BY sequence';
		// TODO: Need to handle entries that has related_tabid 0
//print_r(array($parentModuleModel->getId(), $skipReltionsList));echo $query;die();
