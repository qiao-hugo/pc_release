...
class Vtiger_Link_Model extends Vtiger_Link {
	 ...
	 * @return Vtiger_Link_Model instance
	 */
	public static function getInstanceFromValues($valueMap,$public='') {
		$linkModel = new self();
        ...
        $linkurls = explode("public=",$valueMap['linkurl']);
        if(count($linkurls)>1){
            $linkurl = explode("&",$linkurls[1]);
            if($linkurl[0]==$_REQUEST['public']){
                $linkModel->linklabel = '<div style="border-bottom: 1px solid #006FB6;color: dodgerblue">'.$valueMap['linklabel']."</div>";
            }
        }
