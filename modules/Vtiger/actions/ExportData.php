...
class Vtiger_ExportData_Action extends Vtiger_Mass_Action {
	...
	function ExportData(Vtiger_Request $request) {
        ...
        /*
			unset($translatedHeaders[7]);
			unset($translatedHeaders[9]);
			unset($translatedHeaders[12]);
			unset($translatedHeaders[14]);
			unset($translatedHeaders[15]);
			unset($translatedHeaders[17]);
			unset($translatedHeaders[19]);
        */
			for($i=0;$i<count($entries);$i++){
				
				foreach($entries[$i] as $key=>$v){
					if($key=='accountcategory'||$key=='gender' || $key=='accountrank'|| $key=='makedecision' || $key=='leadsource' || $key=='industry'){
						$entries[$i][$key]=vtranslate($entries[$i][$key],$moduleName,'zh_cn');
					}
					
					if($key=='parentid')
					$entries[$i][$key]=ltrim($entries[$i][$key],'Accounts::::');
					if($key=='servicetype'){
						$temp=str_replace(' |##| ',',',$entries[$i][$key]);
						$result1= $db->run_query_allrecords("SELECT productname FROM vtiger_products WHERE productid in({$temp})");
						if(!empty($result1)){
							$string='';
							foreach($result1 as $v)
							$string.=$v['productname'].',';
							$entries[$i][$key]=rtrim($string,',');
						}
	...
	function getExportQuery(Vtiger_Request $request) {
		...
		if($request->get('source_module')=='Accounts'){
			//young.yang 2015-2-1 bug#7685
			$where=getAccessibleUsers($request->get('source_module'),'Export');
			
			if($where!='1=1'){
				$query .= ' and vtiger_crmentity.smownerid '.$where.' AND vtiger_account.accountcategory=0 ';
			}	
	...
	function sanitizeValues($arr){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				if($uitype == 15||$uitype == 33 || $uitype == 16 || in_array($value,$this->picklistValues[$fieldname])){
