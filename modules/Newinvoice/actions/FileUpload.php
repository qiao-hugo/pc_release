...
class Newinvoice_FileUpload_Action extends Vtiger_Save_Action {


	public function process(Vtiger_Request $request) {
	    $colum='file';
        $extendId=$request->get('extend');
	    if(isset($_FILES['Zizhifile'])){
            $files=$_FILES['Zizhifile'];
            $colum='zizhifile';
        }else if(isset($_FILES['Invoicefile'])){
            $files=$_FILES['Invoicefile'];
            $colum='invoicefile';
        }else if(isset($_FILES['extendfile'.$extendId])){
            $files=$_FILES['extendfile'.$extendId];
            $this->fileUpExtend($request,$files);
        }else{
            $files=$_FILES['File'];
        }
	...
	//发票红冲附件的上传
    public function fileUpExtend($request,$files){
        $model='newinvoiceextend';
        $record=$request->get('record');
        $extendId=$request->get('extend');
        if($files['name'] != '' && $files['size'] > 0){
            global $current_user;
            ...
            $result = $adb->pquery($sql2, $params2);
            //防止上传后不保存不管保存与否都同步当前记录里
            if(!empty($record)&& $record>0&&$extendId&&$extendId){
                $extendSql="select * from vtiger_newinvoiceextend where invoiceid=? and invoiceextendid=?";
                $result=$adb->pquery($extendSql,array($record,$extendId));
                $fileStr=$adb->query_result($result,0,'file');
                $str=$fileStr.'*|*'.$filename.'##'.$current_id;
                $str=ltrim($str,'*|*');
                $sql="UPDATE vtiger_newinvoiceextend SET file=? WHERE invoiceextendid=?";
                $adb->pquery($sql,array($str,$extendId));
            }
            echo json_encode(array('success'=>true,'result'=>array('id'=>base64_encode($current_id),'name'=>$filename)));
        }else{
            echo json_encode(array('success'=>false));
        }
        exit;
