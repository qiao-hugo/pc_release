...
class Newinvoice_DeleteFile_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
        ...
        if($extendId){
            $this->deleteFileExtend($fileid,$recordid,$extendId,$moduleName);
        }
        //echo $fileid;
		if($fileid>0){
     ...
     * @throws AppException
     */
	public function deleteFileExtend($fileid,$recordid,$extendId,$moduleName){
        if($fileid>0){
            global $adb,$current_user;
            $result = $adb->pquery("SELECT * FROM vtiger_files WHERE delflag=0 AND attachmentsid=?", array($fileid));
            if($adb->num_rows($result)) {
                $fileDetails = $adb->query_result_rowdata($result);
                $where=getAccessibleUsers($moduleName,'List',true);
                if(!empty($where)&&$where!='1=1'){
                    if(!in_array($fileDetails['uploader'],$where)){
                        echo json_encode(array('success'=>false,'result'=>array('msg'=>'没有权限')));
                        exit;
                ...
                if(unlink($filePath.$savedFile)){
                    $adb->pquery("UPDATE vtiger_files SET deleter=?,deletertime=?,delflag=? WHERE attachmentsid=?", array($current_user->id,date("Y-m-d H:i:s"),'1',$fileid));
                    if(!empty($recordid)&& $recordid>0&&$extendId&&$extendId>0){
                        $extendSql="select * from vtiger_newinvoiceextend where invoiceid=? and invoiceextendid=?";
                        $result=$adb->pquery($extendSql,array($recordid,$extendId));
                        $fileStr=$adb->query_result($result,0,'file');
                        if($fileStr){
                            $newfileone=explode('*|*',$fileStr);
                            foreach($newfileone as $key=>$val){
                                $newfiletwo=explode('##',$val);
                                if(in_array($fileid,$newfiletwo)){
                                    unset($newfileone[$key]);
