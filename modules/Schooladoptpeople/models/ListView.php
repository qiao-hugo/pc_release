...
class Schooladoptpeople_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel,$request=array()) {
		...
		$listQuery = preg_replace('/SELECT/', 'SELECT vtiger_schooladoptpeople.schooladoptid AS schooladoptid, ', $listQuery, 1);
        //echo $listQuery;die();
		//echo $listQuery;die;
		...
		while($rawData=$db->fetch_array($listResult)) {
			//print_r($rawData);
            $rawData['id'] = $rawData['schooladoptpeopleid'];
			$listViewRecordModels[$rawData['schooladoptpeopleid']] = $rawData;
		}
    ...
    public function getUserWhere(){
       global $current_user;
        ...
        if(!empty($searchDepartment)&&$searchDepartment!='H1'){  
            ...
            $listQuery .= ' and ( vtiger_schooladoptpeople.instructor in ('.implode(',',$where).') OR vtiger_schooladoptpeople.p_chargeperson IN ('.implode(',',$where).') )';
        }else{
            $where=getAccessibleUsers();
            if($where!='1=1'){
                //$listQuery .= ' and vtiger_visitingorder.extractid '.$where;
                //$listQuery .= ' and vtiger_crmentity.smownerid '.$where;
                $listQuery .= ' and ( vtiger_schooladoptpeople.instructor '.$where . ' OR vtiger_schooladoptpeople.p_chargeperson '.$where . ' ) ';
            }
        }
