...
class Vtiger_Functions {
	...
	protected static function getCurrencyInfo($currencyid) {
		return array('id' =>1,'currency_name'=>'China, Yuan Renminbi','currency_code' => 'CNY','currency_symbol' =>'¥','conversion_rate' => '1.00000','currency_status'=> 'Active','defaultid' => -11,'deleted' => 0);
		global $adb;
	...
	//实体表，里面的都有自己的数据库表结构 vtiger_entityname
	static function getEntityModuleInfo($mixed) {
		$name = NULL;
		if (is_numeric($mixed)) $name = self::getModuleName ($mixed);
		else $name = $mixed;
		$moduleEntityCache=Vtiger_Cache::get('global', 'getEntityModuleInfo');
		
		if ($name && empty($moduleEntityCache)) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from vtiger_entityname', array());
			$temp=array();
			while ($row = $adb->fetch_array($result)) {
				$temp[$row['modulename']] = $row;
			}
			Vtiger_Cache::set('global', 'getEntityModuleInfo', $temp);
			//unset($temp);
			//$moduleEntityCache=Vtiger_Cache::get('zdcrm_', 'entity');
            $moduleEntityCache=Vtiger_Cache::get('global', 'getEntityModuleInfo');
		}

		return isset($moduleEntityCache[$name])?
			$moduleEntityCache[$name] : NULL;
	}
	...
	static function getEntityModuleInfoFieldsFormatted($mixed) {
		...
		$info['fieldname'] = $fieldnames;//化成array
		return $info;
	}
    ...
    *
    **/
    static function getProductsRecordLabels($ids) {
		if (!is_array($ids)) $ids = array($ids);

		$nameList = array();
		if ($ids) {
			$nameList = self::getCRMRecordLabels('Products', $ids);
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
                ...
                //20150502 young 将详细页面加入人员+部门+在职状态
                if('Users'==$module){
                    $sql = sprintf('SELECT CONCAT( last_name, \'[\', IFNULL(( SELECT departmentname FROM vtiger_departments WHERE departmentid = ( SELECT departmentid FROM vtiger_user2department WHERE userid = vtiger_users.id LIMIT 1 )), \'\' ), \']\', ( IF ( `status` = \'Active\', \'\', \'[离职]\' ))) AS label, %s AS id FROM %s WHERE %s IN (%s)',
