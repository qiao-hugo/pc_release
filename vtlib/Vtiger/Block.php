...
class Vtiger_Block {
	...
	static function getAllForModule($moduleInstance) {
		global $adb;
		$instances = false;
		$query = "SELECT * FROM vtiger_blocks WHERE tabid=? AND visible=0 ORDER BY sequence";
		$queryParams = Array($moduleInstance->id);
		$result = $adb->pquery($query, $queryParams);
		for($index = 0; $index < $adb->num_rows($result); ++$index) {
			$instance = new self();
			$blockinfo=$adb->fetch_array($result);
			$instance->initialize($blockinfo, $moduleInstance);
			$instances[$blockinfo['blocklabel']] = $instance;
		}
		return $instances;
	}
