...
if(!empty($_GET['tokenauth'])){
    $ReadWriteDatabase=array(
        'accountAllot',
        'addModComment',
        'addAccountContactsToCrm',
        'addRejectContractToCrm',
        'saveAccountRemarkToCrm',
        'saveAccountContactsTagToCrm',
        'cslogin'
    );//读写数据库操作 //如果有写操作请加入到数组中
    $cs_method = explode(',',urldecode(cookiecodeloading($_GET['tokenauth'],'DECODE')));
    if(!in_array($cs_method[0],$ReadWriteDatabase)){//读写数据库的选择
        define("SECOND_DATABASE",1);
    }
}
...
if(is_array($cs_method) && $cs_method[0]=='cslogin') {
	...
	_cs_logs(array("客服登录用户:".$user_name.'|'.$password));
...
if(is_array($cs_method) && $cs_method[0]=='listAccountContacts'){
    ...
    $query="SELECT accountid contactid,
			...
			0 AS leave_office,
			1 type
			FROM vtiger_account WHERE accountid=?";
    $query.=" UNION ALL ";
    ...
    $result=$adb->pquery($query,array($accountid,$accountid));
    $rows = $adb->num_rows($result);
    $arr_contacts=array();
...
if(is_array($cs_method) && $cs_method[0]=='searchAccountList'){
    ...
    $allotBeginTime=$decodedata['allotBeginTime'];
    $allotEndTime=$decodedata['allotEndTime'];
    $transferBeginTime=$decodedata['transferBeginTime'];
    $transferEndTime=$decodedata['transferEndTime'];
    $operationType=$decodedata['operationType'];
    if(empty($data['operationType'])&&$data['operationType']!==0){
    ...
    $pageNum = $p_num * $pageSize;
    _cs_logs(array("查询客户列表接口解析后参数:".json_encode($decodedata)));
    ...
    $query="SELECT TT.*,
            ...
            (SELECT date_format(transfertime,'%Y-%m-%d %H:%i:%S') FROM `vtiger_account_allot_other` WHERE accountid=TT.accountid AND source_type=1) as transfertime_other,
            (SELECT date_format(last_update_time,'%Y-%m-%d %H:%i:%S') FROM `vtiger_account_allot_other` WHERE accountid=TT.accountid AND source_type=1) as allottime_other,
			...
			IFNULL((select departmentname from vtiger_departments where departmentid = (select departmentid FROM vtiger_user2department where userid=TT.serviceid LIMIT 1)),'--') as service_departmentname
			FROM (";
    ...
    if($operationType==1){
        $query .= " AND vtiger_account.transfertime between '".$transferBeginTime."' and '".$transferEndTime."'";
    }else if($operationType===0){
        $query .= " AND vtiger_account.allottime between '".$allotBeginTime."' and '".$allotEndTime."'";
    }
    ...
    if($operationType==1){
        $query_total .= " AND vtiger_account.transfertime between '".$transferBeginTime."' and '".$transferEndTime."'";
    }else if($operationType===0){
        $query_total .= " AND vtiger_account.allottime between '".$allotBeginTime."' and '".$allotEndTime."'";
    }
    ...
    if ($rows>0) {
        while($row=$adb->fetchByAssoc($result)) {
			...
			//负责客服部门
            $lists['service_departmentname']=$row['service_departmentname'];
            ...
            if($sourceType == 1){
                $lists['transfertime']=$row['transfertime_other'];
            }else{
                $lists['transfertime']=$row['transfertime'];
            }
            //分配时间
            if($sourceType == 1){
...
if(is_array($cs_method) && $cs_method[0]=='searchAllotAccountList'){
    ...
    $pageNum = $p_num * $pageSize;
    _cs_logs(array("查询分配客户列表接口解析后参数:".json_encode($decodedata)));
    ...
    $query="SELECT TT.*,
			...
			(SELECT date_format(transfertime,'%Y-%m-%d %H:%i:%S') FROM `vtiger_account_allot_other` WHERE accountid=TT.accountid AND source_type=1) transfertime_other,
    ...
    $query.="SELECT DISTINCT
			...
			(SELECT contract_no FROM vtiger_servicecontracts WHERE sc_related_to=C.sc_related_to AND signdate=C.signdate LIMIT 1) as contract_no,
			C.total,
			C.signfor_date2,
			(select SUM(unit_price) from vtiger_receivedpayments where relatetoid=C.servicecontractsid AND ismatchdepart=1) sumprice,
			D.departmentid,
			vtiger_crmentity.description
			FROM vtiger_account 
			JOIN vtiger_crmentity ON vtiger_account.accountid = vtiger_crmentity.crmid
			JOIN (SELECT DISTINCT sc_related_to,MAX(signdate) signdate,total,servicecontractsid,MAX(signfor_date) as signfor_date2 FROM vtiger_servicecontracts WHERE modulestatus IN ( 'c_complete', 'c_history') GROUP BY sc_related_to) C ON(vtiger_account.accountid=C.sc_related_to)
			LEFT JOIN vtiger_user2department D ON(vtiger_crmentity.smownerid=D.userid)		
			WHERE vtiger_crmentity.deleted=0
			AND vtiger_account.accountflag!=1";
	if($allotType == '1') {
		...
		if(!empty($serviceStatus)){
			if($serviceStatus == '1') {
                $query .= " AND EXISTS(select 1 from vtiger_users where vtiger_account.serviceid=vtiger_users.id and isdimission=0)";
			}
			if($serviceStatus == '2') {
                $query .= " AND EXISTS(select 1 from vtiger_users where vtiger_account.serviceid=vtiger_users.id and isdimission=1)";
			}
		}
	}else if($allotType == '3'){
        //客服在职状态 2021.5.19加入全部
        if($sourceType == 1){
            $query .= " AND (vtiger_account.accountid IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1) or (vtiger_account.accountid NOT IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1) AND vtiger_account.accountid IN(SELECT DISTINCT customerid FROM vtiger_activationcode WHERE productclass=1 and `status` IN(0,1) AND customerid IS NOT NULL)))";
    ...
    $query_total="SELECT count(DISTINCT accountid) AS cnt FROM (
			...
			FROM vtiger_account 
			JOIN vtiger_crmentity ON vtiger_account.accountid = vtiger_crmentity.crmid
			JOIN (SELECT sc_related_to,MAX(signdate) signdate,total,servicecontractsid,MAX(signfor_date) as signfor_date2 FROM vtiger_servicecontracts WHERE modulestatus IN ( 'c_complete', 'c_history') GROUP BY sc_related_to) C ON(vtiger_account.accountid=C.sc_related_to)
			LEFT JOIN vtiger_user2department D ON(vtiger_crmentity.smownerid=D.userid)	
			WHERE vtiger_crmentity.deleted=0
			AND vtiger_account.accountflag!=1";
	if($allotType == '1') {
		...
		if(!empty($serviceStatus)){
			if($serviceStatus == '1') {
				$query_total .= " AND EXISTS(select 1 from vtiger_users where vtiger_account.serviceid=vtiger_users.id and isdimission=0)";
			}
			if($serviceStatus == '2') {
				$query_total .= " AND EXISTS(select 1 from vtiger_users where vtiger_account.serviceid=vtiger_users.id and isdimission=1)";
			}
		}
	}else if($allotType == '3') {
        //客服在职状态 2021.5.19加入全部
        if($sourceType == 1){
            $query_total .= " AND (vtiger_account.accountid IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1) or (vtiger_account.accountid NOT IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1) AND vtiger_account.accountid IN(SELECT DISTINCT customerid FROM vtiger_activationcode WHERE productclass=1 and `status` IN(0,1) AND customerid IS NOT NULL)))";
	...
	if(count($allotSmownerIds)>0){
		if($isChiefInspector == '1'){
            ...
            if($allotType == '3'){
                if($sourceType == 1){
                    $query_where .= " AND ((MM.serviceid IS NULL and MM.signfor_date2>='2020-01-01 00:00:00') OR (MM.serviceid IS not NULL))";
                }else{
                    $query_where .= " AND ((MM.serviceid IS NULL and MM.signfor_date2>='2020-01-01 00:00:00') OR (MM.serviceid IS not NULL))";
                }
            }
			...
			if(count($assign_serviceids)>0 && $allotType == '2') {
				//$query_where .= " AND (vtiger_crmentity.smownerid IN (".implode(',',$all_userid).") OR vtiger_account.serviceid IN({$assign_serviceids}))";
				if($sourceType == 1){
					$query_where .= " AND (MM.accountid IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1 AND csid IN(".implode(',',$assign_serviceids).")))";
			    }else{
					$query_where .= " AND (MM.serviceid IN(".implode(',',$assign_serviceids)."))";
				}
			}else if($allotType == '1'){
				//$query_where.= " AND vtiger_crmentity.smownerid IN (".implode(',',$all_userid).")";
                ...
                if($sourceType == 1){
                    $query_where .= " AND ((MM.smownerid IN (".implode(',',$allotSmownerIds).") and MM.serviceid IS NULL and MM.signfor_date2>='2020-01-01 00:00:00') OR (MM.serviceid IS not NULL and  MM.accountid IN(SELECT accountid FROM `vtiger_account_allot_other` WHERE source_type=1 AND csid IN(".implode(',',$assign_serviceids)."))))";
                }else{
                    $query_where .= " AND ((MM.smownerid IN (".implode(',',$allotSmownerIds).") and MM.serviceid IS NULL and MM.signfor_date2>='2020-01-01 00:00:00') OR (MM.serviceid IS not NULL and MM.serviceid IN(".implode(',',$assign_serviceids).")))";
                }
            }
		}
	}else{
    ...
    if($allotType=='1'){
        //未分配必须大于2021年
        $query_where.=" and MM.signfor_date2>='2020-01-01 00:00:00' ";
    }

    if($moduleName == 'AssignAccount'){
        $query.= $query_where." ORDER BY MM.signfor_date2 DESC LIMIT {$pageNum},{$pageSize}";
    }else{
        $query.= $query_where." ORDER BY MM.signfor_date2 DESC,MM.allottimestamp DESC,MM.accountid DESC LIMIT {$pageNum},{$pageSize}";
    }
    ...
    if ($rows>0) {
        while($row=$adb->fetchByAssoc($result)) {
            ...
            $lists['sumprice']=$row['sumprice'];
            $lists['rate']=bcdiv($lists['sumprice'],$lists['total'],4);
            if($sourceType == 1){
                $lists['transfertime']=$row['transfertime_other'];
            }else{
                $lists['transfertime']=$row['transfertime'];
...
//=================查询分配渠道甩单客户列表API================================================================
if(is_array($cs_method) && $cs_method[0]=='searchAllotQdcsdAccountList'){
    _cs_logs(array("查询分配渠道甩单客户列表接口开始"));
    global $adb;
    $reusltdata = $_REQUEST["data"];
    _cs_logs(array("查询分配渠道甩单客户列表接口参数:".$resultdata));
	$decodedata = json_decode($reusltdata, true);
    $search_field=$decodedata['searchField'];
    $search_value=$decodedata['searchValue'];
    $search_type=$decodedata['searchType'];
    ...
    $pageNum = $p_num * $pageSize;
    _cs_logs(array("查询分配渠道甩单客户列表接口解析后参数:".$decrdata));
    _cs_logs(array("查询分配渠道甩单客户等级:".$isIroncard));
    include_once 'vtlib/Vtiger/Module.php';
    include_once 'includes/main/WebUI.php';
    include_once  'languages/zh_cn/Accounts.php';
    vglobal('default_language', $default_language);
    ...
    $query.="SELECT DISTINCT
			...
			JOIN vtiger_crmentity ON vtiger_account.accountid = vtiger_crmentity.crmid 
			LEFT JOIN vtiger_user2department D ON(vtiger_crmentity.smownerid=D.userid)
			WHERE vtiger_crmentity.deleted=0 AND vtiger_account.accountflag=1";
	$query .= " AND vtiger_account.accountrank NOT IN('chan_notv','iron_isv')";	
	if($allotType == '1') {
        //未分配客服的客户-查询负责客服为空，且没有回款不足的客户
		if($sourceType == 1){
    ...
    if ($rows>0) {
        ...
        //总件数取得
        _cs_logs(array("执行分配渠道甩单客户统计总件数sql：".$query_total));
        $result_total = $adb->pquery($query_total, array());
        $row_total = $adb->query_result_rowdata($result_total);
        $total = $row_total["cnt"];
        $total = empty($total)?0:$total;
        _cs_logs(array("获取分配渠道甩单客户总件数：".$total));

        $data=json_encode($ret_lists,JSON_UNESCAPED_UNICODE);
        $data=encrypt($data);
        echo json_encode(array("success"=>true,"is_multiple"=>true,"total"=>$total,"pageNum"=>$pageNum,"pageSize"=>$pageSize,"data"=>$data));
        _cs_logs(array("查询渠道甩单客户列表信息成功"));
    }else{
        _cs_logs(array("没有查询到相关信息"));
        /*_cs_logs(array("没有查询到相关信息"));
        $lists=array('success'=>false,'msg'=>'没有客户的相关信息',JSON_UNESCAPED_UNICODE);
    ...
    _cs_logs(array("查询分配渠道甩单客户列表信息接口结束"));
    exit;
}
//=================查询全部待分配客户API================================================================
...
//=================查询待分配渠道甩单客户数量API================================================================
if(is_array($cs_method) && $cs_method[0]=='searchWaitAllotQdcsdAccountCount'){
    _cs_logs(array("查询待分配渠道甩单客户数量接口开始"));
    global $adb;
    $reusltdata = $_REQUEST["data"];
    _cs_logs(array("查询待分配渠道甩单客户数量接口参数:".$resultdata));
    $reusltdata = trim($reusltdata);
    $reusltdata = str_replace(' ', '+', $reusltdata);
    $decrdata = decrypt($reusltdata);
    $pad = strrpos($decrdata, '}');
    ...
    _cs_logs(array("查询待分配渠道甩单客户数量接口解析后参数:".$decrdata));
...
$query_total="SELECT count(DISTINCT accountid) AS cnt FROM (
			...
			JOIN vtiger_crmentity ON vtiger_account.accountid = vtiger_crmentity.crmid 
			LEFT JOIN vtiger_user2department D ON(vtiger_crmentity.smownerid=D.userid)
			WHERE vtiger_crmentity.deleted=0 AND vtiger_account.serviceid IS NULL AND vtiger_account.accountflag=1";
	$query_total .= " AND vtiger_account.accountrank NOT IN('chan_notv','iron_isv')";
	$query_total .= ") MM WHERE 1=1 ";
    $query_where = " AND MM.serviceid IS NULL";
    if(!empty($accountids)){
    ...
    $query_total.=$query_where;
    _cs_logs(array("查询待分配渠道甩单客户数量执行查询sql：".$query_total));
    ...
    $total = $row_total["cnt"];
    $total = empty($total)?0:$total;
    _cs_logs(array("获取待分配渠道甩单客户数量：".$total));

    $ret_lists = array();
    $data=json_encode($ret_lists,JSON_UNESCAPED_UNICODE);
    $data=encrypt($data);
    echo json_encode(array("success"=>true,"total"=>$total,"data"=>$data));
    _cs_logs(array("查询待分配渠道甩单客户数量成功"));

    _cs_logs(array("查询待分配渠道甩单客户数量信息接口结束"));
    exit;
}
...
if(is_array($cs_method) && $cs_method[0]=='searchPaymentAccountList'){
    ...
    $decodedata = json_decode($decrdata, true);
    $accountid=$decodedata['accountid'];
    $accountids=$_REQUEST['accountIds'];
    _cs_logs(array("查询回款不足客户接口解析后参数:".json_encode($decodedata)));
    _cs_logs(array("查询回款不足客户接口解析后参数1:".json_encode($_REQUEST)));
    ...
    //2021.5.16 将30%的判断改为50%
    //2021.06.03 将50%的判断改为10%
    $query="SELECT DISTINCT M.accountid,M.servicecontractsid,M.msg,(SELECT accountname FROM vtiger_account WHERE accountid=M.accountid) AS accountname
			...
			UNION ALL
			SELECT T1.sc_related_to AS accountid,T1.servicecontractsid,'回款不足:T云WEB合同 、回款小于合同金额10%' AS msg FROM(
			...
			) T1 WHERE T1.payment/T1.total<0.1
			UNION ALL
    ...
    if(!empty($accountids)){
        $query .= " AND M.accountid in ({$accountids})";
    }
            ...
            if(count($arr_account)>0){
                ...
                for($i=0;$i<count($arr_account);$i++){
					...
					if($sourceType == 1){
                        ...
                        //2021.5.19新如果客服是转移的，那就更新转移时间
                        if($operationType){
                            $update_sql = "UPDATE vtiger_account_allot_other SET transfertime=NOW() WHERE id=?";
                            $adb->pquery($update_sql, array($tmp_row['id']));
                        }
						...
						//更新客户表客服字段
                        //2021.5.19新如果客服是转移的，那就更新转移时间
                        $operationUpdate=',allottime=NOW(),allottimestamp=UNIX_TIMESTAMP(NOW())';
                        if($operationType){
                            $operationUpdate=",transfertime=NOW()";
                        }
						$update_account_Sql = "update vtiger_account set serviceid=?".$operationUpdate." where accountid=?";
						$parm_array=array();
						$parm_array[]=$arr_servicecomments['serviceid'];
						$parm_array[]=$arr_servicecomments['related_to'];
						...
						//更新分配记录表
						$insertRuleSql = "insert into vtiger_serviceassignrule(assigntype,related_to,departmentid,productid,serviceid,ownerid,creatorid,createdtime)
										values('accountby',".$arr_servicecomments['related_to'].",'H1',182,".$arr_servicecomments['serviceid'].",0,".$arr_servicecomments['assignerid'].",NOW())";
						$adb->pquery($insertRuleSql, array());
					}
                }
...
if(is_array($cs_method) && $cs_method[0]=='searchCustomServiceDepartments'){
	global $adb;
	$depth = $_GET['depth'];
	$query_sql = "SELECT departmentid,parentdepartment,departmentname,SUBSTRING_INDEX(SUBSTRING_INDEX(parentdepartment,'::',-2),'::',1) AS parentid FROM vtiger_departments WHERE departmentname like '%客服%' or departmentname like '%客户成功部%'";
	$result = $adb->pquery($query_sql, array());
    $rows = $adb->num_rows($result);
    $ret_lists = array();
    if ($rows>0) {
 ...
 * @return bool|string
 */
function cookiecodeloading($string, $operation = 'DECODE', $key = '', $expiry = 0) {
    $ckey_length = 4;
