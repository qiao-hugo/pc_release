; (function (g, fn) {
    ...
    Pdfh5.prototype = {
                    ...
                    promise = promise.then(function (pageNum) {
                        return self.thePDF.getPage(pageNum).then(function (page) {
                            ...
                            if (window.ActiveXObject || "ActiveXObject" in window) {
                                $(container).css({
                                    'width': options.width ? options.width:viewport.width + 'px',
                                    "height": options.height ? options.height:viewport.height + 'px'
                                ...
                                if(options.width && options.height){
                                    ...
                                    }).attr("data-scale", options.width / options.height)
                                }else{
                                    $(container).css({
        ...
        renderCanvas: function (page, viewport, pageNum, num, container, options) {
            ...
            canvas.style.width = options.width + 'px' || null;
            canvas.style.height = options.height + 'px' || null;
            ...
            return page.render({
                ...
                $(container).find('.loadEffect').remove();
                img.onload = function () {
                    $(container).css({
                        "min-height": options.height?options.height:img.height + 'px'
                    })
                }
