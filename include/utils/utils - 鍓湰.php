...
function get_username_array($where,$status='Active'){
		...
		$result=array();
		global $current_user;
		if($current_user->id==1121||$current_user->id==1){
			$query = "SELECT id, CONCAT( last_name, '[', IFNULL(( SELECT departmentname FROM vtiger_departments WHERE departmentid = ( SELECT departmentid FROM vtiger_user2department WHERE userid = vtiger_users.id LIMIT 1 )), '' ), ']', IF ( `status` = 'Active', '', '[离职]' )) AS last_name FROM vtiger_users ";
        
			$result = $db->pquery($query,array());  //过滤掉离职信息
		}else{
...
function to_html($string, $encode=true){
	...
	if($_REQUEST['module'] != 'Settings' && $file != 'ListView' && $_REQUEST['module'] != 'Portal' && $_REQUEST['module'] != "Reports")// && $_REQUEST['module'] != 'Emails')
...
function uitypeformat($row,$entry,$module,$length=20){
    ...
    if(in_array($uitypeid,array(15,16,151))){ //下拉项需要翻译
        if($row['columnname']=='modulestatus'){
           /* if($value=='a_normal'){
    ...
    if(in_array($uitypeid,array(28,61,153))){  //附件
    ...
    if($row['relfieldmodule']=='Accounts'){ //关于人员的
    ...
    if($row['relfieldmodule']=='ServiceContracts'){ //关于人员的
