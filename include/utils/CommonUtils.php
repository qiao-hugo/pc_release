...
function getProductsList($idList) {
	return Vtiger_Functions::getProductsRecordLabels($idList);
}
/**
...
function do_curl($url, $params = array(), $type = 'POST',$userId)
{
    $headerArray = array("Content-type:application/json;","Accept:application/json","userId:".$userId);
    $method = strtoupper($type);
    if ($method == 'GET') {
        $url = "{$url}?" . http_build_query($params);
    }
    $ch = curl_init();
    // $header[] = "User-Agent:Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.140 Safari/537.36";
    // //定义请求类型
...
/**
 * @desc   wordToPdf         将word转换为pdf
 * @param  string    $wordPath    word原文件路径
 * @param  string    $outPath     pdf输出路径
 * @return string
 */
function wordToPdf($filenamedoc, $filenamepdf)
{
