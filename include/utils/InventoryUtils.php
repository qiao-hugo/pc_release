...
{
	//young.yang 2014-11-12 库存相关统统不要了
	/* global $log;
...
/**
 * 保存工单的流程阶段数据   
 * @author young.yang 2014-11-12
 * @param unknown $wordflowsid
 * @param unknown $crmid
 */
function saveSalesOrderWorkflowStages($wordflowsid,&$focus){
	global $log, $adb;
	//1.判断是否已经有流程,如果已经含有了相同的
	
	$sql="select workflowsid,group_concat(sequence order by isaction desc) as sequence from vtiger_salesorderworkflowstages where salesorderid=".$focus->id." group by workflowsid limit 1";
	$result=$adb->pquery($sql,array());
	
	$oldworkflowsid=$adb->query_result($result, 0,'workflowsid');
	$sequence=$adb->query_result($result, 0,'sequence');
	//节点开始了就不能在操作这些数据
	if(!empty($sequence)&&$sequence!=1){
		return;
	}
	
	if(empty($oldworkflowsid)||(!empty($oldworkflowsid)&&$oldworkflowsid!=$wordflowsid)){
		//2.如果老的流程和新的流程不同，则删除老的流程
		if(!empty($oldworkflowsid)&&$oldworkflowsid!=$wordflowsid){
			$adb->pquery("delete from vtiger_salesorderworkflowstages where salesorderid=?",array($focus->id));
		}
		$crmid=$focus->id;
		
		$query ="insert into vtiger_salesorderworkflowstages( workflowstagesname, workflowstagesid, sequence,salesorderid, addtime,workflowsid) select workflowstagesname,workflowstagesid,sequence,$crmid,NOW(),".$wordflowsid." from vtiger_workflowstages where workflowsid=? ORDER BY sequence";
		//echo $query;die();
		$qparams = array($wordflowsid);
		$adb->pquery($query,$qparams);
		//默认更新第一个活动节点
		
		$sql1="update vtiger_salesorderworkflowstages set isaction=1, actiontime=? where sequence=1 and salesorderid=? and workflowsid=?";
		
		$adb->pquery($sql1,array(date('Y-m-d h:i:s'),$crmid,$wordflowsid));
	}
...
/**
 * 保存工单的产品详细
 * @param unknown $focus 对象
 * @param unknown $module 模块名称
 * @param string $update_prod_stock 
 * @param string $updateDemand
 */
function saveProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand=''){
	global $log, $adb;
	if($focus->mode == 'edit'){
	...
	$tot_no_prod = $_REQUEST['totalProductCount'];//产品总数
	for($i=1; $i<=$tot_no_prod; $i++){
		if($_REQUEST["deleted".$i] == 1)   //他的删除竟然在页面上还有残留物质
			continue;
