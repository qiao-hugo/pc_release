...
class PearDatabase{
    ...
    function startTransaction() {
		...
		//事务开始/gaocl 2014-12-26
		//mysql_query("BEGIN");
    }

    function completeTransaction() {
    	//事务结束/gaocl 2014-12-26 start
	    //if($this->isPostgres()) return;
...
// 			throw new AppException('数据操作失败,请检查! ');
// 			exit;
		}
		else 
		{
	...
	function logSqlTiming($startat, $endat, $sql, $params=false) {
		...
		//young.yang 如果调试打开速度慢，可将这里插入到数据库中的操作关闭
		$sql_array['sql']=$sql;
		$sql_array['sql_param']=$params;//implode(",", (is_array($params) && !empty($params))?$params:array())?"\n[" . implode(",", $params) . "]":"";
		$sql_array['sql_param1']=implode(",", (is_array($params) && !empty($params))?$params:array())?"\n[" . implode(",", $params) . "]":"";
		$type = 'CALLERS';
		$data = array();
		...
		for ($calleridx = 0, $callerscount = count($callers); $calleridx < $callerscount; ++$calleridx) {
			...
			$sql_array['lines'][$calleridx]['line']=$callers[$calleridx]['line'];
			$sql_array['lines'][$calleridx]['file']=$callers[$calleridx]['file'];
			$sql_array['lines'][$calleridx]['func']=$callerfunc;
		}
		$sql_array['exectime']=($endat-$startat).'s';
		$sql_array['mem']=round(memory_get_usage()/1048576,2).'M';
		if (DEVELAPOR){
		$GLOBALS['sql_trace_all_array'][]=$sql_array;
		unset($sql_array);}
	...
	function pquery($sql, $params=array(), $dieOnError=false, $msg='') {
		...
		$sql_start_time = microtime(true);
		$sql_start_mem=memory_get_usage();
