 ...
 * All Rights Reserved.
 * @description 这里是生成的crm相关的信息，包括用户信息，传递过来的webservice，已经权限访问，模块字段,以及从模块读取的基本配置信息
 * @history
 * 		2014-09-08 young.yang 
 *************************************************************************************/
...
class VtigerCRMObjectMeta extends EntityMeta {
	...
	private $meta;//只是一个验证左右，相关的数据是否生成
	private $assign;
	private $hasAccess;
	...
	function VtigerCRMObjectMeta($webserviceObject,$user){
		...
		$instance = vtws_getModuleInstance($this->webserviceObject);//核心部分的代码，即调用自定义的模块的地方，
	...
	/**
	 * 核心的权限验证的地方，在这里模块是否可以访问等
	 * young.yang@trueland.net 2014-09-08
	 */
	private function computeAccess(){
		
		global $adb;
		
		$active = vtlib_isModuleActive($this->getTabName());//跟目录下的tabdata.php文件，就是一个presence属性0为可访问
		if($active == false){
			$this->hasAccess = false;
			$this->hasReadAccess = false;
			$this->hasWriteAccess = false;
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			//$profileList = getCurrentUserProfileList();   //同样是读取用户的缓存文件,已修改如下106-110行代码
	...
	function getFieldColumnMapping(){
		...
		if(!$this->meta){
			$this->retrieveMeta();//这里就是吧meta初始化了
		}
	...
	function getReferenceFieldDetails(){	
		if(!$this->meta){//meta=true ，目的是是否执行了retrievemeta方法，方法是为了初始化一些数据,meta
			$this->retrieveMeta();
		}
		return parent::getReferenceFieldDetails();//返回
	}
	...
	function retrieveMeta(){
		...
		$current_language = vtws_preserveGlobal('current_language',$current_language);//没看懂
		
		$this->computeAccess();//权限验证
		
		$cv = new CustomView();
		$module_info = $cv->getCustomViewModuleInfo($this->getTabName());//传值带有tab名称，在customview中生成，但返回的数据还是全部
		$blockArray = array();
		foreach($cv->module_list[$this->getTabName()] as $label=>$blockList){
			$blockArray = array_merge($blockArray,explode(',',$blockList));
		}
		
		$this->retrieveMetaForBlock($blockArray);//生成相关的字段和块
		
		$this->meta = true;
		VTWS_PreserveGlobal::flush();//刷新全局
	}
    ...
    private function retrieveMetaForBlock_b($block){
        global $adb;

        $tabid = $this->getTabId();
        require('user_privileges/user_privileges_'.$this->user->id.'.php');  //加载当前人员的权限配置文件
        //管理员或者有全局权限的
        if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
            ...
            $profileList = getCurrentUserProfileList();//将当前用户的配置（跟角色关联的，或者是自定义，
	...
	//生成相关的字段在这里进行的 young.yang
	private function retrieveMetaForBlock($block){

        //$allmoduleFields=array();
        //$mymoduleFields=array();
	 	    $tabid = $this->getTabId();
            $allmoduleFields=Vtiger_Cache::get('global','retrieveMetaForBlock');
            if(empty($allmoduleFields)){
            global $adb;
            //require('user_privileges/user_privileges_'.$this->user->id.'.php');  //加载当前人员的权限配置文件
            //管理员或者有全局权限的
            //暂时忽略字段的权限控制，在这里都是有权限的
            $sql = "select *, '0' as readonly from vtiger_field where  displaytype in (1,2,3,4,5)";
            $params = array();
...
//		}
            ...
            for($i=0; $i<$noofrows; $i++){
                $fieldname = $adb->query_result($result,$i,"fieldname");
                $alltabid = $adb->query_result($result,$i,"tabid");
                $allblock = $adb->query_result($result,$i,"block");
                if(strcasecmp($fieldname,'imagename')===0){
                    continue;
                }
                $webserviceField = WebserviceField::fromQueryResult($adb,$result,$i);
                $allmoduleFields[$alltabid][$allblock][$webserviceField->getFieldName()]=$webserviceField;
                //$this->moduleFields[$webserviceField->getFieldName()] = $webserviceField;
            }
            Vtiger_Cache::set('global','retrieveMetaForBlock',$allmoduleFields);
        }
        ...
        foreach($block as $val){
            if($allmoduleFields[$tabid][$val]){
                $temp=$allmoduleFields[$tabid][$val];
                foreach($temp as $key=>$val){
                    $mymoduleFields[$val->getFieldName()]=$temp[$key];
                }
            }
        }
        $this->moduleFields=$mymoduleFields;
	}
