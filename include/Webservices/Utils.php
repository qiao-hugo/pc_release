...
/**
 * 获取相同组的用户 Function to return all the users in the groups that this user is part of.
...
/** 
 * 返回crm实体的列表状态isentitytype=1.function to get the module List to which are crm entities.
...
function vtws_getWebserviceEntities(){
	...
	include 'crmcache/ws_entity.php';
	return $ws_entity;
}

/**
 * 读取页面module方法的地方，即module/模块/模块.php
...
function vtws_getModuleHandlerFromName($name,$user){
	global $adb, $log;
	$webserviceObject = VtigerWebserviceObject::fromName($adb,$name);//读取vtiger_ws_entity数据
	...
	//$a=debug_backtrace();
	//echo $a[0]['file'].$a[1]['function'].$a[1]['line'].'<br>';die();
	return $handler;
}
