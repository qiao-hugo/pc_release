 ...
 *************************************************************************************/
	...
	function vtws_listtypes($fieldTypeList, $user){
		...
		if(!empty($fieldTypeList)) {
			$fieldTypeList = array_map(strtolower, $fieldTypeList);//转化小写
			sort($fieldTypeList);//排序
			$fieldTypeString = implode(',', $fieldTypeList);//拆分数组为字符串
		} else {
			$fieldTypeString = 'all';
		}
		...
		try{
			...
			$allModuleNames = getPermittedModuleNames();//获得当前用户所有有权限访问的模块
			
			
			if(array_search('Calendar',$allModuleNames) !== false){
				array_push($allModuleNames,'Events');//事件在这里是一个碍事的，如果能够取消掉可能会简单很多
			}
			...
			if($webserviceEntities === false || !CRMEntity::isBulkSaveMode()) {
				// Bulk Save Mode: For re-using information
				$webserviceEntities = vtws_getWebserviceEntities();//返回array('module'=>模块,'entity'=>实体);
			}
			//返回的可访问的模块
			$accessibleModules = array_values(array_intersect($webserviceEntities['module'],$allModuleNames));
			
			$entities = $webserviceEntities['entity'];//所有的实体
			...
			if(empty($fieldTypeList)) {
				foreach($entities as $entity){
					...
					if($meta->hasAccess()===true){//实例化，根据vtiger_ws_entity获得的权限设置可访问的实体
		...
		$current_language = vtws_preserveGlobal('current_language',$current_language);//设置当前的语言
