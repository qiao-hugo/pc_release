...
abstract class EntityMeta{
	...
	public function getReferenceFieldDetails(){
		if($this->referenceFieldDetails === null){
			...
			//print_r($moduleFields);die();
			foreach ($moduleFields as $fieldName=>$webserviceField) {
				if(strcasecmp($webserviceField->getFieldDataType(),'reference') === 0){
					$this->referenceFieldDetails[$fieldName] = $webserviceField->getReferenceList();//返回当前可访问的reference类型字段
				}
			}
		}
	...
	/**
	 * 将vtiger_ws_fieldtype类型=0wner（id=53）字段全部查找到，一般关联的是vtiger_crmentity.smownerid
	 */
	...
	/**
	 * 返回where条件第一个条件，为通用条件
	 * @history 
	 * 	2014-10-27 young 包含vtiger_crmentity和不包含的分别处理
	 */
	public function getEntityDeletedQuery(){
		...
		if(is_array($this->tableList)&&in_array('vtiger_crmentity', $this->tableList)){
